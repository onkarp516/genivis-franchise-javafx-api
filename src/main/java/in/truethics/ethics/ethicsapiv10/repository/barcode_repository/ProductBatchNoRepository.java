...
//    List<ProductBatchNo> findByBatchList1(Long productId, Long id, boolean b);
    ...
    List<ProductBatchNo> findByProductIdAndStatusOrderByIdDesc(Long id, boolean b);

    @Query(
            value = "SELECT product_batchno_tbl.* FROM product_batchno_tbl JOIN product_barcode_tbl ON " +
                    "product_batchno_tbl.product_id=product_barcode_tbl.product_id AND " +
                    "product_batchno_tbl.id=product_barcode_tbl.batch_id where " +
                    "product_barcode_tbl.product_id=?1 AND (product_barcode_tbl.barcode_unique_code=?2 OR product_barcode_tbl.company_barcode=?2) AND product_batchno_tbl.status=?3",
            nativeQuery = true
    )
    List<ProductBatchNo> findBatchDataByBarcode(Long productId,String barcode, boolean status);
