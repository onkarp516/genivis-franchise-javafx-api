...
public interface TransSalesPaymentTypeRepository extends JpaRepository<TranxSalesPaymentType,Long> {
    ...
    @Query(value = "SELECT * FROM sales_payment_type_tbl WHERE " +
            "counter_sale_id=?1 AND status=?2 group by type",nativeQuery = true)
    List<TranxSalesPaymentType> findByCounterSaleId(Long id, boolean b);

    @Query(value = "SELECT * FROM sales_payment_type_tbl WHERE " +
            "tranx_sales_comp_invoice_id=?1 AND status=?2 group by type",nativeQuery = true)
    List<TranxSalesPaymentType> findByConsumerSaleById(Long id, boolean b);

    List<TranxSalesPaymentType> findByTranxSalesInvoiceIdAndStatusAndType(Long id, boolean b, String type);
    List<TranxSalesPaymentType> findByCounterSaleIdAndStatusAndType(Long id, boolean b, String type);
    List<TranxSalesPaymentType> findByTranxSalesCompInvoiceIdAndStatusAndType(Long id, boolean b, String type);
}
