...
public interface TranxCounterSalesRepository extends JpaRepository<TranxCounterSales, Long> {
    ...
    /*** Specifi for Counter Sale  of Franchsie fro Mobile App *****/
    @Query(
            value = " SELECT COUNT(*) FROM tranx_counter_sales_tbl WHERE outlet_id=?1 AND " +
                    "branch_id=?2 AND status=?3 AND DATE(transaction_date)=?4", nativeQuery = true
    )
    Long findFrLastRecordWithBranch(Long id, Long branchId, Boolean flag,LocalDate date);
    ...
    @Query(
            value = " SELECT COUNT(*) FROM tranx_counter_sales_tbl WHERE outlet_id=?1 AND " +
                    "branch_id IS NULL AND status=?2 AND DATE(transaction_date)=?3" , nativeQuery = true
    )
    Long findFrLastRecord(Long id, Boolean flag,LocalDate date);
