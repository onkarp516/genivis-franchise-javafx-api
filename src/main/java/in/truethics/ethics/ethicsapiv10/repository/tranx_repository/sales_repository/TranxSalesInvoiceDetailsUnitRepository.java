...
public interface TranxSalesInvoiceDetailsUnitRepository extends JpaRepository<TranxSalesInvoiceDetailsUnits, Long> {
    ...
    @Query(value = "Select sum(tranx_sales_invoice_details_units_tbl.total_amount) as amt, tranx_sales_invoice_details_units_tbl.rate, " +
            "tranx_sales_invoice_details_units_tbl.product_id, product_tbl.product_name, packing_master_tbl.pack_name from tranx_sales_invoice_details_units_tbl, " +
            "product_tbl, packing_master_tbl where outlet_id=?1 And branch_id=?2 And status=1 And tranx_sales_invoice_details_units_tbl.product_id=product_tbl.id and" +
            " product_tbl.packing_master_id=packing_master_tbl.id group by tranx_sales_invoice_details_units_tbl.product_id order by amt asc",nativeQuery = true)
    List<TranxSalesInvoiceDetailsUnits> findLeastSelling(Long id, Long id1, boolean b);

    @Query(value = "Select sum(tranx_sales_invoice_details_units_tbl.total_amount) as amt, tranx_sales_invoice_details_units_tbl.rate, " +
            "tranx_sales_invoice_details_units_tbl.product_id, product_tbl.product_name, packing_master_tbl.pack_name from tranx_sales_invoice_details_units_tbl, " +
            "product_tbl, packing_master_tbl where outlet_id=?1 And branch_id=?2 And status=1 And tranx_sales_invoice_details_units_tbl.product_id=product_tbl.id and" +
            " product_tbl.packing_master_id=packing_master_tbl.id group by tranx_sales_invoice_details_units_tbl.product_id order by amt desc",nativeQuery = true)

    List<TranxSalesInvoiceDetailsUnits> findTopSelling(Long id, Long id1, boolean b);
}