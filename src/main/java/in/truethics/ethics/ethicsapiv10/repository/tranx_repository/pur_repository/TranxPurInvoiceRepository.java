...
*/
    ...
    @Query(value="SELECT * FROM `tranx_purchase_invoice_tbl` WHERE outlet_id = ?1 And branch_id=?2 And status=1 ORDER BY id DESC LIMIT 5",nativeQuery = true)
    List<TranxPurInvoice> findPurchaseInvoice(Long id, Long id1, boolean b);

    @Query(value="SELECT * FROM `tranx_purchase_invoice_tbl` WHERE outlet_id = ?1 And branch_id Is Null  And status=1 ORDER BY id DESC LIMIT 5",nativeQuery = true)

    List<TranxPurInvoice> findPucrhaseInvoiceBranchNull(Long id, boolean b);
}
