...
public interface SalesmanMasterRepository extends JpaRepository<SalesManMaster, Long> {
    ...
    @Query(
            ...
            "middle_name=?4 AND last_name=?5 AND status=?6",  nativeQuery = true
    )
   SalesManMaster findDuplicateWithBranch(Long outletId,  Long branchId, String firstName, String middleName, String lastName, boolean b);
    @Query(
            value = "SELECT * FROM `salesman_master_tbl` WHERE outlet_id=?1 AND first_name=?2 AND " +
            "middle_name=?3 AND last_name=?4 AND status=?5 AND branch_id IS NULL", nativeQuery = true
    )
  SalesManMaster findDuplicate(Long outletId, String firstName, String middleName, String lastName, boolean b);
