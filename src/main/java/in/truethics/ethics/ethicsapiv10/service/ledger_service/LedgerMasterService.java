...
//            ledgerRepository.save(ledgerMaster);
    ...
    public JsonObject getGvBankDetails(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        JsonObject response = new JsonObject();
        JsonArray result = new JsonArray();
        List<LedgerBankDetails> bankDetails = new ArrayList<>();
        LedgerMaster mLedger = ledgerRepository.findByLedgerCodeAndStatusAndOutletIdAndBranchIsNull("gvmh001",
                true, users.getOutlet().getId());
        bankDetails = ledgerbankDetailsRepository.findByLedgerMasterIdAndStatus(mLedger.getId(), true);
        if (bankDetails != null && bankDetails.size() > 0) {
            for (LedgerBankDetails mDetails : bankDetails) {
                JsonObject mObject = new JsonObject();
                mObject.addProperty("id", mDetails.getId());
                mObject.addProperty("bank_name", mDetails.getBankName());
                mObject.addProperty("account_no", mDetails.getAccountNo());
                mObject.addProperty("ledger_code", "gvmh001");
                result.add(mObject);
            }
        }
        response.addProperty("message", "success");
    ...
    public JSONArray getLedgersJSONArray(String key, Long outletId, Branch branch) {
        Principles principles = principleRepository.findByPrincipleNameIgnoreCaseAndStatus(key, true);
        List<LedgerMaster> indirect_incomes = new ArrayList<>();
        if (branch != null) {
            indirect_incomes = ledgerRepository.findByOutletIdAndBranchIdAndPrinciplesIdAndStatus(outletId, branch.getId(), principles.getId(), true);
        ...
        for (LedgerMaster mAccount : indirect_incomes) {
            JSONObject response = new JSONObject();
            response.put("id", mAccount.getId());
            response.put("name", mAccount.getLedgerName());
            response.put("unique_code", principles.getUniqueCode());
            result.put(response);
        }
        return result;
    }
