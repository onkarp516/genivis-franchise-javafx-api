...
import in.truethics.ethics.ethicsapiv10.dto.purchasedto.PurInvoiceDTO;
...
public class TranxPurInvoiceService {
        ...
        try {
            ...
            if (mPurchaseTranx != null) {
                ...
                LedgerMaster ledgerMaster = ledgerMasterRepository
                        .findByIdAndStatus(mPurchaseTranx.getSundryCreditors().getId(), true);
    ...
    private void createPaymentInvoice(TranxPurInvoice newInvoice, Users users, String paymentType, Double totalPaidAmt,
            Double returnAmt, String paymentMode, String key) {
        ...
        if (newInvoice.getBranch() != null) {
            count = tranxPaymentMasterRepository.findBranchLastRecord(users.getOutlet().getId(),
        ...
        if (newInvoice.getFiscalYear() != null)
            tranxPaymentMaster.setFiscalYear(newInvoice.getFiscalYear());
        ...
        if (key.equalsIgnoreCase("create")) {
            insertIntoPaymentPerticualrs(newTranxPaymentMaster, sundryDebtors, "SC", totalPaidAmt, key, paymentMode,
            ...
            if (paymentType != null) {
                ...
                // ledgerMasterRepository.findByIdAndStatus(object.get("id").getAsLong(), true);
                // insertIntoPaymentPerticualrs(newTranxPaymentMaster, ledgerMaster,
                // object.get("type").getAsString(), object.get("amount").getAsDouble(), key,
            ...
            /***** Sales Invoice by Cash Only : Receipt Details of Cash Account only ******/
                ...
                insertIntoPaymentPerticualrs(newTranxPaymentMaster, ledgerMaster, "others", totalPaidAmt, key, "cash",
                        tranxType, newInvoice);
            }
        } /*
           * else {
    ...
    private void insertIntoPaymentPerticualrs(TranxPaymentMaster newTranxPaymentMaster, LedgerMaster ledgerMaster,
            String type, double amount, String key, String payMode, TransactionTypeMaster tranxType,
            TranxPurInvoice newInvoice) {
        ...
        tranxPaymentPerticulars
                .setTransactionDate(DateConvertUtil.convertDateToLocalDate(newTranxPaymentMaster.getTranscationDate()));
         ...
         * ledgerMaster.getAssociateGroups(), newTranxPaymentMaster.getFiscalYear(),
         * newTranxPaymentMaster.getBranch(), newTranxPaymentMaster.getOutlet(),
         * newTranxPaymentMaster.getTranscationDate(), newTranxPaymentMaster.getId(),
         * newTranxPaymentMaster.getPaymentNo(), ledgerType, true,
        ...
        ledgerCommonPostings.callToPostingsTranxCode(amount, ledgerMaster, tranxType, ledgerMaster.getAssociateGroups(),
                newTranxPaymentMaster.getFiscalYear(), newTranxPaymentMaster.getBranch(),
                newTranxPaymentMaster.getOutlet(),
                newTranxPaymentMaster.getTranscationDate(), newTranxPaymentMaster.getId(),
                newTranxPaymentMaster.getPaymentNo(), ledgerType, true, tranxType.getTransactionCode(), "Insert",
                newTranxPaymentMaster.getTranxCode());
    ...
    private void InsertIntoBillDetails(TranxPaymentPerticulars paymentPerticulars, String key,
            TranxPurInvoice newInvoice) {
        TranxPaymentPerticularsDetails tranxPaymentDetails = new TranxPaymentPerticularsDetails();
        if (key.equalsIgnoreCase("update")) {
            tranxPaymentDetails = tranxPaymentPerticularsDetailsRepository
                    .findByStatusAndTranxPaymentPerticularsId(true, paymentPerticulars.getId());
        }
        if (key.equalsIgnoreCase("create")) {
    ...
    private void insertIntoLedgerTranxDetails(TranxPurInvoice mPurchaseTranx, HttpServletRequest request) {
        ...
        try {
            ...
            // insertIntoTranxDetailPD(mPurchaseTranx, tranxType, "CR", "Insert"); // for
            // Purchase Discounts : cr
            insertIntoTranxDetailRO(mPurchaseTranx, tranxType); // for Round Off : cr or dr
            // insertDB(mPurchaseTranx, "AC", tranxType, "DR", "Insert"); // for Additional
            // Charges : dr
            ...
            if (mPurchaseTranx.getTcsMode().equalsIgnoreCase("tcs"))
                ...
                insertIntoTCS(mPurchaseTranx, "TDS", tranxType, "DR", "Insert");
    ...
    private void insertIntoTCS(TranxPurInvoice mPurchaseTranx, String type, TransactionTypeMaster tranxType,
            String crdr, String operation) {
        try {
            /**** New Postings Logic *****/
            LedgerMaster mLedger = null;
            if (type.equalsIgnoreCase("tcs")) {
                if (mPurchaseTranx.getBranch() != null)
                    mLedger = ledgerMasterRepository.findByLedgerCodeAndStatusAndOutletIdAndBranchId("tcs", true,
                            mPurchaseTranx.getOutlet().getId(), mPurchaseTranx.getBranch().getId());
                else {
                    mLedger = ledgerMasterRepository.findByLedgerCodeAndStatusAndOutletIdAndBranchIsNull("tcs", true,
                            mPurchaseTranx.getOutlet().getId());
                }
            } else if (type.equalsIgnoreCase("tds")) {
                if (mPurchaseTranx.getBranch() != null)
                    mLedger = ledgerMasterRepository.findByLedgerCodeAndStatusAndOutletIdAndBranchId("tds", true,
                            mPurchaseTranx.getOutlet().getId(), mPurchaseTranx.getBranch().getId());
                else {
                    mLedger = ledgerMasterRepository.findByLedgerCodeAndStatusAndOutletIdAndBranchIsNull("tds", true,
                            mPurchaseTranx.getOutlet().getId());
                }
            }
            ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getTcsAmt(), mLedger, tranxType,
                    mLedger.getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(),
                    mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(), mPurchaseTranx.getId(),
                    mPurchaseTranx.getVendorInvoiceNo(), crdr, true, tranxType.getTransactionCode(), operation,
    ...
    private void deleteIntoLedgerTranxDetails(TranxPurInvoice mPurchaseTranx) {
        ...
        try {
            ...
            // insertIntoTranxDetailPD(mPurchaseTranx, tranxType, "DR", "Delete"); // for
            ...
            /* end of ledger transaction details */
            /****
             * make Status 0 to postings of corresponding ledgers , Its is used when we are
             * displyaing ledger details list
             ****/
    ...
    public void insertDB(TranxPurReturnInvoice mPurchaseTranx, String ledgerName, TransactionTypeMaster tranxType,
            String crdrType, String operation) {
    ...
    public void insertFromAdditionalCharges(TranxPurReturnInvoiceAddCharges mAdditinoalCharges,
            TranxPurReturnInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String crdrType, String operation) {
        /**** New Postings Logic *****/
        ledgerCommonPostings.callToPostings(mAdditinoalCharges.getAmount(), mAdditinoalCharges.getAdditionalCharges(),
                tranxType, mAdditinoalCharges.getAdditionalCharges().getAssociateGroups(),
                mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(),
                mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), crdrType, true,
        ...
        if (operation.equalsIgnoreCase("delete")) {
            ...
            LedgerOpeningClosingDetail ledgerDetail = ledgerOpeningClosingDetailRepository
                    .findByLedgerMasterIdAndTranxTypeIdAndTranxIdAndStatus(
                            mAdditinoalCharges.getAdditionalCharges().getId(), tranxType.getId(),
            ...
            if (ledgerDetail != null) {
                ...
                /***** NEW METHOD FOR LEDGER POSTING *****/
                postingUtility.updateLedgerPostings(mAdditinoalCharges.getAdditionalCharges(),
                        mPurchaseTranx.getPurReturnDate(),
                        tranxType, mPurchaseTranx.getFiscalYear(), detail);
            }
        }
    ...
    /*
     * Insertion into Transaction Details Table of Sundry Creditors Ledgers for
     * Purchase Invoice
     */
    public void insertIntoTranxDetailSC(TranxPurReturnInvoice mPurchaseTranx, TransactionTypeMaster tranxType,
        ...
        try {
            ...
            ledgerCommonPostings.callToPostings(mPurchaseTranx.getTotalAmount(),
                    ...
                    mPurchaseTranx.getOutlet(), mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(),
                    mPurchaseTranx.getPurRtnNo(), crdrType, true, "Purchase Return", operation);

            if (operation.equalsIgnoreCase("delete")) {
                /**** NEW METHOD FOR LEDGER POSTING ****/
                LedgerOpeningClosingDetail ledgerDetail = ledgerOpeningClosingDetailRepository
                        .findByLedgerMasterIdAndTranxTypeIdAndTranxIdAndStatus(
                                mPurchaseTranx.getSundryCreditors().getId(), tranxType.getId(), mPurchaseTranx.getId(),
                                true);
                if (ledgerDetail != null) {
                    Double closing = Constants.CAL_DR_CLOSING(ledgerDetail.getOpeningAmount(), 0.0, 0.0);
                    ledgerDetail.setAmount(0.0);
                    ...
                    /***** NEW METHOD FOR LEDGER POSTING *****/
                    postingUtility.updateLedgerPostings(mPurchaseTranx.getSundryCreditors(),
                            mPurchaseTranx.getPurReturnDate(),
                            tranxType, mPurchaseTranx.getFiscalYear(), detail);
                }
            }
    ...
    /*
     * Insertion into Transaction Details Table of Purchase Accounts Ledgers for
     * Purchase Invoice
     */
    public void insertIntoTranxDetailPA(TranxPurReturnInvoice mPurchaseTranx,
        ...
        ledgerCommonPostings.callToPostings(mPurchaseTranx.getTotalBaseAmount(),
                mPurchaseTranx.getPurchaseAccountLedger(), tranxType,
                mPurchaseTranx.getPurchaseAccountLedger().getAssociateGroups(),
                mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(),
        ...
        if (operation.equalsIgnoreCase("delete")) {
            ...
            LedgerOpeningClosingDetail ledgerDetail = ledgerOpeningClosingDetailRepository
                    .findByLedgerMasterIdAndTranxTypeIdAndTranxIdAndStatus(
                            mPurchaseTranx.getPurchaseAccountLedger().getId(), tranxType.getId(),
                            mPurchaseTranx.getId(), true);
            if (ledgerDetail != null) {
                Double closing = Constants.CAL_DR_CLOSING(ledgerDetail.getOpeningAmount(), 0.0, 0.0);
                ledgerDetail.setAmount(0.0);
                ...
                /***** NEW METHOD FOR LEDGER POSTING *****/
                postingUtility.updateLedgerPostings(mPurchaseTranx.getPurchaseAccountLedger(),
                        mPurchaseTranx.getPurReturnDate(),
                        tranxType, mPurchaseTranx.getFiscalYear(), detail);
            }
        }
    ...
    /*
     * Insertion into Transaction Details Table of Purchase Discount Ledgers for
     * Purchase Invoice
     */
        ...
        try {
            if (mPurchaseTranx.getPurchaseDiscountLedger() != null) {
                ...
                if (operation.equalsIgnoreCase("delete")) {
                    ...
                    LedgerOpeningClosingDetail ledgerDetail = ledgerOpeningClosingDetailRepository
                            .findByLedgerMasterIdAndTranxTypeIdAndTranxIdAndStatus(
                                    mPurchaseTranx.getPurchaseDiscountLedger().getId(), tranxType.getId(),
                                    mPurchaseTranx.getId(), true);
                    if (ledgerDetail != null) {
                        Double closing = Constants.CAL_DR_CLOSING(ledgerDetail.getOpeningAmount(), 0.0, 0.0);
                        ledgerDetail.setAmount(0.0);
                        ...
                        /***** NEW METHOD FOR LEDGER POSTING *****/
                        postingUtility.updateLedgerPostings(mPurchaseTranx.getPurchaseDiscountLedger(),
                                mPurchaseTranx.getPurReturnDate(),
                                tranxType, mPurchaseTranx.getFiscalYear(), detail);
                    }
                }
     ...
     * Purchase Invoice
     */
    public void insertIntoTranxDetailSC(TranxPurInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String type,
            String operation) {
        try {
            /**** New Postings Logic *****/
            Double amt = 0.0;
            if (operation.equalsIgnoreCase("Insert"))
                amt = mPurchaseTranx.getBalance();
            else
                amt = mPurchaseTranx.getTotalAmount();
            // ledgerCommonPostings.callToPostings(amt, mPurchaseTranx.getSundryCreditors(),
            // tranxType, mPurchaseTranx.getSundryCreditors().getAssociateGroups(),
            // mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(),
            // mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
            // mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), type, true,
            // tranxType.getTransactionCode(), operation);
            ledgerCommonPostings.callToPostingsTranxCode(amt, mPurchaseTranx.getSundryCreditors(), tranxType,
                    mPurchaseTranx.getSundryCreditors().getAssociateGroups(), mPurchaseTranx.getFiscalYear(),
                    mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                    mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), type, true,
                    tranxType.getTransactionCode(), operation, mPurchaseTranx.getTranxCode());

            if (operation.equalsIgnoreCase("insert")) {
                /**** NEW METHOD FOR LEDGER POSTING ****/
                postingUtility.callToPostingLedger(tranxType, type, amt, mPurchaseTranx.getFiscalYear(),
                        mPurchaseTranx.getSundryCreditors(),
                        mPurchaseTranx.getInvoiceDate(), mPurchaseTranx.getId(), mPurchaseTranx.getOutlet(),
                        mPurchaseTranx.getBranch(), mPurchaseTranx.getTranxCode());
            }
            if (operation.equalsIgnoreCase("delete")) {
                /**** NEW METHOD FOR LEDGER POSTING ****/
                LedgerOpeningClosingDetail ledgerDetail = ledgerOpeningClosingDetailRepository
    ...
    /*
     * Deletion of Transaction Details Table of Purchase RoundOff Ledgers for
     * Purchase Invoice
     */
    private void deleteIntoTranxDetailRO(TranxPurInvoice mPurchaseTranx, TransactionTypeMaster tranxType) {
        try {
            if (mPurchaseTranx.getRoundOff() >= 0) {
                /**** New Postings Logic *****/
                ledgerCommonPostings.callToPostings(mPurchaseTranx.getRoundOff(), mPurchaseTranx.getPurchaseRoundOff(),
                        tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(),
                        mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(),
                        mPurchaseTranx.getInvoiceDate(), mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(),
                        "CR", true, "Purchase Invoice", "Delete");
            } else if (mPurchaseTranx.getRoundOff() < 0) {
                ledgerCommonPostings.callToPostings(mPurchaseTranx.getRoundOff(), mPurchaseTranx.getPurchaseRoundOff(),
                        tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(),
                        mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(),
                        mPurchaseTranx.getInvoiceDate(), mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(),
                        "DR", true, "Purchase Invoice", "Delete");
            }
            ...
            if (operation.equalsIgnoreCase("delete")) {
                ...
                LedgerOpeningClosingDetail ledgerDetail = ledgerOpeningClosingDetailRepository
                        .findByLedgerMasterIdAndTranxTypeIdAndTranxIdAndStatus(
                                mPurchaseTranx.getPurchaseRoundOff().getId(), tranxType.getId(), mPurchaseTranx.getId(),
                                true);
                if (ledgerDetail != null) {
                    Double closing = Constants.CAL_DR_CLOSING(ledgerDetail.getOpeningAmount(), 0.0, 0.0);
                    ledgerDetail.setAmount(0.0);
                    ...
                    /***** NEW METHOD FOR LEDGER POSTING *****/
                    postingUtility.updateLedgerPostings(mPurchaseTranx.getPurchaseRoundOff(),
                            mPurchaseTranx.getInvoiceDate(),
                            tranxType, mPurchaseTranx.getFiscalYear(), detail);
                }
            }
    ...
    private void deleteIntoTranxDetailRODebitNote(TranxPurReturnInvoice mPurchaseTranx,
        ...
        try {
            if (mPurchaseTranx.getRoundOff() >= 0) {
                ...
                ledgerCommonPostings.callToPostings(mPurchaseTranx.getRoundOff(), mPurchaseTranx.getPurchaseRoundOff(),
                        tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(),
                        mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(),
                        mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), "DR",
                        true, "Purchase Invoice", "Delete");
            } else if (mPurchaseTranx.getRoundOff() < 0) {
                ledgerCommonPostings.callToPostings(mPurchaseTranx.getRoundOff(), mPurchaseTranx.getPurchaseRoundOff(),
                        tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(),
                        mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(),
                        mPurchaseTranx.getPurReturnDate(), mPurchaseTranx.getId(), mPurchaseTranx.getPurRtnNo(), "CR",
                        true, "Purchase Invoice", "Delete");
            }
     ...
     * Purchase Invoice
     */
    public void insertIntoTranxDetailPA(TranxPurInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String type,
            String operation) {
        try {
            /**** New Postings Logic *****/
            ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getTaxableAmount(),
                    mPurchaseTranx.getPurchaseAccountLedger(), tranxType,
                    mPurchaseTranx.getPurchaseAccountLedger().getAssociateGroups(), mPurchaseTranx.getFiscalYear(),
                    mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                    mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), type, true,
                    tranxType.getTransactionCode(), operation, mPurchaseTranx.getTranxCode());
            if (operation.equalsIgnoreCase("insert")) {
                ...
                postingUtility.callToPostingLedger(tranxType, type, mPurchaseTranx.getTaxableAmount(),
                        mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getPurchaseAccountLedger(),
                        mPurchaseTranx.getInvoiceDate(),
                        mPurchaseTranx.getId(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getBranch(),
                        mPurchaseTranx.getTranxCode());
            }
     ...
     * Purchase Invoice
     */
    public void insertIntoTranxDetailPD(TranxPurInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String cr,
            String operation) {
        try {
            if (mPurchaseTranx.getPurchaseDiscountLedger() != null) {
                /**** New Postings Logic *****/
                ledgerCommonPostings.callToPostings(mPurchaseTranx.getPurchaseDiscountAmount(),
                        mPurchaseTranx.getPurchaseDiscountLedger(), tranxType,
                        mPurchaseTranx.getPurchaseDiscountLedger().getAssociateGroups(), mPurchaseTranx.getFiscalYear(),
                        mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                        mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), cr, true,
                        tranxType.getTransactionCode(), operation);
            }
        } catch (Exception e) {
            StringWriter sw = new StringWriter();
    ...
    /*
     * Insertion into Transaction Details Table of Purchase RoundOff Ledgers for
     * Purchase Invoice
     */
    private void insertIntoTranxDetailRO(TranxPurInvoice mPurchaseTranx, TransactionTypeMaster tranxType) {
        try {
            ...
            if (mPurchaseTranx.getRoundOff() >= 0) {
                tranxAction = "DR";
                /**** New Postings Logic *****/
                // ledgerCommonPostings.callToPostings(mPurchaseTranx.getRoundOff(),
                // mPurchaseTranx.getPurchaseRoundOff(), tranxType,
                // mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(),
                // mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(),
                // mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                // mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), "DR", true,
                // tranxType.getTransactionCode(), "Insert");
                ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getRoundOff(),
                        mPurchaseTranx.getPurchaseRoundOff(),
                        tranxType, mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(),
                        mPurchaseTranx.getFiscalYear(),
                        mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                        mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), "DR", true,
                        tranxType.getTransactionCode(), "Insert", mPurchaseTranx.getTranxCode());
            } else if (mPurchaseTranx.getRoundOff() < 0) {
                // ledgerCommonPostings.callToPostings(Math.abs(mPurchaseTranx.getRoundOff()),
                // mPurchaseTranx.getPurchaseRoundOff(), tranxType,
                // mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(),
                // mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(),
                // mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                // mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), "CR", true,
                // tranxType.getTransactionCode(), "Insert");
                ledgerCommonPostings.callToPostingsTranxCode(Math.abs(mPurchaseTranx.getRoundOff()),
                        mPurchaseTranx.getPurchaseRoundOff(), tranxType,
                        mPurchaseTranx.getPurchaseRoundOff().getAssociateGroups(), mPurchaseTranx.getFiscalYear(),
                        mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                        mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), "CR", true,
                        tranxType.getTransactionCode(), "Insert", mPurchaseTranx.getTranxCode());
            }
            ...
            postingUtility.callToPostingLedger(tranxType, tranxAction, Math.abs(mPurchaseTranx.getRoundOff()),
                    mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getPurchaseRoundOff(),
                    mPurchaseTranx.getInvoiceDate(),
                    mPurchaseTranx.getId(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getBranch(),
                    mPurchaseTranx.getTranxCode());
        } catch (Exception e) {
    ...
    public void insertDB(TranxPurInvoice mPurchaseTranx, String ledgerName, TransactionTypeMaster tranxType,
        ...
        if (ledgerName.equalsIgnoreCase("DT")) {
            ...
            if (mPurchaseTranx.getAdditionLedger3() != null) {
                ledgerCommonPostings.callToPostingsTranxCode(mPurchaseTranx.getAdditionLedgerAmt3(),
                        mPurchaseTranx.getAdditionLedger3(), tranxType,
                        mPurchaseTranx.getAdditionLedger3().getAssociateGroups(), mPurchaseTranx.getFiscalYear(),
                        mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                        mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(),
                        mPurchaseTranx.getAdditionLedgerAmt3() > 0 ? "DR" : "CR", true, tranxType.getTransactionCode(),
                        operation, mPurchaseTranx.getTranxCode());
            }
        }
    }

    public void insertFromDutiesTaxes(TranxPurInvoiceDutiesTaxes mDuties, TranxPurInvoice mPurchaseTranx,
        ...
        try {
            ...
            // mDuties.getDutiesTaxes().getAssociateGroups(),
            // mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(),
            // mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
            // mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), type, true,
            // tranxType.getTransactionCode(), operation);
    ...
    public void insertFromAdditionalCharges(TranxPurInvoiceAdditionalCharges mAdditinoalCharges,
            TranxPurInvoice mPurchaseTranx, TransactionTypeMaster tranxType, String type, String operation) {
        try {
            /**** New Postings Logic *****/
            ledgerCommonPostings.callToPostingsTranxCode(mAdditinoalCharges.getAmount(),
                    mAdditinoalCharges.getAdditionalCharges(), tranxType,
                    mAdditinoalCharges.getAdditionalCharges().getAssociateGroups(), mPurchaseTranx.getFiscalYear(),
                    mPurchaseTranx.getBranch(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(),
                    mPurchaseTranx.getId(), mPurchaseTranx.getVendorInvoiceNo(), type, true,
                    tranxType.getTransactionCode(), operation, mPurchaseTranx.getTranxCode());
            ...
            if (operation.equalsIgnoreCase("insert")) {
                ...
                postingUtility.callToPostingLedger(tranxType, type, mAdditinoalCharges.getAmount(),
                        mPurchaseTranx.getFiscalYear(), mAdditinoalCharges.getAdditionalCharges(),
                        mPurchaseTranx.getInvoiceDate(),
                        mPurchaseTranx.getId(), mPurchaseTranx.getOutlet(), mPurchaseTranx.getBranch(),
                        mPurchaseTranx.getTranxCode());
            }
    ...
    public TranxPurInvoice saveIntoPurchaseInvoice(MultipartHttpServletRequest request) {
        ...
        try {
            ...
            LedgerMaster purchaseAccount = ledgerMasterRepository.findByIdAndOutletIdAndStatus(
                    Long.parseLong(request.getParameter("purchase_id")), users.getOutlet().getId(), true);
            ...
            if (users.getBranch() == null)
                discountLedger = ledgerMasterRepository.findByLedgerNameIgnoreCaseAndOutletIdAndStatusAndBranchIsNull(
                        "purchase discount", users.getOutlet().getId(), true);
            else
                discountLedger = ledgerMasterRepository.findByLedgerNameIgnoreCaseAndOutletIdAndBranchIdAndStatus(
                        "purchase discount", users.getOutlet().getId(), users.getBranch().getId(), true);
            ...
            LedgerMaster sundryCreditors = ledgerMasterRepository.findByIdAndOutletIdAndStatus(
                    Long.parseLong(request.getParameter("supplier_code_id")), users.getOutlet().getId(), true);
            ...
            if (mPurchaseTranx != null) {
                ...
                if (paramMap.containsKey("debitNoteReference")) {
                    if (Boolean.parseBoolean(request.getParameter("debitNoteReference"))) {
                        ...
                        for (JsonElement mBill : debitNotes) {
                            ...
                            tranxDebitNoteNewReference = tranxDebitNoteNewReferenceRepository
                                    .findByIdAndStatus(mDebitNote.get("debitNoteId").getAsLong(), true);
                            ...
                            TranxDebitNoteNewReferenceMaster newReferenceMaster = tranxDebitNoteNewReferenceRepository
                                    .save(tranxDebitNoteNewReference);
                ...
                saveIntoPurchaseInvoiceDetails(array, mPurchaseTranx, branch, outlet, users.getId(), tranxType,
                        referenceObject);
            }
    ...
    private void saveIntoDayBook(TranxPurInvoice mPurchaseTranx) {
        ...
        if (mPurchaseTranx.getBranch() != null)
            dayBook.setBranch(mPurchaseTranx.getBranch());
                ...
                for (JsonElement mList : cgstList) {
                    ...
                    if (mPurchaseTranx.getBranch() != null)
                        dutiesTaxes = ledgerMasterRepository.findByOutletIdAndBranchIdAndLedgerNameIgnoreCase(
                                mPurchaseTranx.getOutlet().getId(), mPurchaseTranx.getBranch().getId(), ledgerName);
                    else
    ...
    public void saveIntoPurchaseAdditionalCharges(JsonArray additionalCharges, TranxPurInvoice mPurchaseTranx,
    ...
    public void saveIntoPurchaseInvoiceDetails(JsonArray array, TranxPurInvoice mPurchaseTranx, Branch branch,
        ...
        try {
            ...
            for (JsonElement mList : array) {
                ...
                TransactionStatus transactionStatus = transactionStatusRepository.findByStatusNameAndStatus("opened",
                        true);
                ...
                try {
                    if (object.get("is_batch").getAsBoolean()) {
                         ...
                         * case 1 : if supplier is changed ,create new batch even batch number is same
                         * or different
                         * against that supplier so that we can maintain valuation of that product
                         * supplier wise
                         * case 2 : if anything changes in purchase rate or costing ,create new batch
                         * even
                        ...
                        if (object.get("b_details_id").getAsLong() == 0) {
                            ...
                            if (object.has("b_purchase_rate")
                                    && !object.get("b_purchase_rate").getAsString().equals(""))
                    ...
                    ProductUnitPacking mUnitPackaging = productUnitRepository.findRate(mProduct.getId(), levelAId,
                            levelBId, levelCId, units.getId(), true);
                    if (mUnitPackaging != null) {
                        if (mUnitPackaging.getPurchaseRate() != null && mUnitPackaging.getPurchaseRate() != 0
                                && prrate != mUnitPackaging.getPurchaseRate()) {
                    ...
                    if (jsonArray != null && jsonArray.size() > 0) {
                        ...
                        for (JsonElement jsonElement : jsonArray) {
                            ...
                            productSerialNumber.setUnits(units);
                            TranxPurchaseInvoiceProductSrNumber mSerialNo = serialNumberRepository
                                    .save(productSerialNumber);
                            if (mProduct.getIsInventory() && !referenceType.equalsIgnoreCase("PRSCHN")) {
                                stkQty = unitConversion.convertToLowerUnit(mProduct.getId(), units.getId(),
                                        (tranxQty + free_qty));
                                inventoryCommonPostings.callToInventoryPostings("CR",
                                        mPurchaseTranx.getInvoiceDate(), mPurchaseTranx.getId(),
                                        stkQty, branch, outlet, mProduct, tranxType, levelA, levelB, levelC, units,
                                        productBatchNo, batchNo, mPurchaseTranx.getFiscalYear(), serialNo);
                            }
                        }
                    } else {
                        /**** Inventory Postings *****/
                ...
                // barcode =
                // barcodeRepository.findByProductIdAndOutletIdAndStatus(mProduct.getId(),
                // outlet.getId(), true);
                ...
                productBatchNo.getBarcode();
                System.out.println("CompanyBarco=>"+object.get("barcode").getAsString());
                barcode.setCompanyBarcode(object.get("barcode").getAsString());
                //   barcode.setCompanyBarcode(""+productBatchNo.getBarcode().doubleValue());
                ...
                if (levelA != null)
                    barcode.setLevelA(levelA);
                if (levelB != null)
                    barcode.setLevelB(levelB);
                if (levelC != null)
                    barcode.setLevelC(levelC);
                ...
                /*
                 * closing of purchase orders while converting into purchase invoice using its
                 * qnt
                 */
                ...
                if (referenceType.equalsIgnoreCase("PRSORD")) {
                    TranxPurOrderDetailsUnits orderDetails = tranxPurOrderDetailsUnitRepository
                 ...
                 */
                    ...
                    TranxPurChallanDetailsUnits challanDetails = tranxPurChallanDetailsUnitRepository
                            .findByProductDetailsLevel(referenceId, mProduct.getId(), units.getId(), levelAId, levelBId,
                                    levelCId, true);
    ...
    public JsonObject purchaseLastRecord(HttpServletRequest request) {
        ...
        Long count = 0L;
        if (users.getBranch() != null) {
            count = tranxPurInvoiceRepository.findBranchLastRecord(users.getOutlet().getId(),
                    users.getBranch().getId());
        } else {
    ...
    public JsonObject AllpurchaseList(HttpServletRequest request) {
        ...
        if (flag == true) {
            if (users.getBranch() != null) {
                purInvoice = tranxPurInvoiceRepository.findPurchaseInvoicesListWithDate(users.getOutlet().getId(),
                        users.getBranch().getId(), startDatep, endDatep, true);
            } else {
                purInvoice = tranxPurInvoiceRepository.findPurchaseInvoicesListWithDateNoBr(users.getOutlet().getId(),
                        startDatep, endDatep, true);
            }
        } else {
            if (users.getBranch() != null) {
                purInvoice = tranxPurInvoiceRepository.findByOutletIdAndBranchIdAndStatusOrderByIdDesc(
        ...
        for (TranxPurInvoice invoices : purInvoice) {
            ...
            response.addProperty("invoice_date",
                    DateConvertUtil.convertDateToLocalDate(invoices.getInvoiceDate()).toString());
    ...
    public Object purchaseList(@RequestBody Map<String, String> request, HttpServletRequest req) {
        ...
        try {
            String query = "SELECT tranx_purchase_invoice_tbl.id FROM `tranx_purchase_invoice_tbl` LEFT JOIN ledger_master_tbl ON "
                    + "tranx_purchase_invoice_tbl.sundry_creditors_id=ledger_master_tbl.id WHERE "
                    + "tranx_purchase_invoice_tbl.outlet_id=" + users.getOutlet().getId()
                    + " AND tranx_purchase_invoice_tbl.status=1";
            if (users.getBranch() != null) {
            ...
            if (!startDate.equalsIgnoreCase("") && !endDate.equalsIgnoreCase(""))
                query += "  AND DATE(tranx_purchase_invoice_tbl.invoice_date) BETWEEN '" + startDate + "' AND '"
                        + endDate + "'";
            ...
            if (!searchText.equalsIgnoreCase("")) {
                query = query + " AND (vendor_invoice_no LIKE '%" + searchText + "%' OR DATE(invoice_date)="
                        + searchText + " OR total_amount=" + searchText + " OR taxable_amount=" + searchText + ")";
            }
             ...
             */
            /**** *****/
            String query1 = "SELECT COUNT(tranx_purchase_invoice_tbl.id) as totalcount FROM tranx_purchase_invoice_tbl "
                    + "WHERE tranx_purchase_invoice_tbl.status=? AND tranx_purchase_invoice_tbl.outlet_id=?";
            if (users.getBranch() != null) {
            ...
            for (Object mList : purchaseInvoice) {
                TranxPurInvoice invoiceListView = tranxPurInvoiceRepository
             ...
             */
            GenericDatatable<PurInvoiceDTO> data = new GenericDatatable<>(purInvoiceDTOList, totalProducts, pageNo,
        ...
        if (purInvoice.getReference() != null && !purInvoice.getReference().isEmpty()) {
            if (purInvoice.getReference().equalsIgnoreCase("PRSORD") && purInvoice.getPoId() != null) {
                ...
                for (int i = 0; i < idList.length; i++) {
                    TranxPurOrder tranxPurOrder = tranxPurOrderRepository.findByIdAndStatus(Long.parseLong(idList[i]),
                            true);
                    if (tranxPurOrder != null) {
                ...
                idList = purInvoice.getPoId().split(",");
                for (int i = 0; i < idList.length; i++) {
                    TranxPurChallan tranxPurChallan = tranxPurChallanRepository
    ...
    public JsonObject getPurchaseInvoiceById(HttpServletRequest request) {
        ...
        try {
            ...
            TranxPurInvoice purchaseInvoice = tranxPurInvoiceRepository.findByIdAndOutletIdAndStatus(id,
                    users.getOutlet().getId(), true);
            list = invoiceDetailsRepository.findByPurchaseTransactionIdAndStatus(id, true);
            /*
             * serialNumbers =
             * serialNumberRepository.findByPurchaseTransactionIdAndStatus(purchaseInvoice.
             * getId(), true);
             * additionalCharges =
             * purInvoiceAdditionalChargesRepository.findByPurchaseTransactionIdAndStatus(
             * purchaseInvoice.getId(), true);
             */
            finalResult.addProperty("tcs", purchaseInvoice.getTcs());
            finalResult.addProperty("narration",
                    purchaseInvoice.getNarration() != null ? purchaseInvoice.getNarration() : "");
            finalResult.addProperty("discountLedgerId",
                    purchaseInvoice.getPurchaseDiscountLedger() != null
                            ? purchaseInvoice.getPurchaseDiscountLedger().getId()
                            : 0);
            ...
            result.addProperty("invoice_dt",
                    DateConvertUtil.convertDateToLocalDate(purchaseInvoice.getInvoiceDate()).toString());
            ...
            if (list.size() > 0) {
                for (TranxPurInvoiceDetails mDetails : list) {
                    ...
                    List<TranxPurInvoiceDetailsUnits> unitDetails = tranxPurInvoiceUnitsRepository
                            .findByPurInvoiceDetailsIdAndStatus(mDetails.getId(), true);
    ...
    public JsonObject getPurchaseInvoiceByIdNew(HttpServletRequest request) {
        ...
        try {
            ...
            if (purchaseInvoice != null) {
                ...
                finalResult.addProperty("narration",
                        purchaseInvoice.getNarration() != null ? purchaseInvoice.getNarration() : "");
                finalResult.addProperty("paymentMode",
                        purchaseInvoice.getPaymentMode() != null ? purchaseInvoice.getPaymentMode() : "");
                finalResult.addProperty("discountLedgerId",
                        purchaseInvoice.getPurchaseDiscountLedger() != null
                                ? purchaseInvoice.getPurchaseDiscountLedger().getId()
                                : 0);
                ...
                finalResult.addProperty("additionLedger1",
                        purchaseInvoice.getAdditionLedger1() != null ? purchaseInvoice.getAdditionLedger1().getId()
                                : 0);
                finalResult.addProperty("additionLedgerAmt1",
                        purchaseInvoice.getAdditionLedgerAmt1() != null ? purchaseInvoice.getAdditionLedgerAmt1() : 0);
                finalResult.addProperty("additionLedger2",
                        purchaseInvoice.getAdditionLedger2() != null ? purchaseInvoice.getAdditionLedger2().getId()
                                : 0);
                finalResult.addProperty("additionLedgerAmt2",
                        purchaseInvoice.getAdditionLedgerAmt2() != null ? purchaseInvoice.getAdditionLedgerAmt2() : 0);
                finalResult.addProperty("additionLedger3",
                        purchaseInvoice.getAdditionLedger3() != null ? purchaseInvoice.getAdditionLedger3().getId()
                                : 0);
                finalResult.addProperty("additionLedgerAmt3",
                        purchaseInvoice.getAdditionLedgerAmt3() != null ? purchaseInvoice.getAdditionLedgerAmt3() : 0);
                finalResult.addProperty("debitNoteReference",
                        purchaseInvoice.getIsDebitNoteRef() != null ? purchaseInvoice.getIsDebitNoteRef() : false);
                /* Purchase Invoice Data */
                result.addProperty("id", purchaseInvoice.getId());
                result.addProperty("invoice_dt",
                        DateConvertUtil.convertDateToLocalDate(purchaseInvoice.getInvoiceDate()).toString());
                ...
                result.addProperty("transaction_dt", purchaseInvoice.getTransactionDate().toString());
                result.addProperty("additional_charges_total", purchaseInvoice.getAdditionalChargesTotal());
                result.addProperty("gstNo",
                        purchaseInvoice.getGstNumber() != null ? purchaseInvoice.getGstNumber() : "");
                result.addProperty("isRoundOffCheck", purchaseInvoice.getIsRoundOff());
                result.addProperty("roundoff", purchaseInvoice.getRoundOff());
                result.addProperty("mode",
                        purchaseInvoice.getPaymentMode() != "" ? purchaseInvoice.getPaymentMode() : "");
                result.addProperty("image",
                        purchaseInvoice.getImagePath() != null ? serverUrl + purchaseInvoice.getImagePath() : "");
                /* End of Purchase Invoice Data */
            }
            ...
            List<TranxPurInvoiceDetailsUnits> unitsArray = tranxPurInvoiceUnitsRepository
                    .findByPurchaseTransactionIdAndStatus(purchaseInvoice.getId(), true);
            for (TranxPurInvoiceDetailsUnits mUnits : unitsArray) {
                ...
                selectedProduct.addProperty("packing",
                ...
                unitsJsonObjects.addProperty("packing",
                        mUnits.getProduct() != null
                                ? (mUnits.getProduct().getPackingMaster() != null
                                        ? mUnits.getProduct().getPackingMaster().getPackName()
                                        : "")
                                : "");
                ...
                unitsJsonObjects.addProperty("transaction_status",
                        mUnits.getTransactionStatus() != null ? mUnits.getTransactionStatus().getId().toString() : "");
                unitsJsonObjects.addProperty("inventoryId", 0);
                InventoryDetailsPostings inventoryDetailsPostings = inventoryDetailsPostingsRepository
                        .findByPurInventoryRow(mUnits.getProduct().getId(), purchaseInvoice.getFiscalYear().getId(),
                                purchaseInvoice.getOutlet().getId(),
                ...
                if (mUnits.getProductBatchNo() != null) {
                    ...
                    unitsJsonObjects.addProperty("min_discount",
                            mUnits.getProductBatchNo().getMinDiscount() != null
                                    ? mUnits.getProductBatchNo().getMinDiscount().toString()
                                    : "");
                    unitsJsonObjects.addProperty("max_discount",
                            mUnits.getProductBatchNo().getMaxDiscount() != null
                                    ? mUnits.getProductBatchNo().getMaxDiscount().toString()
                                    : "");
            ...
            selectedBillList = tranxDebitNoteNewReferenceRepository.findSelectedBills(sundryCreditorId, true, "credit",
            ...
            /**** Barcode Data *****/
            List<ProductBarcode> barcodeList = barcodeRepository.findByTransactionIdAndStatus(purchaseInvoice.getId(),
                    true);
            ...
            for (ProductBarcode mBarcode : barcodeList) {
                ...
                barcodeJsonObject.addProperty("batch_id",
                        mBarcode.getProductBatch() != null ? mBarcode.getProductBatch().getId().toString() : "");
                barcodeJsonObject.addProperty("batch_no",
                        mBarcode.getProductBatch() != null ? mBarcode.getProductBatch().getBatchNo() : "");
                ...
                barcodeJsonObject.addProperty("packing_id",
                        mBarcode.getPackingMaster() != null ? mBarcode.getPackingMaster().getId().toString() : "");
                barcodeJsonObject.addProperty("packing_name",
                        mBarcode.getProduct().getPackingMaster() != null
                                ? mBarcode.getProduct().getPackingMaster().getPackName()
                                : "");
                barcodeJsonObject.addProperty("units_id",
                        mBarcode.getUnits() != null ? mBarcode.getUnits().getId().toString() : "");
                barcodeJsonObject.addProperty("units_name",
                        mBarcode.getUnits() != null ? mBarcode.getUnits().getUnitName() : "");

                barcodeJsonObject.addProperty("levela_id",
                        mBarcode.getLevelA() != null ? mBarcode.getLevelA().getId().toString() : "");
                barcodeJsonObject.addProperty("levela_name",
                        mBarcode.getLevelA() != null ? mBarcode.getLevelA().getLevelName() : "");

                barcodeJsonObject.addProperty("levelb_id",
                        mBarcode.getLevelB() != null ? mBarcode.getLevelB().getId().toString() : "");

                barcodeJsonObject.addProperty("levelb_name",
                        mBarcode.getLevelB() != null ? mBarcode.getLevelB().getLevelName() : "");

                barcodeJsonObject.addProperty("levelc_id",
                        mBarcode.getLevelC() != null ? mBarcode.getLevelC().getId().toString() : "");

                barcodeJsonObject.addProperty("levelc_name",
                        mBarcode.getLevelC() != null ? mBarcode.getLevelC().getLevelName() : "");
            ...
            /* Purchase Additional Charges */
            JsonArray jsonAdditionalList = new JsonArray();
            additionalCharges = purInvoiceAdditionalChargesRepository
                    .findByPurchaseTransactionIdAndStatus(purchaseInvoice.getId(), true);
            if (additionalCharges.size() > 0) {
                for (TranxPurInvoiceAdditionalCharges mAdditionalCharges : additionalCharges) {
                    JsonObject json_charges = new JsonObject();
    ...
    private ResponseMessage saveIntoPurchaseInvoiceEdit(MultipartHttpServletRequest request, String key) {
        ...
        if (invoiceTranx != null) {
            ...
            /* End of fiscal year mapping */
            invoiceTranx.setVendorInvoiceNo(request.getParameter("invoice_no"));
            purchaseAccount = ledgerMasterRepository.findByIdAndOutletIdAndStatus(
                    Long.parseLong(request.getParameter("purchase_id")), users.getOutlet().getId(), true);
            /*
            ...
            if (purchaseAccount.getId() != null && invoiceTranx.getId() != null && tranxType.getId() != null) {
                ...
                if (isContains) {
                    ...
                    LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository
                            .findByLedgerMasterIdAndTransactionTypeIdAndTransactionId(purchaseAccount.getId(),
                    ...
                    // invoiceTranx.getId(), invoiceTranx.getVendorInvoiceNo(), "DR", true,
                ...
                postingUtility.callToPostingLedgerForUpdate(isLedgerContains, amount,
                        invoiceTranx.getPurchaseAccountLedger().getId(),
                        tranxType, "DR", invoiceTranx.getId(), purchaseAccount, strDt, fiscalYear,
                        invoiceTranx.getOutlet(),
            ...
            if (Double.parseDouble(request.getParameter("purchase_discount_amt")) > 0) {
                if (users.getBranch() == null)
                    discountLedger = ledgerMasterRepository
                            .findByLedgerNameIgnoreCaseAndOutletIdAndStatusAndBranchIsNull("purchase discount",
                                    users.getOutlet().getId(), true);
                else
                    discountLedger = ledgerMasterRepository.findByLedgerNameIgnoreCaseAndOutletIdAndBranchIdAndStatus(
                            "purchase discount", users.getOutlet().getId(), users.getBranch().getId(), true);
                ...
                if (discountLedger != null) {
                     ...
                     * invoiceTranx.getId(), tranxType.getId(), "CR",
                     * Double.parseDouble(request.getParameter("total_purchase_discount_amt")));
                     *//**** New Postings Logic *****/
                    /*
                     * LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository.
                     ...
                     *//**** New Postings Logic *****//*
                                                       ...
                                                       * invoiceTranx.getBranch(), invoiceTranx.getOutlet(), date,
                                                       * invoiceTranx.getId(),
                                                       * invoiceTranx.getVendorInvoiceNo(), "CR", true,
                                                       * tranxType.getTransactionCode(), "Insert");
                                                       * }
                                                       * }
                                                       */
            ...
            sundryCreditors = ledgerMasterRepository.findByIdAndOutletIdAndStatus(
                    Long.parseLong(request.getParameter("supplier_code_id")), users.getOutlet().getId(), true);
            /*
             * calling store procedure for updating opening and closing balance of Sundry
             * Creditors
             */
            if (sundryCreditors.getId() != null && invoiceTranx.getId() != null && tranxType.getId() != null) {
                ...
                LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository
                        .findByLedgerMasterIdAndTransactionTypeIdAndTransactionId(sundryCreditors.getId(),
                                tranxType.getId(), invoiceTranx.getId());
                if (isContains) {
                    // transactionDetailsRepository.ledgerPostingEdit(sundryCreditors.getId(),
                    // invoiceTranx.getId(), tranxType.getId(), "CR",
                    // Double.parseDouble(request.getParameter("totalamt")));
                    /**** New Postings Logic *****/
                    /*
                     * LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository.
                     * findByLedgerMasterIdAndTransactionTypeIdAndTransactionId(sundryCreditors.
                    ...
                    /**** New Postings Logic *****/
                    ledgerCommonPostings.callToPostings(Double.parseDouble(request.getParameter("bill_amount")),
                            sundryCreditors, tranxType, sundryCreditors.getAssociateGroups(), fiscalYear,
                            invoiceTranx.getBranch(), invoiceTranx.getOutlet(), strDt, invoiceTranx.getId(),
                            invoiceTranx.getVendorInvoiceNo(), "CR", true, tranxType.getTransactionCode(), "Insert");
                    if (mLedger != null) {
                        mLedger.setAmount(
                                mLedger.getAmount() - Double.parseDouble(request.getParameter("bill_amount")));
                        mLedger.setTransactionDate(strDt);
                ...
                /**** NEW METHOD FOR LEDGER POSTING ****/
                postingUtility.callToPostingLedgerForUpdate(isLedgerContains, amount,
                        invoiceTranx.getSundryCreditors().getId(),
                        tranxType, "CR", invoiceTranx.getId(), sundryCreditors, strDt, fiscalYear,
                        invoiceTranx.getOutlet(),
                        invoiceTranx.getBranch(), invoiceTranx.getTranxCode());
            } /*
            ...
            invoiceTranx.setGrossAmount(Double.parseDouble(request.getParameter("total_base_amt")));
            // roundoff =
            // ledgerMasterRepository.findByLedgerNameIgnoreCaseAndOutletId("Round off",
            // users.getOutlet().getId());
            if (users.getBranch() != null)
                roundoff = ledgerMasterRepository.findByOutletIdAndBranchIdAndLedgerNameIgnoreCase(
                        users.getOutlet().getId(), users.getBranch().getId(), "Round off");
            else
                roundoff = ledgerMasterRepository.findRoundOff(users.getOutlet().getId(), "Round off");
            invoiceTranx.setRoundOff(Double.parseDouble(request.getParameter("roundoff")));
            invoiceTranx.setPurchaseRoundOff(roundoff);
            ...
            if (isContains) {
                ...
                // transactionDetailsRepository.ledgerPostingEdit(roundoff.getId(),
                // invoiceTranx.getId(), tranxType.getId(), crdr, rf * -1);
                /**** New Postings Logic *****/
                LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository
                ...
                if (rf >= 0) {
                    /**** New Postings Logic *****/
                    ledgerCommonPostings.callToPostings(Math.abs(rf), roundoff, tranxType,
                            roundoff.getAssociateGroups(), fiscalYear, invoiceTranx.getBranch(),
                            invoiceTranx.getOutlet(), strDt, invoiceTranx.getId(), invoiceTranx.getVendorInvoiceNo(),
            ...
            /**** NEW METHOD FOR LEDGER POSTING ****/
            postingUtility.callToPostingLedgerForUpdate(isLedgerContains, amount,
                    invoiceTranx.getPurchaseRoundOff().getId(),
             ...
             * Double addCharges =
             * Double.parseDouble(request.getParameter("addChgLedgerAmt1"));
                ...
                if (additionalChgLedger3 != null) {
                    ...
                    if (isContains) {
                        ...
                        ledgerCommonPostings.callToPostings(
                                Double.parseDouble(request.getParameter("addChgLedgerAmt3")), additionalChgLedger3,
                                tranxType, additionalChgLedger3.getAssociateGroups(), fiscalYear,
                                invoiceTranx.getBranch(), invoiceTranx.getOutlet(), strDt, invoiceTranx.getId(),
                                invoiceTranx.getVendorInvoiceNo(), addCharges > 0 ? "DR" : "CR", true,
                                tranxType.getTransactionCode(), "Insert");
                    }
                }
            }
            ...
            try {
                ...
                if (mPurchaseTranx != null) {
                    ...
                    if (paramMap.containsKey("debitNoteReference")) {
                        if (Boolean.parseBoolean(request.getParameter("debitNoteReference"))) {
                            ...
                            for (JsonElement mBill : debitNotes) {
                                ...
                                TransactionStatus transactionStatus = transactionStatusRepository
                                        .findByStatusNameAndStatus("closed", true);
                                tranxDebitNoteNewReference
                                        .setTotalAmount(mDebitNote.get("debitNotePaidAmt").getAsDouble());
                                if (mDebitNote.get("debitNotePaidAmt").getAsDouble() > 0) {
                                    tranxDebitNoteNewReference.setAdjustedId(mPurchaseTranx.getId());
                                    tranxDebitNoteNewReference.setIsSelected(true);
                                }
                                ...
                                tranxDebitNoteNewReference
                                        .setBalance(mDebitNote.get("debitNoteRemaningAmt").getAsDouble());
                                TranxDebitNoteNewReferenceMaster newReferenceMaster = tranxDebitNoteNewReferenceRepository
                                        .save(tranxDebitNoteNewReference);
                    ...
                    saveIntoPurchaseInvoiceDetailsEdit(array, mPurchaseTranx,
                            users.getBranch() != null ? users.getBranch() : null, users.getOutlet(), users.getId(),
                            rowsDeleted, tranxType, purDate);
                    /**** end of PurchaseInvoice DetailsEdit ****/

                    /**** for Purchase Duties and Texes Edit ****/
                    String taxStr = request.getParameter("taxCalculation");
                    JsonObject duties_taxes = new Gson().fromJson(taxStr, JsonObject.class);
                    saveIntoPurchaseDutiesTaxesEdit(duties_taxes, mPurchaseTranx, taxFlag, tranxType,
                    ...
                    if (paramMap.containsKey("additionalCharges")) {
                        ...
                        if (additionalCharges.size() > 0) {
                            saveIntoPurchaseAdditionalChargesEdit(additionalCharges, mPurchaseTranx, tranxType,
                    ...
                    for (Long mDblist : dbList) {
                        if (!mInputList.contains(mDblist)) {
                            // transactionDetailsRepository.ledgerPostingRemove(mDblist,
                            // invoiceTranx.getId(), tranxType.getId());
                            /**** New Postings Logic *****/
                            LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository
                                    .findByLedgerMasterIdAndTransactionTypeIdAndTransactionId(mDblist,
                                            tranxType.getId(), invoiceTranx.getId());
                    ...
                    for (Long mDblist : ledgerList) {
                        if (!ledgerInputList.contains(mDblist)) {
                            ...
                            if (ledgerDetail != null) {
                                ...
                                ledgerDetail.setClosingAmount(closing);
                                ledgerDetail.setStatus(false);
                                LedgerOpeningClosingDetail detail = ledgerOpeningClosingDetailRepository
                                        .save(ledgerDetail);

                                /***** NEW METHOD FOR LEDGER POSTING *****/
                                postingUtility.updateLedgerPostings(ledgerDetail.getLedgerMaster(),
                                        mPurchaseTranx.getInvoiceDate(), tranxType, mPurchaseTranx.getFiscalYear(),
                                        detail);
                            }
    ...
    private void updatePaymentInvoice(TranxPurInvoice mPurchaseTranx, Users users, Double totalAmount, double v,
        ...
        TransactionTypeMaster tranxType = tranxRepository.findByTransactionCodeIgnoreCase("PMT");
        TranxPaymentMaster tranxPaymentMaster = tranxPaymentMasterRepository
                .findByTranxIdAndStatus(mPurchaseTranx.getId(), true);
        Long count = 0L;
        if (mPurchaseTranx.getBranch() != null)
            tranxPaymentMaster.setBranch(mPurchaseTranx.getBranch());
        if (mPurchaseTranx.getFiscalYear() != null)
            tranxPaymentMaster.setFiscalYear(mPurchaseTranx.getFiscalYear());
        ...
        if (totalAmount > 0.0) {
            ...
            if (mPurchaseTranx.getBranch() != null) {
                ledgerMaster = ledgerMasterRepository.findByUniqueCodeAndOutletIdAndBranchIdAndStatus("CAIH",
                        mPurchaseTranx.getOutlet().getId(), mPurchaseTranx.getBranch().getId(), true);
            } else {
                ledgerMaster = ledgerMasterRepository.findByUniqueCodeAndOutletIdAndStatusAndBranchIsNull("CAIH",
                        mPurchaseTranx.getOutlet().getId(), true);
            }
            updateIntoPaymentPerticualrs(newTranxReceiptMaster, ledgerMaster, "others", totalAmount, key, cash,
                    tranxType, mPurchaseTranx);
        }
    ...
    private void updateIntoPaymentPerticualrs(TranxPaymentMaster newTranxReceiptMaster,
         ...
         * findByTranxPaymentMasterIdAndOutletIdAndStatusAndTypeIgnoreCase(
         * newTranxReceiptMaster.getId(),
         * newTranxReceiptMaster.getOutlet().getId(),true, type);
         */
        TranxPaymentPerticulars mTranxPerticular = tranxPaymentPerticularRepository
                .findByTranxPaymentMasterIdAndStatusAndLedgerTypeIgnoreCaseAndLedgerMasterId(
        ...
        if (mTranxPerticular != null) {
            ...
            if (mLedger != null) {
                ...
                ledgerCommonPostings.callToPostings(totalAmount, ledgerMaster, tranxType,
                        ...
                        newTranxReceiptMaster.getPaymentNo(), ledgerType, true, tranxType.getTransactionCode(),
                        "Insert");
            }
            ...
            postingUtility.callToPostingLedgerForUpdateByDetailsId(totalAmount, ledgerMaster.getId(), tranxType,
                    ...
                    newTranxReceiptMaster.getBranch(),
        ...
        for (JsonElement mList : array) {
            ...
            try {
                if (object.get("is_batch").getAsBoolean()) {
                    ...
                    double costing_with_tax = 0;
                    /*****
                     * if org_b_details_id !=0 then reverese the inventory postings against the
                     * org_b_details_id
                     *****/
                    if (object.has("org_b_details_id")) {
                        if (object.get("org_b_details_id").getAsLong() != 0) {
                            /***** new architecture of Inventory Postings *****/
                            inventoryCommonPostings.callToInventoryPostings("DR", mPurchaseTranx.getInvoiceDate(),
                                    mPurchaseTranx.getId(), invoiceUnits.getQty(), mPurchaseTranx.getBranch(),
                                    mPurchaseTranx.getOutlet(), invoiceUnits.getProduct(), tranxType, levelA, levelB,
                                    levelC, invoiceUnits.getUnits(), invoiceUnits.getProductBatchNo(),
                                    invoiceUnits.getProductBatchNo() != null
                                            ? invoiceUnits.getProductBatchNo().getBatchNo()
                                            : null,
                                    mPurchaseTranx.getFiscalYear(), null);
                        }
                    }
                    ...
                    if (object.has("manufacturing_date") &&
                            ...
                            !object.get("manufacturing_date").getAsString().toLowerCase().contains("invalid"))
                        productBatchNo
                                .setManufacturingDate(LocalDate.parse(object.get("manufacturing_date").getAsString()));
                    if (object.has("costing") && !object.get("costing").isJsonNull())
                        productBatchNo.setCosting(object.get("costing").getAsDouble());
                    if (object.has("costing_with_tax") && !object.get("costing_with_tax").isJsonNull())
                        productBatchNo.setCostingWithTax(object.get("costing_with_tax").getAsDouble());
                ...
                Double costing = object.get("costing").getAsDouble();
                Double costingWithTax = object.get("costing_with_tax").getAsDouble();
                ProductUnitPacking mUnitPackaging = productUnitRepository.findRate(mProduct.getId(), levelAId, levelBId,
                        levelCId, units.getId(), true);
                if (mUnitPackaging != null) {
                    if (mUnitPackaging.getPurchaseRate() != null && mUnitPackaging.getPurchaseRate() != 0
                            && prrate != mUnitPackaging.getPurchaseRate()) {
                        mUnitPackaging.setPurchaseRate(prrate);
                    ...
                    for (JsonElement jsonElement : jsonArray) {
                        ...
                        if (detailsId == 0) {
                            ...
                            if (mProduct.getIsInventory()) {
                                inventoryCommonPostings.callToInventoryPostings("CR", mPurchaseTranx.getInvoiceDate(),
                                        mPurchaseTranx.getId(), tranxQty + free_qty, branch, outlet, mProduct,
                                        tranxType, levelA, levelB, levelC, units, productBatchNo, batchNo,
                                        mPurchaseTranx.getFiscalYear(), serialNo);
                            }
                        } else {
                            TranxPurchaseInvoiceProductSrNumber productSerialNumber1 = serialNumberRepository
                            ...
                            inventoryCommonPostings.callToEditInventoryPostings(mPurchaseTranx.getInvoiceDate(),
                                    mPurchaseTranx.getId(), tranxQty + free_qty, branch, outlet, mProduct, tranxType,
                        ...
                        if (details_id != 0) {
                            ...
                            if (productRow != null) {
                                if (mPurchaseTranx.getInvoiceDate().compareTo(productRow.getTranxDate()) == 0 &&
                                        tranxQty.doubleValue() != object.get("qty").getAsDouble()) { // DATE SAME AND
                                                                                                     // QTY DIFFERENT
                                    Double closingStk = closingUtility.CAL_CR_STOCK(productRow.getOpeningStock(),
                                            object.get("qty").getAsDouble(), free_qty);
                                    ...
                                    InventorySummaryTransactionDetails mInventory = stkTranxDetailsRepository
                                            .save(productRow);
                                    closingUtility.updatePosting(mInventory, mProduct.getId(),
                                            mPurchaseTranx.getInvoiceDate());
                                } else if (mPurchaseTranx.getInvoiceDate().compareTo(productRow.getTranxDate()) != 0) { // DATE
                                                                                                                        // IS
                                                                                                                        // DIFFERENT
                                    ...
                                    if (newDate.after(oldDate)) { // FORWARD INSERT
                                        ...
                                        for (InventorySummaryTransactionDetails closingDetail : openingClosingList) {
                                            ...
                                            if (closingDetail.getTranxAction().equalsIgnoreCase("IN"))
                                                closing = closingUtility.CAL_CR_STOCK(opening, closingDetail.getQty(),
                                                        0.0);
                                            else if (closingDetail.getTranxAction().equalsIgnoreCase("OUT"))
                                                closing = closingUtility.CAL_DR_STOCK(opening, 0.0,
                                                        closingDetail.getQty());
                                    ...
                                    closingUtility.stockPosting(outlet, branch, mPurchaseTranx.getFiscalYear().getId(),
                                            ...
                                            mPurchaseTranx.getId(), units.getId(), levelAId, levelBId, levelCId,
                                            productBatchNo,
                                            mPurchaseTranx.getTranxCode(), userId, "IN",
                                            mProduct.getPackingMaster().getId());
            ...
            if (productBatchNo != null) {
                barcode = barcodeRepository
                        .findByProductIdAndOutletIdAndStatusAndTransactionIdAndTransactionTypeMasterIdAndProductBatchId(
                                mProduct.getId(), outlet.getId(), true, mPurchaseTranx.getId(), tranxType.getId(),
                                productBatchNo.getId());
            }
            if (barcode == null) {
            ...
            try {
                ...
                if (mProduct.getIsInventory() && flag) {
                    ...
                    if (details_id != 0) {
                        inventoryCommonPostings.callToEditInventoryPostings(mPurchaseTranx.getInvoiceDate(),
                                mPurchaseTranx.getId(), object.get("qty").getAsDouble() + free_qty, branch, outlet,
                ...
                for (JsonElement element : deletedArrays) {
                    ...
                    if (deletedRowsId.size() > 0) {
                        if (deletedRowsId.has("del_id")) {
                            mDeletedInvoices = tranxPurInvoiceUnitsRepository
                            ...
                            if (mDeletedInvoices != null) {
                                ...
                                try {
                                    ...
                                    closingUtility.stockPosting(outlet, branch, mPurchaseTranx.getFiscalYear().getId(),
                                            ...
                                            mDeletedInvoices.getLevelA() != null ? mDeletedInvoices.getLevelA().getId()
                                                    : null,
                                            mDeletedInvoices.getLevelB() != null ? mDeletedInvoices.getLevelB().getId()
                                                    : null,
                                            mDeletedInvoices.getLevelC() != null ? mDeletedInvoices.getLevelC().getId()
                                                    : null,
                                            mDeletedInvoices.getProductBatchNo() != null
                                                    ? mDeletedInvoices.getProductBatchNo()
                                                    : null,
                                    ...
                                    String exceptionAsString = sw.toString();
                                    purInvoiceLogger.error(
                                            "Exception in saveInto PurchaseInvoiceDetails Edit:" + exceptionAsString);
                                } catch (Exception ex) {
                                    StringWriter sw = new StringWriter();
    ...
    public void saveIntoPurchaseDutiesTaxesEdit(JsonObject duties_taxes, TranxPurInvoice invoiceTranx, Boolean taxFlag,
        ...
        if (travelArray.size() > 0) {
            if (db_dutiesLedgerIds.size() > 0) {
                purchaseDutiesTaxes = purInvoiceDutiesTaxesRepository.findByPurchaseTransactionAndStatus(invoiceTranx,
                        true);
            }
            if (taxFlag) {
                JsonArray cgstList = duties_taxes.getAsJsonArray("cgst");
                        ...
                        if (dutiesTaxes != null) {
                            ...
                            if (isContains) {
                                ...
                                ledgerCommonPostings.callToPostings(amt, dutiesTaxes, tranxType,
                                        ...
                                        invoiceTranx.getBranch(), invoiceTranx.getOutlet(),
                                        invoiceTranx.getInvoiceDate(), invoiceTranx.getId(),
                                        invoiceTranx.getVendorInvoiceNo(), "DR", true, "Purchase Invoice", "Insert");
                            }
                        ...
                        for (JsonElement mIgst : igstList) {
                            ...
                            if (dutiesTaxes != null) {
                                ...
                                if (isContains) {
                                    ...
                                    LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository
                                            .findByLedgerMasterIdAndTransactionTypeIdAndTransactionId(
                                                    dutiesTaxes.getId(), tranxType.getId(), invoiceTranx.getId());
                                    if (mLedger != null) {
                                        mLedger.setAmount(amt);
                                        mLedger.setTransactionDate(invoiceTranx.getInvoiceDate());
                                        mLedger.setOperations("Updated");
                                    ...
                                    ledgerCommonPostings.callToPostings(amt, dutiesTaxes, tranxType,
                                            ...
                                            invoiceTranx.getBranch(), invoiceTranx.getOutlet(),
                                            invoiceTranx.getInvoiceDate(), invoiceTranx.getId(),
                                            invoiceTranx.getVendorInvoiceNo(), "DR", true, "Purchase Invoice",
                                            "Insert");
                                }
                                ...
                                postingUtility.callToPostingLedgerForUpdate(isLedgerContains, amt, dutiesTaxes.getId(),
                                        tranxType,
                                        "DR", invoiceTranx.getId(), dutiesTaxes, invoiceTranx.getInvoiceDate(),
                                        invoiceTranx.getFiscalYear(), invoiceTranx.getOutlet(),
                                        invoiceTranx.getBranch(),
                                        invoiceTranx.getTranxCode());
                ...
                if (cgstList.size() > 0) {
                    for (JsonElement mCgst : cgstList) {
                        ...
                        if (dutiesTaxes != null) {
                            ...
                            ledgerCommonPostings.callToPostings(amt, dutiesTaxes, tranxType,
                                    dutiesTaxes.getAssociateGroups(), invoiceTranx.getFiscalYear(),
                                    invoiceTranx.getBranch(), invoiceTranx.getOutlet(), invoiceTranx.getInvoiceDate(),
                                    invoiceTranx.getId(), invoiceTranx.getVendorInvoiceNo(), "DR", true,
                                    "Purchase Invoice", "Insert");

                            /***** NEW METHOD FOR LEDGER POSTING *****/
                            postingUtility.callToPostingLedger(tranxType, "DR", amt, invoiceTranx.getFiscalYear(),
                                    dutiesTaxes, invoiceTranx.getInvoiceDate(), invoiceTranx.getId(),
                                    invoiceTranx.getOutlet(), invoiceTranx.getBranch(),
                                    invoiceTranx.getTranxCode());
                        }
                    }
                }
    ...
    /*
     * private List<Long> getInputLedgerIds(Boolean taxFlag, JsonObject
     * duties_taxes, Long outletId) {
     ...
     *//* this is for Cgst creation *//*
                                       ...
                                       *//* this is for Sgst creation *//*
                                                                         * if (sgstList.size() > 0) {
                                                                         * for (JsonElement mSgst : sgstList) {
                                                                         * TranxPurInvoiceDutiesTaxes taxes = new
                                                                         * TranxPurInvoiceDutiesTaxes();
                                                                         * JsonObject sgstObject =
                                                                         * mSgst.getAsJsonObject();
                                                                         * LedgerMaster dutiesTaxes = null;
                                                                         * int inputGst = (int)
                                                                         * sgstObject.get("gst").getAsDouble();
                                                                         * String ledgerName = "INPUT SGST " + inputGst;
                                                                         * dutiesTaxes = ledgerMasterRepository.
                                                                         * findByLedgerNameIgnoreCaseAndOutletId(
                                                                         * ledgerName, outletId);
                                                                         * if (dutiesTaxes != null) {
                                                                         * returnLedgerIds.add(dutiesTaxes.getId());
                                                                         * }
                                                                         ...
                                                                         * } else {
                                                                         * JsonArray igstList =
                                                                         * duties_taxes.getAsJsonArray("igst");
                                                                         *//* this is for Igst creation *//*
                                                                                                           * if
                                                                                                           * (igstList.
                                                                                                           * size() > 0)
    ...
    public void saveIntoPurchaseAdditionalChargesEdit(JsonArray additionalCharges, TranxPurInvoice mPurchaseTranx,
            TransactionTypeMaster tranxTypeMater, Long outletId, String acRowsDeleted) {
        ...
        for (JsonElement mAddCharges : additionalCharges) {
            ...
            if (object.has("amt") && !object.get("amt").getAsString().equalsIgnoreCase("")) {
                ...
                charges = purInvoiceAdditionalChargesRepository
                        .findByAdditionalChargesIdAndPurchaseTransactionIdAndStatus(ledgerId, mPurchaseTranx.getId(),
                                true);
                if (detailsId == 0L) {
                ...
                if (isContains) {
                    ...
                    LedgerTransactionPostings mLedger = ledgerTransactionPostingsRepository
                            .findByLedgerMasterIdAndTransactionTypeIdAndTransactionId(addcharges.getId(),
                                    tranxTypeMater.getId(), mPurchaseTranx.getId());
                    if (mLedger != null) {
                    ...
                    ledgerCommonPostings.callToPostings(amount, addcharges, tranxTypeMater,
                            addcharges.getAssociateGroups(), mPurchaseTranx.getFiscalYear(), mPurchaseTranx.getBranch(),
                            mPurchaseTranx.getOutlet(), mPurchaseTranx.getInvoiceDate(), mPurchaseTranx.getId(),
                            mPurchaseTranx.getVendorInvoiceNo(), "DR", true, "Purchase Invoice", "Insert");
                }

                /***** NEW METHOD FOR LEDGER POSTING *****/
                postingUtility.callToPostingLedgerForUpdate(isLedgerContains, amount, addcharges.getId(),
                        tranxTypeMater,
         ...
         * } catch (DataIntegrityViolationException e1) {
         * e1.printStackTrace();
         * purInvoiceLogger.error("Error in saveIntoPurchaseAdditionalChargesEdit" +
         * e1.getMessage());
         ...
         */
        /*
         * if Purchace Account ledger is deleted from details table from front end,
         * while edit the purchase invoice
         */
    }
    ...
    public void delAddCharges(JsonArray deletedArrays) {
        ...
        for (JsonElement element : deletedArrays) {
            JsonObject deletedRowsId = element.getAsJsonObject();
            if (deletedRowsId.has("del_id")) {
                mDeletedInvoices = purInvoiceAdditionalChargesRepository
                        .findByIdAndStatus(deletedRowsId.get("del_id").getAsLong(), true);
                if (mDeletedInvoices != null) {
                    mDeletedInvoices.setStatus(false);
                    try {
                        ...
                        ex.printStackTrace(new PrintWriter(sw));
                        String exceptionAsString = sw.toString();
                        purInvoiceLogger.error("Exception in saveInto PurchaseInvoiceDetails Additional Charges Edit:"
                                + exceptionAsString);
                    }
                }
            }
    ...
    public JsonObject purchaseDelete(HttpServletRequest request) {
        JsonObject jsonObject = new JsonObject();
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        TranxPurInvoice invoiceTranx = tranxPurInvoiceRepository
        ...
        try {
            ...
            /*****
             * check whether the reference of purchase invoice is available in purchase
             * return table or not,
            ...
            for (TranxPurReturnInvoice mReturnInvoice : purReturnList) {
                try {
                    ...
                    purInvoiceLogger
                            .error("Exception in Deleting the Purchase Return of Purchase Invoice ->" + e.getMessage());
                    System.out.println(
                            "Exception in Deleting the Purchase Return of Purchase Invoice ->" + e.getMessage());
                }
            }
            List<TranxDebitNoteNewReferenceMaster> noteNewReferenceMasters = new ArrayList<>();
            noteNewReferenceMasters = tranxDebitNoteNewReferenceRepository
                    .findByPurchaseInvoiceIdAndStatus(invoiceTranx.getId(), true);
            if (noteNewReferenceMasters != null && noteNewReferenceMasters.size() > 0) {
                for (TranxDebitNoteNewReferenceMaster mdebitNote : noteNewReferenceMasters) {
                    try {
                        ...
                        String exceptionAsString = sw.toString();
                        purInvoiceLogger
                                .error("Exception in Deleting  NewReference of Purchase Invoice :" + exceptionAsString);
                    }
                }
            }
            tranxPurInvoiceRepository.save(invoiceTranx);
            if (invoiceTranx != null) {
                ...
                /****
                 * make status=0 to all ledgers of respective purchase invoice id, due to this
                 * we wont get
                ...
                mInoiceLedgers = ledgerTransactionPostingsRepository
                        .findByTransactionTypeIdAndTransactionIdAndStatus(1L, invoiceTranx.getId(), true);
                for (LedgerTransactionPostings mPostings : mInoiceLedgers) {
                    try {
                        ...
                        e.printStackTrace(new PrintWriter(sw));
                        String exceptionAsString = sw.toString();
                        purInvoiceLogger.error("Exception in Delete functionality for all ledgers of"
                                + " deleted purchase invoice->" + exceptionAsString);
                    }
                }
                /****
                 * make status=0 to all ledgers of respective purchase return id, due to this we
                 * wont get
                 * details of deleted invoice when we want get details of respective ledger
                 ****/
                for (TranxPurReturnInvoice mList : purReturnList) {
                    ...
                    mReturnList = ledgerTransactionPostingsRepository
                            .findByTransactionTypeIdAndTransactionIdAndStatus(2L, mList.getId(), true);
                ...
                for (TranxPurReturnInvoice returnInvoice : purReturnList) {
                    ...
                    returnList = tranxPurReturnDetailsUnitRepository
                            .findByTranxPurReturnInvoiceIdAndStatus(returnInvoice.getId(), true);
                ...
                unitsList = tranxPurInvoiceUnitsRepository.findByPurchaseTransactionIdAndStatus(invoiceTranx.getId(),
                        true);
                if (unitsList != null && unitsList.size() > 0) {
                    for (TranxPurInvoiceDetailsUnits mUnitObjects : unitsList) {
                        ...
                        inventoryCommonPostings.callToInventoryPostings("DR", invoiceTranx.getInvoiceDate(),
                                ...
                                invoiceTranx.getOutlet(), mUnitObjects.getProduct(), purTranx, null, null, null,
                                mUnitObjects.getUnits(), mUnitObjects.getProductBatchNo(),
                                mUnitObjects.getProductBatchNo() != null ? mUnitObjects.getProductBatchNo().getBatchNo()
                                        : null,
                                invoiceTranx.getFiscalYear(), null);
                        /***** End of new architecture of Inventory Postings *****/
                    }
                }
    ...
    public Object validatePurchaseInvoices(HttpServletRequest request) {
        ...
        if (users.getBranch() != null) {
            purInvoice = tranxPurInvoiceRepository
                    .findByOutletIdAndBranchIdAndSundryCreditorsIdAndStatusAndVendorInvoiceNoIgnoreCase(
                            users.getOutlet().getId(), users.getBranch().getId(), sundryCreditorId, true, billNo);
        } else {
            purInvoice = tranxPurInvoiceRepository
                    .findByOutletIdAndSundryCreditorsIdAndStatusAndVendorInvoiceNoIgnoreCaseAndBranchIsNull(
                            users.getOutlet().getId(), sundryCreditorId, true, billNo);
        }
        if (purInvoice != null) {
    ...
    public Object validateInvoiceNoUpdateNo(HttpServletRequest request) {
        ...
        if (users.getBranch() != null) {
            purInvoice = tranxPurInvoiceRepository
                    .findByOutletIdAndBranchIdAndSundryCreditorsIdAndVendorInvoiceNoIgnoreCase(
                            users.getOutlet().getId(), users.getBranch().getId(), sundryCreditorId,
                            request.getParameter("bill_no"));
        } else {
            purInvoice = tranxPurInvoiceRepository
                    .findByOutletIdAndSundryCreditorsIdAndVendorInvoiceNoIgnoreCaseAndBranchIsNull(
    ...
    public Object updateProductStock(HttpServletRequest request) {
        ...
        try {
            Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
            TransactionTypeMaster tranxType = tranxRepository
                    .findByTransactionCodeIgnoreCase(request.getParameter("tranxType")); // "PRS" => purchase
            String tranxAction = request.getParameter("tranxAction"); // CR or DR
            // LocalDate tranxDate = LocalDate.parse(request.getParameter("tranxDate")); //
            // for purchase => invoice date, for sales => bill date
            ...
            if (!request.getParameter("batchNo").equalsIgnoreCase(""))
            ...
            if (levelAId != null)
                levelA = levelARepository.findByIdAndStatus(levelAId, true);
            if (levelBId != null)
                levelB = levelBRepository.findByIdAndStatus(levelBId, true);
            if (levelCId != null)
                levelC = levelCRepository.findByIdAndStatus(levelCId, true);
            if (unitId != null)
                units = unitsRepository.findByIdAndStatus(unitId, true);
            ...
            try {
                InventoryDetailsPostings savedInventoryDetailsPostings = inventoryDetailsPostingsRepository
                        .save(inventoryDetailsPostingsNew);
    ...
    public JsonObject getSupplierListByProductId(HttpServletRequest request) {
        ...
        List<TranxPurInvoiceDetailsUnits> tranxPurInvoiceDetailsUnits = tranxPurInvoiceUnitsRepository
                .findTop5ByProductIdOrderByIdDesc(productId);

        for (TranxPurInvoiceDetailsUnits obj : tranxPurInvoiceDetailsUnits) {
            if (obj.getPurchaseTransaction().getStatus()) {
                JsonObject response = new JsonObject();
                response.addProperty("supplier_name",
                        obj.getPurchaseTransaction().getSundryCreditors().getLedgerName());
                response.addProperty("invoice_no", obj.getPurchaseTransaction().getVendorInvoiceNo());
                response.addProperty("invoice_date", DateConvertUtil
                        .convertDateToLocalDate(obj.getPurchaseTransaction().getInvoiceDate()).toString());
                response.addProperty("batch",
                        obj.getProductBatchNo() != null ? obj.getProductBatchNo().getBatchNo() : "");
                response.addProperty("mrp",
                        obj.getProductBatchNo() != null ? obj.getProductBatchNo().getMrp().toString() : "");
                response.addProperty("rate",
                        obj.getProductBatchNo() != null ? obj.getProductBatchNo().getPurchaseRate().toString() : "");
    ...
    public Object purchasemobileList(Map<String, String> request) {
        ...
        purInvoice = tranxPurInvoiceRepository.findPurchaseInvoicesListNoBr(
                Long.valueOf(request.get("sundry_creditor_id")), startDatep, endDatep, true);
    ...
    public Object purchaseinvoicedetails(Map<String, String> request) {
        ...
        purInvoice = tranxPurInvoiceUnitsRepository
                .findPurchaseInvoicesDetails(Long.valueOf(request.get("purchase_invoice_id")), true);
        ...
        TranxPurInvoice tranxPurInvoice = tranxPurInvoiceRepository
                .findByIdAndStatus(Long.parseLong(request.get("purchase_invoice_id")), true);
        JsonObject output = new JsonObject();
        output.addProperty("message", "success");
        output.addProperty("responseStatus", HttpStatus.OK.value());
    ...
    public JsonObject purchasePrintInvoice(HttpServletRequest request) {
        ...
        try {
            ...
            if (source.equalsIgnoreCase("purchase_invoice")) {
                if (users.getBranch() != null) {
                    if (key.equalsIgnoreCase("create")) {
                        purInvoice = tranxPurInvoiceRepository.findByVendorInvoiceNoAndOutletIdAndBranchIdAndStatus(
                                invoiceNo, users.getOutlet().getId(), users.getBranch().getId(), true);
                    } else {
                        id = Long.parseLong(invoiceNo);
                        purInvoice = tranxPurInvoiceRepository.findByIdAndOutletIdAndBranchIdAndStatus(id,
                                users.getOutlet().getId(), users.getBranch().getId(), true);
                    }
                } else {
                    if (key.equalsIgnoreCase("create")) {
                        purInvoice = tranxPurInvoiceRepository.findByVendorInvoiceNoAndOutletIdAndStatusAndBranchIsNull(
                                invoiceNo, users.getOutlet().getId(), true);
                    } else {
                        id = Long.parseLong(invoiceNo);
                        purInvoice = tranxPurInvoiceRepository.findByIdAndOutletIdAndStatusAndBranchIsNull(id,
                                users.getOutlet().getId(), true);
                    }
                }
            } else {
                 ...
                 * if (key.equalsIgnoreCase("create")) {
                 * counterSales =
                 * counterSaleRepository.findByCounterSaleNoAndOutletIdAndBranchIdAndStatus(
                 * invoiceNo, users.getOutlet().getId(), users.getBranch().getId(), true);
                 * } else {
                 * id = Long.parseLong(invoiceNo);
                 * counterSales =
                 * counterSaleRepository.findByIdAndOutletIdAndBranchIdAndStatus(id,
                 * users.getOutlet().getId(), users.getBranch().getId(), true);
                 * }
                 * } else {
                 * if (key.equalsIgnoreCase("create")) {
                 * counterSales =
                 * counterSaleRepository.findByCounterSaleNoAndOutletIdAndStatusAndBranchIsNull(
                 * invoiceNo, users.getOutlet().getId(), true);
                 * } else {
                 * id = Long.parseLong(invoiceNo);
                 * counterSales =
                 * counterSaleRepository.findByIdAndOutletIdAndStatusAndBranchIsNull(id,
                 * users.getOutlet().getId(), true);
                 * }
                 * }
                 */
            ...
            if (purInvoice != null) {
                // list =
                // tranxPurInvoiceUnitsRepository.findByPurchaseTransactionIdAndStatus(purInvoice.getId(),
                // true);
                ...
                invoiceObject.addProperty("total_discount",
                        numFormat.numFormat(purInvoice.getTotalPurchaseDiscountAmt()));
                ...
                // numFormat.numFormat(purInvoice.getAd() != null ?
                // purInvoice.getAdvancedAmount() : 0.0));
                ...
                List<TranxPurInvoiceDetailsUnits> unitDetails = tranxPurInvoiceUnitsRepository
                        .findByPurchaseTransactionIdAndStatus(purInvoice.getId(), true);
                JsonArray productDetails = new JsonArray();
                unitDetails.forEach(mUnit -> {
                    JsonObject mObject = new JsonObject();
             ...
             * else {
             * listcs =
             * tranxCSDetailsUnitsRepository.findByCounterSalesIdAndStatus(counterSales.
             * getId(), true);
             * JsonObject companyObject = new JsonObject();
             * companyObject.addProperty("company_name",
             * users.getOutlet().getCompanyName());
             * companyObject.addProperty("company_address",
             * users.getOutlet().getCorporateAddress());
             * companyObject.addProperty("phone_number",
             * users.getOutlet().getMobileNumber());
             * companyObject.addProperty("email_address", users.getOutlet().getEmail());
             * companyObject.addProperty("gst_number", users.getOutlet().getGstNumber());
             * JsonObject debtorsObject = new JsonObject();
            ...
            /* Sales Invoice Data *//*
                                     ...
                                     * counterSales.getCounterSalesDate().toString());
                                     * invoiceObject.addProperty("invoice_no", counterSales.getCounterSaleNo());
                                     * invoiceObject.addProperty("state_code",
                                     * counterSales.getOutlet().getStateCode());
                                     * invoiceObject.addProperty("state_name",
                                     * counterSales.getOutlet().getState().getName());
                                     * invoiceObject.addProperty("taxable_amt",
                                     * numFormat.numFormat(counterSales.getTotalBaseAmt()));
                                     * invoiceObject.addProperty("tax_amount",
                                     * numFormat.numFormat(counterSales.getTotaligst()));
                                     * invoiceObject.addProperty("total_cgst",
                                     * numFormat.numFormat(counterSales.getTotalcgst()));
                                     * invoiceObject.addProperty("total_sgst",
                                     * numFormat.numFormat(counterSales.getTotalsgst()));
                                     * invoiceObject.addProperty("net_amount",
                                     * numFormat.numFormat(counterSales.getTotalBaseAmt()));
                                     * invoiceObject.addProperty("total_discount",
                                     * numFormat.numFormat(counterSales.getTotalDiscount()));
                                     * invoiceObject.addProperty("total_amount",
                                     * numFormat.numFormat(counterSales.getTotalBill()));
                                     * invoiceObject.addProperty("advanced_amount",
                                     * numFormat.numFormat(counterSales.getAdvancedAmount() != null ?
                                     * salesInvoice.getAdvancedAmount() : 0.0));
                                                                      ...
                                                                      *//* Sales Invoice Details *//*
                                                                                                     ...
                                                                                                     *//*
                                                                                                        ...
                                                                                                        *//*
                                                                                                           ...
                                                                                                           * unitDetails
                                                                                                           * =
                                                                                                           * tranxCSDetailsUnitsRepository
                                                                                                           * .
                                                                                                           * findByCounterSalesIdAndStatus
                                                                                                           * (mDetails.
                                                                                                           * getId(),
                                                                                                           * true);
                                                                                                           * JsonArray
    ...
    public JsonObject dashboardPucrhaseList(HttpServletRequest request) {
        JsonArray result = new JsonArray();
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        List<TranxPurInvoice> purInvoice = new ArrayList<>();
        if (users.getBranch() != null) {
            purInvoice = tranxPurInvoiceRepository.findPurchaseInvoice(users.getOutlet().getId(), users.getBranch().getId(), true);
        } else {
            purInvoice = tranxPurInvoiceRepository.findPucrhaseInvoiceBranchNull(users.getOutlet().getId(), true);
        }
        for (TranxPurInvoice invoices : purInvoice) {
            JsonObject response = new JsonObject();
            response.addProperty("id", invoices.getId());
            ...
            response.addProperty("total_amount", invoices.getTotalAmount());
            response.addProperty("sundry_debtor_name", invoices.getSundryCreditors().getLedgerName());
            response.addProperty("sundry_debtor_id", invoices.getSundryCreditors().getId());
            response.addProperty("sale_account_name", invoices.getPurchaseAccountLedger().getLedgerName());
            response.addProperty("baseamt", invoices.getTotalBaseAmount());
