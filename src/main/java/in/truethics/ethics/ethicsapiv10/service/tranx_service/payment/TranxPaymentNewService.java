...
public class TranxPaymentNewService {
    ...
    private String gvserverUrl;
    @Autowired
    PostingUtility postingUtility;
...
//        tranxPayment.setTranscationDate(tranxDate);
        ...
        /****** GV specific Code , while payment from FR to GV *****/
        if (paramMap.containsKey("gv_bank_name")) {
            String accountNo = request.getParameter("gv_bank_name");
            if (!accountNo.equalsIgnoreCase("") && accountNo != null) {
                LedgerBankDetails bankDetails = ledgerBankDetailsRepository.findByAccountNoAndStatus(accountNo, true);
                if (bankDetails != null)
                    tranxPayment.setGvBankName(bankDetails.getBankName());
                tranxPayment.setGvBankLedgerId(bankDetails.getId());
            }
        }
        if (paramMap.containsKey("gv_bank_payment_type"))
            tranxPayment.setGvPaymentMode(request.getParameter("gv_bank_payment_type"));
        if (paramMap.containsKey("gv_payment_tranx_no"))
            tranxPayment.setGvPaymentTranxNo(request.getParameter("gv_payment_tranx_no"));
        /***** End Gv specific Code *****/
        ...
        try {
            ...
            for (int i = 0; i < row.size(); i++) {
                ...
                if (ledgerMaster != null) {
                    ...
                    if (ledgerMaster.getLedgerCode() != null && ledgerMaster.getLedgerCode().equalsIgnoreCase("gvmh001")) {
                        isGVLedger = true;
                    }
                }
            ...
            if (isGVLedger == true) {
                /****** Automatic creation of receipt in GV DB *******/
                ...
                gvBody.add("transaction_dt", request.getParameter("transaction_dt"));
                gvBody.add("narration", request.getParameter("narration"));
                gvBody.add("total_amt", request.getParameter("total_amt"));
                PaymentModeMaster mPaymentModeMaster = paymentModeMasterRepository.findByIdAndStatus(Long.parseLong(request.getParameter("gv_bank_payment_type")), true);
                gvBody.add("gv_bank_payment_type", request.getParameter("gv_bank_payment_type"));
                gvBody.add("gv_bank_payment_type_name", mPaymentModeMaster.getPaymentMode());
                gvBody.add("gv_bank_payment_no", request.getParameter("gv_bank_payment_no"));
                gvBody.add("gv_payment_tranx_no", request.getParameter("gv_payment_tranx_no"));
                gvBody.add("gv_bank_name", request.getParameter("gv_bank_name"));
                gvBody.add("payment_dt", request.getParameter("transaction_dt"));
                gvBody.add("row", request.getParameter("row"));
                gvBody.add("receipt_status", "Placed");

                HttpEntity gvEntity = new HttpEntity<>(gvBody, gvHdr);
                String gvData = restTemplate.exchange(
                        gvserverUrl + "/create_receipt_fr", HttpMethod.POST, gvEntity, String.class).getBody();
            }
                ...
                if (ledgerMaster.getBalancingMethod().getId() == 1) {
                    ...
                    if (purInvoice.size() > 0) {
                        for (TranxPurInvoice newPurInvoice : purInvoice) {
                            ...
                            response.addProperty("tranx_tracking_code", newPurInvoice.getTransactionTrackingNo()
                                    != null ? newPurInvoice.getTransactionTrackingNo() : "");
...
//            responseMessage.setMessage(HttpStatus.BAD_REQUEST.toString());
    ...
    private PaymentDTO convertToDTDTO(TranxPaymentMaster tranxPaymentMaster) {
        ...
        paymentDTO.setTransaction_dt(tranxPaymentMaster.getTranscationDate().toString());
    ...
    public JsonObject getPaymentById(HttpServletRequest request) {
        ...
        try {
            ...
            finalResult.addProperty("gv_bank_payment_type", tranxPaymentMaster.getGvPaymentMode());
            finalResult.addProperty("gv_bank_name", tranxPaymentMaster.getGvBankName());
            finalResult.addProperty("gv_bank_id", tranxPaymentMaster.getGvBankLedgerId() != null ? tranxPaymentMaster.getGvBankLedgerId().toString():"");
            /****** get account number by id from ledger bank details *****/
            LedgerBankDetails bank = ledgerBankDetailsRepository.findByLedgerMasterIdAndBankNameIgnoreCaseAndStatus(
                    tranxPaymentMaster.getGvBankLedgerId(), tranxPaymentMaster.getGvBankName(), true);
            if(bank!=null) {
                finalResult.addProperty("gv_bank_account_no", bank.getAccountNo() != "" && bank.getAccountNo() != null ? bank.getAccountNo() : "");
            }
            else{
                finalResult.addProperty("gv_bank_account_no", "");
            }
            finalResult.addProperty("gv_payment_tranx_no", tranxPaymentMaster.getGvPaymentTranxNo()!=""?tranxPaymentMaster.getGvPaymentTranxNo():"");
