...
public class ProductService {
    ...
    public JsonObject validateProduct(HttpServletRequest request) {
        ...
        try {
            ...
            if (users.getBranch() != null) {
                if (packageId != null) {
                    product = productRepository.findByduplicateProductPKWBR(users.getOutlet().getId(),
                            users.getBranch().getId(), productName, packageId, true);
                } else {
                    product = productRepository.findByduplicateProductPNWBR(users.getOutlet().getId(),
                            users.getBranch().getId(), productName, true);
                }
            } else if (packageId != null) {
                product = productRepository.findByduplicateProductPK(users.getOutlet().getId(), productName, packageId,
                        true);
            } else {
    ...
    private void setProductOpeningStocks(Product newProduct, Users users, double opening_qty, Units unit,
                                         PackingMaster packingMaster, Double openingRate, Double valuation) {
            ...
            if (mProduct != null) {
                ...
                for (ProductUnitPacking mUnit : units) {
                    ...
                    if (mUnit.getPackingMaster() != null)
                        pUnit.setPackageId(mUnit.getPackingMaster().getId());
                    if (mUnit.getBrand() != null)
                        pUnit.setBrandId(mUnit.getBrand().getId());
                    if (mUnit.getGroup() != null)
                        pUnit.setGroupId(mUnit.getGroup().getId());
                    if (mUnit.getCategory() != null)
                        pUnit.setCategoryId(mUnit.getCategory().getId());
                    if (mUnit.getSubcategory() != null)
                        pUnit.setSubcategoryId(mUnit.getSubcategory().getId());
                    list.add(pUnit);
                }
                pData.setUnits(list);
    ...
    public JsonObject getProductsOfOutlet(HttpServletRequest request) {
        ...
        Long branchId = null;
        if (users.getBranch() != null) {
            productList = productRepository.findByOutletIdAndBranchIdAndStatus(users.getOutlet().getId(),
        ...
        for (Product mProduct : productList) {
            ...
            mObject.addProperty("packing",
                    mProduct.getPackingMaster() != null ? mProduct.getPackingMaster().getPackName() : "");
            ...
            if (mProduct.getIsBatchNumber()) {
                ...
                if (tranxPurInvoiceDetailsUnits != null) {
                    ...
                    ProductBatchNo productBatchNos = productBatchNoRepository
                            .findTop1ByProductIdAndStatusOrderByIdDesc(mProduct.getId(), true);
                ...
                if (mUnits != null && mUnits.size() > 0) {
                    ...
                    mObject.addProperty("sales_rate",
                            mUnits.get(0).getMinRateA() != null ? mUnits.get(0).getMinRateA() : 0.00);
                }
                mObject.addProperty("unit", "PCS");
            ...
            Double freeQty = inventoryCommonPostings.calculateFreeQty(mProduct.getId(), users.getOutlet().getId(),
                    branchId, fiscalYear);
            Double openingStocks = 0.0;
            openingStocks = inventoryCommonPostings.calculateOpening(mProduct.getId(), users.getOutlet().getId(),
                    branchId, fiscalYear);
            mObject.addProperty("opening_stocks", openingStocks);
            Double closingStocks = inventoryCommonPostings.getClosingStockProduct(mProduct.getId(),
                    users.getOutlet().getId(), branchId, fiscalYear);
    ...
    public Object getProductsOfOutletNew(@RequestBody Map<String, String> request, HttpServletRequest req) {
        ...
        try {
            if (searchText.equalsIgnoreCase("")) {
                ...
                query = "SELECT distinct(product_tbl.id) FROM product_tbl WHERE status=1 AND outlet_id="
            ...
            if (!searchText.equalsIgnoreCase(""))
                query = query + " AND (product_tbl.product_name LIKE '%" + searchText
                        + "%' OR product_content_master_tbl.content_type LIKE '%" + searchText
                        + "%' OR product_tbl.product_code LIKE '%" + searchText + "%' )";
            ...
            String query1 = "SELECT COUNT(product_tbl.id) as totalcount FROM product_tbl WHERE product_tbl.status=? "
                    + "AND product_tbl.outlet_id=?";
            if (users.getBranch() != null) {
            ...
            for (Object mProduct : products) {
                ...
                Product productListView = productRepository.findByIdAndStatus(Long.parseLong(mProduct.toString()),
                        true);
                productDTOList.add(convertToDTDTO(productListView, users, branchId));
            }
            GenericDatatable<ProductDTO> data = new GenericDatatable<>(productDTOList, totalProducts, pageNo, pageSize,
                    total_pages);
    ...
    private ProductDTO convertToDTDTO(Product productList, Users users, Long branchId) {
        ...
        productDTO
                .setPacking(productList.getPackingMaster() != null ? productList.getPackingMaster().getPackName() : "");
        ...
        if (productList.getIsBatchNumber()) {
            TranxPurInvoiceDetailsUnits tranxPurInvoiceDetailsUnits = tranxPurInvoiceDetailsUnitsRepository
                    .findTop1ByProductIdOrderByIdDesc(productList.getId());
            if (tranxPurInvoiceDetailsUnits != null) {
                productDTO.setMrp(tranxPurInvoiceDetailsUnits.getProductBatchNo() != null
                        ? tranxPurInvoiceDetailsUnits.getProductBatchNo().getMrp()
                        : 0.00);
                productDTO.setSales_rate(tranxPurInvoiceDetailsUnits.getProductBatchNo() != null
                        ? tranxPurInvoiceDetailsUnits.getProductBatchNo().getSalesRate()
                        : 0.00);
            } else {
                ProductBatchNo productBatchNos = productBatchNoRepository
                        .findTop1ByProductIdAndStatusOrderByIdDesc(productList.getId(), true);
                productDTO.setMrp(
                        productBatchNos != null ? productBatchNos.getMrp() != null ? productBatchNos.getMrp() : 0.0
                                : 0.00);
                productDTO.setSales_rate(productBatchNos != null
                        ? productBatchNos.getSalesRate() != null ? productBatchNos.getSalesRate() : 0.0
                        : 0.00);
                productDTO.setPurchase_rate(productBatchNos != null
                        ? productBatchNos.getPurchaseRate() != null ? productBatchNos.getPurchaseRate() : 0.0
                        : 0.0);
            ...
            if (mUnits != null && mUnits.size() > 0) {
                ...
                productDTO.setPurchase_rate(
                        mUnits.get(0).getPurchaseRate() != null ? mUnits.get(0).getPurchaseRate() : 0.0);
            } else {
                productDTO.setUnit("PCS");
        ...
        /* fiscal year mapping */
        FiscalYear fiscalYear = generateFiscalYear.getFiscalYear(currentDate);
        Double freeQty = inventoryCommonPostings.calculateFreeQty(productList.getId(), users.getOutlet().getId(),
                branchId, fiscalYear);
        Double openingStocks = 0.0;
        openingStocks = inventoryCommonPostings.calculateOpening(productList.getId(), users.getOutlet().getId(),
                branchId, fiscalYear);
        productDTO.setOpening_stocks(openingStocks);
        Double closingStocks = inventoryCommonPostings.getClosingStockProduct(productList.getId(),
                users.getOutlet().getId(), branchId, fiscalYear);
        Double currentStock = closingStocks + openingStocks + freeQty;
    ...
    /******
     * new Architecture : added Level A ,Level B , Level C and units in Product ,PK
     * visit
     *****/
        ...
        for (Long mLeveA : levelaArray) {
            ...
            for (Long mLeveB : levelBunits) {
                ...
                for (Long mLeveC : levelCunits) {
                    ...
                    List<Object[]> unitList = productUnitRepository.findUniqueUnitsByProductId(product_id, mLeveA,
                            mLeveB, mLeveC);
    ...
    public Object updateProduct(HttpServletRequest request) {
        ...
        if (paramMap.containsKey("productCode"))
        ...
        if (paramMap.containsKey("barcodeNo"))
            product.setBarcodeNo(request.getParameter("barcodeNo"));
        else
            product.setBarcodeNo("");
        ...
        if (paramMap.containsKey("drugType"))
            product.setDrugType(request.getParameter("drugType"));
        if (paramMap.containsKey("productType"))
        ...
        if (paramMap.containsKey("shelfId"))
            product.setShelfId(request.getParameter("shelfId"));
        else
            product.setShelfId("");
        ...
        if (paramMap.containsKey("packagingId")) {
            mPackingMaster = packingMasterRepository
                    .findByIdAndStatus(Long.parseLong(request.getParameter("packagingId")), true);
        ...
        if (paramMap.containsKey("categoryId")) {
            mCategoryMaster = categoryRepository.findByIdAndStatus(Long.parseLong(request.getParameter("categoryId")),
                    true);
        ...
        if (paramMap.containsKey("weightUnit"))
        ...
        if (paramMap.containsKey("disPer1"))
            ...
            product.setDiscountInPer(0.0);
        if (paramMap.containsKey("hsnNo")) {
            ProductHsn productHsn = productHsnRepository
                    .findByIdAndStatus(Long.parseLong(request.getParameter("hsnNo")), true);
            if (productHsn != null) {
        ...
        if (paramMap.containsKey("tax")) {
            ...
            TaxMaster taxMaster = taxMasterRepository.findByIdAndStatus(Long.parseLong(request.getParameter("tax")),
                    true);
            if (taxMaster != null) {
        ...
        if (paramMap.containsKey("taxType"))
            product.setTaxType(request.getParameter("taxType"));
        if (paramMap.containsKey("igst"))
            product.setIgst(Double.parseDouble(request.getParameter("igst")));
        if (paramMap.containsKey("cgst"))
            product.setCgst(Double.parseDouble(request.getParameter("cgst")));
        if (paramMap.containsKey("sgst"))
            product.setSgst(Double.parseDouble(request.getParameter("sgst")));
            ...
            for (JsonElement mList : array) {
                ...
                for (JsonElement mLevelB : leveBArray) {
                    ...
                    if (!mLevelBAsJsonObject.get("levelb_id").getAsString().equalsIgnoreCase("")) {
                        levelB = levelBRepository.findByIdAndStatus(mLevelBAsJsonObject.get("levelb_id").getAsLong(),
                                true);
                    }
                    ...
                    for (JsonElement mLevelC : levelCArray) {
                        ...
                        if (!mLevelCObject.get("levelc_id").getAsString().equalsIgnoreCase("")) {
                            levelC = levelCRepository.findByIdAndStatus(mLevelCObject.get("levelc_id").getAsLong(),
                                    true);
                        }
                        ...
                        for (JsonElement mUnitsList : unitsArray) {
                            ...
                            Units unit = unitsRepository.findByIdAndStatus(mUnitObject.get("unit_id").getAsLong(),
                                    true);
                            ...
                            for (JsonElement mBatchElement : mBatchJsonArray) {
                                ...
                                if (mBatchJsonObject.get("isOpeningbatch").getAsBoolean()) {
                                    ...
                                    if (batch_id.equalsIgnoreCase("")) {
                                        ...
                                        mproductBatchNo = productBatchNoRepository
                                                .findByIdAndStatus(Long.parseLong(batch_id), true);
                                    ...
                                    if (mBatchJsonObject.has("b_purchase_rate"))
                                        mproductBatchNo
                                                .setPurchaseRate(mBatchJsonObject.get("b_purchase_rate").getAsDouble());
                                    ...
                                    if (mBatchJsonObject.has("b_manufacturing_date") && !mBatchJsonObject
                                            .get("b_manufacturing_date").getAsString().equalsIgnoreCase(""))
                                        mproductBatchNo.setManufacturingDate(LocalDate
                                                .parse(mBatchJsonObject.get("b_manufacturing_date").getAsString()));
                                    if (mBatchJsonObject.has("b_expiry")
                                            && !mBatchJsonObject.get("b_expiry").getAsString().equalsIgnoreCase(""))
                                        mproductBatchNo.setExpiryDate(
                                                LocalDate.parse(mBatchJsonObject.get("b_expiry").getAsString()));
                                ...
                                try {
                                    ...
                                    newOpeningStock.setOpeningStocks(
                                            Double.parseDouble(mBatchJsonObject.get("opening_qty").getAsString()));
                                    newOpeningStock.setProductBatchNo(productBatchNo);
                                    if (mBatchJsonObject.has("b_free_qty"))
                                        newOpeningStock
                                                .setFreeOpeningQty(mBatchJsonObject.get("b_free_qty").getAsDouble());
                                    if (mBatchJsonObject.has("b_mrp"))
                                    ...
                                    if (mBatchJsonObject.has("b_manufacturing_date") && !mBatchJsonObject
                                            .get("b_manufacturing_date").getAsString().equalsIgnoreCase(""))
                                        newOpeningStock.setManufacturingDate(LocalDate
                                                .parse(mBatchJsonObject.get("b_manufacturing_date").getAsString()));
                                    if (mBatchJsonObject.has("b_expiry")
                                            && !mBatchJsonObject.get("b_expiry").getAsString().equalsIgnoreCase(""))
                                        newOpeningStock.setExpiryDate(
                                                LocalDate.parse(mBatchJsonObject.get("b_expiry").getAsString()));
    ...
    public JsonObject getProductLevelsNew(Product mProduct, Users users) {
        ...
        if (users.getBranch() != null) {
            list = levelARepository.findByOutletIdAndStatusAndBranchId(users.getOutlet().getId(), true,
        ...
        if (users.getBranch() != null) {
            levelBList = levelBRepository.findByOutletIdAndStatusAndBranchId(users.getOutlet().getId(), true,
                    users.getBranch().getId());
        } else {
        ...
        if (users.getBranch() != null) {
            levelCList = levelCRepository.findByOutletIdAndBranchIdAndStatus(users.getOutlet().getId(),
                    users.getBranch().getId(), true);
        } else {
        ...
        if (isPakage == false) {
            ...
            for (ProductUnitPacking mUnits : productUnitPackings) {
                ...
                if (mUnits.getIsNegativeStocks() != null)
                    ...
                    mUnitsObj.addProperty("isNegativeStocks", 0);
                ...
                mUnitsObj.addProperty("taxMasterId",
                        mUnits.getTaxMaster() != null ? mUnits.getTaxMaster().getId() : null);
                mUnitsObj.addProperty("applicableDate",
                        mUnits.getTaxApplicableDate() != null ? mUnits.getTaxApplicableDate().toString() : "");
            ...
            if (unitPackingList != null && unitPackingList.size() > 0) {
                for (Long mPack : unitPackingList) {
                    ...
                    if (mPack != null) {
                        ...
                        productUnitPackings = productUnitRepository.findByProductIdAndPackingMasterId(product_id,
                                mPack);
                    } else {
                        mObject.addProperty("id", "");
                        mObject.addProperty("pack_name", "");
    ...
    public JsonObject getProduct(HttpServletRequest request) {
        ...
        for (Product mProduct : productList) {
            ...
            ProductBarcode productBarcode = barcodeRepository.findByProductIdAndOutletIdAndStatus(mProduct.getId(),
            ...
            response.addProperty("isBatchNo",
                    mProduct.getIsBatchNumber() != null ? mProduct.getIsBatchNumber() : false);
    ...
    public JsonObject deleteProductList(HttpServletRequest request) {
        ...
        if (object != 0) {
            List<ProductOpeningStocks> productOpeningStocks = openingStocksRepository.findByProductIdAndStatus(object,
                    true);
            if (productOpeningStocks != null && productOpeningStocks.size() > 0) {
                ...
                List<InventoryDetailsPostings> inventoryList = inventoryDetailsPostingsRepository
                        .findByProductIdAndStatus(object, true);
                // double sumOfCr =
                // inventoryDetailsPostingsRepository.getSumOfCrOrDrByProductId(object, "CR");
                // double sumOfDr =
                // inventoryDetailsPostingsRepository.getSumOfCrOrDrByProductId(object, "DR");
    ...
    public Object createNewProduct(MultipartHttpServletRequest request) {
        ...
        try {
            ...
            if (paramMap.containsKey("categoryId")) {
                mCategoryMaster = categoryRepository
                        .findByIdAndStatus(Long.parseLong(request.getParameter("categoryId")), true);
                product.setCategory(mCategoryMaster);
            }
            if (paramMap.containsKey("weight"))
                product.setWeight(Double.parseDouble(request.getParameter("weight")));
            ...
            if (paramMap.containsKey("tax")) {
                 ...
                 * productTaxDateMaster.setApplicableDate(productUnitPacking.
                 * getTaxApplicableDate());
                 ...
                 * } catch (Exception e) {
                 * productLogger.
                 * error("Error in Product Creation-> ProductTaxDateMaster Creation-> " +
                 * e.getMessage());
                 * }
                 * }
                 * /***** End of inserting into ProductTax Master
                        ...
                        for (JsonElement mUnitsList : unitsArray) {
                            ...
                            for (JsonElement mBatchElement : mBatchJsonArray) {
                                ...
                                if (mBatchJsonObject.get("isOpeningbatch").getAsBoolean()) {
                                    ...
                                    mproductBatchNo.setUnits(unit);
                                    mproductBatchNo.setQnty(
                                            Integer.parseInt(mBatchJsonObject.get("opening_qty").getAsString()));
                                    if (fiscalYear != null)
                                        mproductBatchNo.setFiscalYear(fiscalYear);
                                    productBatchNo = productBatchNoRepository.save(mproductBatchNo);
                                }
        ...
        if (productContentElementsJson.size() > 0) {
            for (JsonElement mList : productContentElementsJson) {
                ...
                if (object.has("id") && !object.get("id").getAsString().isEmpty())
                    productContent = productContentMasterRepository.findByIdAndStatus(object.get("id").getAsLong(),
                            true);
            ...
            if (removeJsonBalance.size() > 0) {
                for (JsonElement mList : removeJsonBalance) {
                    ...
                    if (object != 0) {
                        ...
                        if (mBalanceDetails != null)
                            mBalanceDetails.setStatus(false);
                        try {
    ...
    public JsonObject getProductByIdEditFlavourNew(HttpServletRequest request) {
        ...
        Product mProduct = productRepository.findByIdAndStatus(productId, true);
        JsonObject result = new JsonObject();
        // List<ProductUnitPacking> units =
        // productUnitRepository.findByProductIdAndStatus(mProduct.getId(), true);
        JsonObject response = new JsonObject();
        try {
            if (mProduct != null) {
                response.addProperty("productName", mProduct.getProductName());
                ...
                response.addProperty("packagingId",
                        mProduct.getPackingMaster() != null ? mProduct.getPackingMaster().getId() : null);
                response.addProperty("groupId", mProduct.getGroup() != null ? mProduct.getGroup().getId() : null);
                response.addProperty("categoryId",
                        mProduct.getCategory() != null ? mProduct.getCategory().getId() : null);
                ...
                response.addProperty("hsnNo",
                        mProduct.getProductHsn() != null ? mProduct.getProductHsn().getId() : null);
                response.addProperty("tax", mProduct.getTaxMaster() != null ? mProduct.getTaxMaster().getId() : null);
                response.addProperty("taxApplicableDate",
                        mProduct.getApplicableDate() != null ? mProduct.getApplicableDate().toString() : null);
    ...
    public Object productTransactionsDetails(HttpServletRequest request) {
        ...
        JsonObject mObject = new JsonObject();
        Long productId = Long
                .parseLong(request.getParameter("product_id").isEmpty() ? "0" : request.getParameter("product_id"));
        Product product = null;
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        try {
            if (users.getBranch() != null) {
                product = productRepository.findByIdAndOutletIdAndBranchIdAndStatus(productId,
                        users.getOutlet().getId(), users.getBranch().getId(), true);
            } else {
                product = productRepository.findByIdAndOutletIdAndStatusAndBranchIsNull(productId,
                        users.getOutlet().getId(), true);
            }
            if (product != null) {
                ...
                ProductBatchNo productBatchNo = productBatchNoRepository
                        .findTop1ByProductIdAndStatusOrderByIdDesc(product.getId(), true);
                List<ProductUnitPacking> mUnits = productUnitRepository.findByProductId(product.getId());
                if (mUnits != null) {
                    mObject.addProperty("min_stocks",
                            mUnits.get(0).getMinQty() != null ? mUnits.get(0).getMinQty() : 0.0);
                    mObject.addProperty("max_stocks",
                            mUnits.get(0).getMaxQty() != null ? mUnits.get(0).getMaxQty() : 0.0);
                }
                if (productBatchNo != null) {
                    ...
                    mObject.addProperty("purchase_rate",
                            productBatchNo.getPurchaseRate() != null ? productBatchNo.getPurchaseRate() : 0.00);
                    mObject.addProperty("cost",
                            productBatchNo.getCosting() != null ? productBatchNo.getCosting() : 0.00);
                } else {
                    if (mUnits != null && mUnits.size() > 0) {
                        mObject.addProperty("purchase_rate",
                                mUnits.get(0).getPurchaseRate() != null ? mUnits.get(0).getPurchaseRate() : 0.0);
                        mObject.addProperty("cost",
                                mUnits.get(0).getCosting() != null ? mUnits.get(0).getCosting() : 0.0);
                    } else {
                ...
                mObject.addProperty("subgroup",
                        product.getSubgroup() != null ? product.getSubgroup().getSubgroupName() : "");
                mObject.addProperty("category",
                        product.getCategory() != null ? product.getCategory().getCategoryName() : "");
                mObject.addProperty("hsn",
                        product.getProductHsn() != null ? product.getProductHsn().getHsnNumber() : "");
                ...
                TranxPurInvoiceDetailsUnits tranxPurInvoiceDetailsUnits = tranxPurInvoiceDetailsUnitsRepository
                        .findTop1ByProductIdOrderByIdDesc(product.getId());
                if (tranxPurInvoiceDetailsUnits != null) {
                    mObject.addProperty("supplier",
                            tranxPurInvoiceDetailsUnits.getPurchaseTransaction().getSundryCreditors().getLedgerName());
                }
    ...
    public Object productTransactionsListByBarcode(HttpServletRequest request) {
        ...
        String query = "SELECT * FROM `product_tbl` LEFT JOIN product_barcode_tbl ON product_tbl.id=product_barcode_tbl.product_id"
                + " WHERE product_tbl.outlet_id=" + users.getOutlet().getId() + " AND product_tbl.status=1";

        if (users.getBranch() != null) {
            query = query + " AND product_tbl.branch_id=" + users.getBranch().getId();
        }
        if (!barcode.equalsIgnoreCase("")) {
            query = query + " AND product_barcode_tbl.barcode_unique_code='" + barcode + "'";
        }
        System.out.println("query " + query);
        Query q = entityManager.createNativeQuery(query, Product.class);
        ...
        if (productDetails != null && productDetails.size() == 1) {
            for (Product mDetails : productDetails) {
                ...
                JsonObject batchJsobObject = new JsonObject();
                JsonArray unitJsonArray = new JsonArray();
                JsonObject mObject = new JsonObject();


                if (productBatchNo != null) {
                    batchJsobObject.addProperty("batch_no", productBatchNo.getBatchNo());
                    batchJsobObject.addProperty("batch_id", productBatchNo.getId());
                }
                ...
                if (productUnitPacking != null && productUnitPacking.size() > 0) {
                    for (ProductUnitPacking unitPacking :
                        ...
                        unitJsonObject.addProperty("unitName", unitPacking.getUnits().getUnitName());
                        unitJsonObject.addProperty("unitPackingId", unitPacking.getId());
                        unitJsonObject.addProperty("unitId", unitPacking.getUnits().getId());
                        unitJsonArray.add(unitJsonObject);
                    }
                }

                mObject.add("batch_data", batchJsobObject);
                mObject.add("unit_data", unitJsonArray);
                ...
                mObject.addProperty("barcode", barcode);
                mObject.addProperty("tax_igst", mDetails.getTaxMaster().getIgst());
                mObject.addProperty("drug_type", mDetails.getDrugType());
    ...
    public Object productUnits(HttpServletRequest request) {
        ...
        try {
            ...
            Long level_b_id = request.getParameter("level_b_id").equalsIgnoreCase("null") ? null
                    : Long.valueOf(request.getParameter("level_b_id"));
            Long level_c_id = request.getParameter("level_c_id").equalsIgnoreCase("null") ? null
                    : Long.valueOf(request.getParameter("level_c_id"));
            List<Object[]> unitList = productUnitRepository.findUniqueUnitsByProductId(Long.valueOf(productId),
                    level_a_id, level_b_id, level_c_id);
            ...
            for (Long mLevelB : levelBArray) {
                ...
                for (Long mLevelC : levelCArray) {
                    ...
                    for (ProductUnitPacking mUnits : unitPackingList) {
                        ...
                        mUnitObject.addProperty("levelAId",
                                mUnits.getLevelA() != null ? mUnits.getLevelA().getId() : null);
                        mUnitObject.addProperty("levelBId",
                                mUnits.getLevelB() != null ? mUnits.getLevelB().getId() : null);
                        mUnitObject.addProperty("levelCId",
                        ...
                        List<ProductOpeningStocks> openingStocks = openingStocksRepository.findByProductOpening(
                                mUnits.getProduct().getId(), mUnits.getUnits().getId(), levelaUnit, levelbUnit,
                                levelcUnit);
                        for (ProductOpeningStocks mOpeningStocks : openingStocks) {
                            JsonObject mObject = new JsonObject();
                            mObject.addProperty("id", mOpeningStocks.getId());
                            mObject.addProperty("b_no",
                                    mOpeningStocks.getProductBatchNo() != null
                                            ? mOpeningStocks.getProductBatchNo().getBatchNo()
                                            : "");
                            mObject.addProperty("batch_id",
                                    mOpeningStocks.getProductBatchNo() != null
                                            ? mOpeningStocks.getProductBatchNo().getId().toString()
                                            : "");
                            ...
                            mObject.addProperty("b_expiry",
                                    mOpeningStocks.getExpiryDate() != null ? mOpeningStocks.getExpiryDate().toString()
                                            : "");
                            mObject.addProperty("b_manufacturing_date",
                                    mOpeningStocks.getManufacturingDate() != null
                                            ? mOpeningStocks.getManufacturingDate().toString()
                                            : "");
    ...
    public Object getLastproductData(HttpServletRequest request) {
        ...
        if (mProduct != null) {
            ...
            response.addProperty("subcategoryId",
                    mProduct.getSubcategory() != null ? mProduct.getSubcategory().getId() : null);
            ...
            response.addProperty("cgst", mProduct.getCgst() != null ? mProduct.getCgst() : null);
            response.addProperty("sgst", mProduct.getSgst() != null ? mProduct.getSgst() : null);
            // response.addProperty("minStock", mProduct.getMinStock() != null ?
            // mProduct.getMinStock() : 0.0);
            // response.addProperty("maxStock", mProduct.getMaxStock() != null ?
            // mProduct.getMaxStock() : 0.0);
    ...
    public Object getPurchaseRateProduct(HttpServletRequest request) {
        ...
        try {
            ...
            ProductUnitPacking mUnitPackaging = productUnitRepository.findRate(productId, levelAId, levelBId, levelCId,
                    unitId, true);
            JsonObject jsonObject = new JsonObject();
    ...
    public Object createProductNew(MultipartHttpServletRequest request) {
        ...
        try {
            ...
            if (paramMap.containsKey("drugType"))
                product.setDrugType(request.getParameter("drugType"));
            if (paramMap.containsKey("drug_contents"))
                product.setDrugContents(request.getParameter("drug_contents"));
            if (paramMap.containsKey("productType"))
                product.setProductType(request.getParameter("productType"));
            ...
            if (paramMap.containsKey("isMIS"))
                product.setIsMIS(Boolean.parseBoolean(request.getParameter("isMIS")));
            if (paramMap.containsKey("isPrescription"))
                product.setIsPrescription(Boolean.parseBoolean(request.getParameter("isPrescription")));
            // if (paramMap.containsKey("uploadImage"))
            // product.setUploadImage(request.getParameter("uploadImage"));
            ...
            if (paramMap.containsKey("subgroupId")) {
                mSubgroup = subgroupRepository.findByIdAndStatus(Long.parseLong(request.getParameter("subgroupId")),
                        true);
            ...
            if (paramMap.containsKey("subcategoryId")) {
                msubCategory = subcategoryRepository
                        .findByIdAndStatus(Long.parseLong(request.getParameter("subcategoryId")), true);
        ...
        try {
            ...
            for (JsonElement mList : array) {
                ...
                if (object.get("is_rate").getAsBoolean()) {
                    productUnitPacking.setFsrmh(object.get("rate_1").getAsString().isEmpty() ? 0.0 : object.get("rate_1").getAsDouble());
                    productUnitPacking.setFsrai(object.get("rate_2").getAsString().isEmpty() ? 0.0 : object.get("rate_2").getAsDouble());
                    productUnitPacking.setCsrmh(object.get("rate_3").getAsString().isEmpty() ? 0.0 : object.get("rate_3").getAsDouble());
                ...
                if (mBatchJsonArray.size() > 0) {
                    for (JsonElement mBatchElement : mBatchJsonArray) {
                        ...
                        if (mBatchJsonObject.get("isOpeningbatch").getAsBoolean()) {
                            ...
                            if (mBatchJsonObject.has("b_costing")
                                    && !mBatchJsonObject.get("b_costing").getAsString().isEmpty()) {
                            ...
                            mproductBatchNo
                                    .setQnty(Integer.parseInt(mBatchJsonObject.get("opening_qty").getAsString()));
                            ...
                            if (mBatchJsonObject.has("b_manufacturing_date") &&
                                    ...
                                    && !mBatchJsonObject.get("b_manufacturing_date").getAsString().toLowerCase()
                                    .contains("invalid"))
                                mproductBatchNo.setManufacturingDate(
                                        LocalDate.parse(mBatchJsonObject.get("b_manufacturing_date").getAsString()));
                            if (mBatchJsonObject.has("b_expiry") &&
                                    !mBatchJsonObject.get("b_expiry").getAsString().equalsIgnoreCase("")
                                    && !mBatchJsonObject.get("b_expiry").getAsString().toLowerCase()
                                    .contains("invalid"))
                                mproductBatchNo
                                        .setExpiryDate(LocalDate.parse(mBatchJsonObject.get("b_expiry").getAsString()));
                            mproductBatchNo.setStatus(true);
                            ...
                            List<ProductUnitPacking> mUnitPackaging = productUnitRepository
                                    .findByProductIdAndStatus(newProduct.getId(), true);
                            if (mUnitPackaging != null) {
                                if (mBatchJsonObject.has("b_costing")
                                        && !mBatchJsonObject.get("b_costing").getAsString().equalsIgnoreCase(""))
                                    costing = mBatchJsonObject.get("b_costing").getAsDouble();
                        ...
                        try {
                            ...
                            if (mBatchJsonObject.has("b_manufacturing_date")
                                    && !mBatchJsonObject.get("b_manufacturing_date").getAsString().equalsIgnoreCase(""))
                                newOpeningStock.setManufacturingDate(
                                        LocalDate.parse(mBatchJsonObject.get("b_manufacturing_date").getAsString()));
                            if (mBatchJsonObject.has("b_expiry")
                                    && !mBatchJsonObject.get("b_expiry").getAsString().equalsIgnoreCase(""))
                                newOpeningStock
    ...
    public void createProductForFranchise(MultipartHttpServletRequest request) {
        try {
            ...
            if (paramMap.containsKey("productCode"))
                body.add("productCode", request.getParameter("productCode"));
            if (paramMap.containsKey("productDescription"))
                body.add("productDescription", request.getParameter("productDescription"));
            body.add("status", true);
            if (paramMap.containsKey("barcodeNo"))
                body.add("barcodeNo", request.getParameter("barcodeNo"));
            if (paramMap.containsKey("isSerialNo"))
            ...
            if (paramMap.containsKey("drugType"))
                body.add("drugType", request.getParameter("drugType"));
            if (paramMap.containsKey("drug_contents"))
                body.add("drug_contents", request.getParameter("drug_contents"));
            if (paramMap.containsKey("productType"))
                body.add("productType", request.getParameter("productType"));
            if (paramMap.containsKey("isGroup"))
            ...
            if (paramMap.containsKey("isMIS"))
                body.add("isMIS", Boolean.parseBoolean(request.getParameter("isMIS")));
            if (paramMap.containsKey("isPrescription"))
            ...
            /**** Modification after PK visits at Solapur 25th to 30th January 2023 ******/
            if (paramMap.containsKey("shelfId"))
                body.add("shelfId", request.getParameter("shelfId"));
            if (paramMap.containsKey("barcodeSaleQuantity"))
            ...
            if (paramMap.containsKey("weight"))
                body.add("weight", Double.parseDouble(request.getParameter("weight")));
            if (paramMap.containsKey("weightUnit"))
                body.add("weightUnit", request.getParameter("weightUnit"));
            if (paramMap.containsKey("disPer1"))
            ...
            if (paramMap.containsKey("taxType"))
                body.add("taxType", request.getParameter("taxType"));
            if (paramMap.containsKey("igst"))
                body.add("igst", Double.parseDouble(request.getParameter("igst")));
            if (paramMap.containsKey("cgst"))
                body.add("cgst", Double.parseDouble(request.getParameter("cgst")));
            if (paramMap.containsKey("sgst"))
                body.add("sgst", Double.parseDouble(request.getParameter("sgst")));
            if (paramMap.containsKey("minStock"))
            ...
            for (JsonElement mList : array) {
                ...
                if (object.has("conv"))
                    productRow.addProperty("conv", object.get("conv").getAsDouble());
                if (object.has("unit_marg"))
                    productRow.addProperty("unit_marg", object.get("unit_marg").getAsDouble());
                if (object.has("is_negetive"))
                    productRow.addProperty("is_negetive", object.get("is_negetive").getAsBoolean());
                if (object.has("mrp"))
                    productRow.addProperty("mrp", object.get("mrp").getAsDouble());
                if (object.has("pur_rate"))
                    productRow.addProperty("pur_rate", object.get("pur_rate").getAsDouble());
                if (object.has("min_margin"))
                ...
                if (object.has("rate_1")) {
                    ...
                    productRow.addProperty("fsr", object.get("rate_1").getAsDouble());
                }
                if (object.has("rate_2")) {
                    productRow.addProperty("rate_2", object.get("rate_2").getAsDouble());// csr
                    productRow.addProperty("csr", object.get("rate_2").getAsDouble());
                }
                if (object.has("rate_3"))
                    productRow.addProperty("rate_3", object.get("rate_3").getAsDouble());
                if (object.has("min_qty"))
                    productRow.addProperty("min_qty", object.get("min_qty").getAsDouble());
                if (object.has("max_qty"))
                    productRow.addProperty("max_qty", object.get("max_qty").getAsDouble());
            ...
            if (franchiseMasters != null) {
                ...
                for (FranchiseMaster franchiseMaster : franchiseMasters) {
                    ...
                    System.out.println("url " + frUrl + "/create_product_from_gv");
    ...
    public Object importProduct(MultipartHttpServletRequest request) {
        ...
        try {
            ...
            if (excelFile != null) {
                ...
                int productCode = 0, productName = 1, pkg = 2, brandName = 3, salesTax = 4, hsn = 5, discount = 6,
                        margin = 7, shelfId = 8, misMfg = 9, formation = 10, misCategory = 11, misSubCategory = 12,
                        minQty = 13, maxQty = 14, isBatchRequired = 13, drug = 21, mrp = 35, csrMh = 36, csrAi = 37,
                        fsrMh = 38, fsrAi = 39, purchaseTax = 4, isPrescription = 17, isVaccine = 41, isActive = 42,
                        isDeleted = 43, inStock = 44, scheduledH = 18, scheduledH1 = 19, narcotic = 20, commission = 22,
                        mainCategoryName = 11, subCategoryName = 12, lowestUnitName = 27, lowestUnitConversion = 28,
                        mediumUnitName = 29, mediumUnitConversion = 30, highestUnitName = 31,
                        highestUnitConversion = 32, isGVProduct = 23, gvProductType = 24;
                ...
                for (int s = 1; s < records; s++) {
                    ...
                    product = productRepository.findFirstByProductCodeAndProductNameAndOutletId(productcode,
                            productname, outlet.getId());
                    ...
                    if (product == null) {
                        ...
                        // productLogger.error("Before TaxMaster Find Execution... " + sdf.format(new
                        // Date()));
                        // TaxMaster taxMaster =
                        // taxMasterRepository.findDuplicateGSTWithOutlet(outlet.getId(),
                        // Integer.parseInt(tax+"")+"", true);
                        // productLogger.error("After TaxMaster Find Execution... " + sdf.format(new
                        // Date()));
                        TaxMaster taxMaster = null;
                        if (branch != null)
                            taxMaster = taxMasterRepository.findDuplicateGSTWithBranch(outlet.getId(), branch.getId(),
                                    (decimalFormat.format(tax)), true);
                        else
                            taxMaster = taxMasterRepository.findDuplicateGSTWithOutlet(outlet.getId(),
                                    (decimalFormat.format(tax)), true);
                         ...
                         *
                         * contentMaster =
                         * contentMasterRepository.findByContentNameAndStatus(mContents[0], true);
                         * // productLogger.error("After ProductContentMaster find Execution... " +
                         * sdf.format(new Date()));
                         ...
                         * productContentMaster.setBranchId(users.getBranch()!=null?users.getBranch().
                         * getId():null);
                        ...
                        productLogger.error("Product : " + row.getCell(productName).toString() + " Saved Successfully!"
                                + sdf.format(new Date()));
                        ...
                        if (highunitname != null && !highunitname.isEmpty()) {
                            ...
                            if (unit == null) {
                                ...
                                productLogger.error(
                                        "New unit : " + highunitname + " found and saved!" + sdf.format(new Date()));
                            }
                            ProductUnitPacking productUnitPacking = new ProductUnitPacking();
                            productUnitPacking.setUnits(unit);
                            ...
                            productUnitRepository.save(productUnitPacking);
                            productLogger
                                    .error("After Successfully Saving productUnitPacking  " + sdf.format(new Date()));
                            productLogger.error(s + "Product Unit : " + highunitname + " Saved Successfully!"
                                    + sdf.format(new Date()));
                        ...
                        if (mediumunitname != null && !mediumunitname.isEmpty()) {
                            ...
                            if (unit == null) {
                                ...
                                productLogger.error("After Successfully Saving Units  " + sdf.format(new Date()));
                                productLogger.error(
                                        "New unit : " + mediumunitname + " found and saved!" + sdf.format(new Date()));
                            }
                            ProductUnitPacking productUnitPacking = new ProductUnitPacking();
                            productUnitPacking.setUnits(unit);
                            ...
                            productLogger
                                    .error("After Successfully Saving productUnitPacking  " + sdf.format(new Date()));
                            productLogger.error(s + "Product Unit : " + mediumunitname + " Saved Successfully!"
                                    + sdf.format(new Date()));
                        ...
                        if (lowestunitname != null && !lowestunitname.isEmpty()) {
                            ...
                            if (unit == null) {
                                ...
                                productLogger.error("After Successfully Saving Units  " + sdf.format(new Date()));
                                productLogger.error(
                                        "New unit : " + lowestunitname + " found and saved!" + sdf.format(new Date()));
                            }
                            ProductUnitPacking productUnitPacking = new ProductUnitPacking();
                            productUnitPacking.setUnits(unit);
                            ...
                            productLogger
                                    .error("After Successfully Saving productUnitPacking  " + sdf.format(new Date()));
                            productLogger.error(s + "Product Unit : " + lowestunitname + " Saved Successfully!"
                                    + sdf.format(new Date()));
        ...
        try {
            ...
            if (excelFile != null) {
                ...
                for (int s = 1; s < sheet.getPhysicalNumberOfRows(); s++) {
                    ...
                    product = productRepository.findFirstByProductCodeAndProductNameAndOutletId(pcode,
                            row.getCell(1).toString(), outlet.getId());
                    ...
                    if (product == null) {
                        ...
                        productLogger.error("Before Packing Find Execution... " + sdf.format(new Date()));
                        mPackingMaster = packingMasterRepository
                                .findFirstByPackNameIgnoreCase(row.getCell(2).toString());
                        productLogger.error("After Packing Find Execution... " + sdf.format(new Date()));
                        if (mPackingMaster == null) {
                            mPackingMaster = new PackingMaster();
                        ...
                        TaxMaster taxMaster = taxMasterRepository.findDuplicateGSTWithOutlet(outlet.getId(), igst,
                                true);
                         ...
                         *
                         * if (branch != null)
                         * taxMaster = taxMasterRepository.findDuplicateGSTWithBranch(outlet.getId(),
                         * branch.getId(), row.getCell(12).toString(), true);
                         * else
                         * taxMaster = taxMasterRepository.findDuplicateGSTWithOutlet(outlet.getId(),
                         * row.getCell(12).toString(), true);
                         *
                         * if (taxMaster == null) {
                         * taxMaster = new TaxMaster();
                        ...
                        for (String content : contents) {
                            ...
                            productContentMaster
                                    .setBranchId(users.getBranch() != null ? users.getBranch().getId() : null);
                            productContentMasterRepository.save(productContentMaster);

                        }

                        productLogger.error("After Successfully Saving Product " + sdf.format(new Date()));
                        productLogger.error("Product : " + row.getCell(1).toString() + " Saved Successfully!"
                                + sdf.format(new Date()));
                        ...
                        if (unit == null) {
                            ...
                            unitsRepository.save(unit);
                            productLogger.error("After Successfully Saving Units  " + sdf.format(new Date()));
                            productLogger.error("New unit : " + row.getCell(4).toString() + " found and saved!"
                                    + sdf.format(new Date()));
                        }
                        ProductUnitPacking productUnitPacking = new ProductUnitPacking();
                        productUnitPacking.setUnits(unit);
                        ...
                        productLogger.error(s + "Product Unit : " + row.getCell(4).toString() + " Saved Successfully!"
                                + sdf.format(new Date()));
        ...
        try {
            ...
            if (excelFile != null) {
                ...
                for (int s = 1; s < sheet.getPhysicalNumberOfRows(); s++) {
                    ...
                    // product =
                    // productRepository.findFirstByProductCodeAndProductNameAndOutletId(row.getCell(0).getStringCellValue(),
                    // row.getCell(1).getStringCellValue(), outlet.getId());
    ...
    public Object importProductStock(MultipartHttpServletRequest request) {
        ...
        try {
            ...
            if (excelFile != null) {
                XSSFWorkbook workbook = new XSSFWorkbook(excelFile.getInputStream());
                XSSFSheet sheet = workbook.getSheetAt(0);
                int productCodeIndex = 0, productNameIndex = 1, currentStockIndex = 2, stockUnitIndex = 3,
                        costingPriceIndex = 4, mrpRateIndex = 5, fsrMhIndex = 7, fsrAiIndex = 8, csrMhIndex = 9,
                        csrAiIndex = 10, batchNoIndex = 12, mfgIndex = 13, expIndex = 14, purchasePriceIndex = 6,
                        salesPriceIndex = 11;
                for (int s = 1; s < sheet.getPhysicalNumberOfRows(); s++) {
                        ...
                        if (batchNo.isEmpty()) {
                            ...
                            if (!mfgDate.isEmpty())
                                productBatchNo.setManufacturingDate(
                                        LocalDate.parse(mfgDate, DateTimeFormatter.ofPattern("dd/MM/yyyy")));
                            if (!expDate.isEmpty())
                                productBatchNo.setExpiryDate(
                                        LocalDate.parse(expDate, DateTimeFormatter.ofPattern("dd/MM/yyyy")));
                            ...
                            List<ProductUnitPacking> unit = productUnitRepository
                                    .findByProductIdAndStatus(product.getId(), true);
                            for (ProductUnitPacking pup : unit) {
    ...
    public JsonObject getProductByIdEditNew(HttpServletRequest request) {
        ...
        if (mProduct != null) {
            ...
            response.addProperty("isFormulation",
                    mProduct.getIsFormulation() != null ? mProduct.getIsFormulation() : false);
            response.addProperty("isCategory", mProduct.getIsCategory() != null ? mProduct.getIsCategory() : false);
            response.addProperty("isSubcategory",
                    mProduct.getIsSubCategory() != null ? mProduct.getIsSubCategory() : false);
            response.addProperty("isPrescription",
                    mProduct.getIsPrescription() != null ? mProduct.getIsPrescription() : false);
            ...
            response.addProperty("ecomType",
                    mProduct.getEcommerceTypeId() != null ? mProduct.getEcommerceTypeId().toString() : "");
            response.addProperty("ecomPrice",
                    mProduct.getSellingPrice() != null ? mProduct.getSellingPrice().toString() : "");
            response.addProperty("ecomDiscount",
                    mProduct.getDiscountPer() != null ? mProduct.getDiscountPer().toString() : "");
            ...
            /*
             * response.addProperty("fsrmh", mProduct.getFsrmh() != null ?
             * mProduct.getFsrmh().toString() : "");
             * response.addProperty("fsrai", mProduct.getFsrai() != null ?
             * mProduct.getFsrai().toString() : "");
             * response.addProperty("csrmh", mProduct.getCsrmh() != null ?
             * mProduct.getCsrmh().toString() : "");
             * response.addProperty("csrai", mProduct.getCsrai() != null ?
             * mProduct.getCsrai().toString() : "");
             */
            response.addProperty("imageExists", false);
            if (mProduct.getImage1() != null || mProduct.getImage2() != null || mProduct.getImage3() != null
                    || mProduct.getImage4() != null
            ...
            List<ProductContentMaster> bankList = productContentMasterRepository
                    .findByProductIdAndStatus(mProduct.getId(), true);
            ;
    ...
    private JsonArray getUnitRowsNewProductEdit(Long id) {
        ...
        for (ProductUnitPacking munitPacking : mUnits) {
            if (munitPacking.getStatus() == true) {
                ...
                mObject.addProperty("selectedLevelA",
                        munitPacking.getLevelA() != null ? munitPacking.getLevelA().getId().toString() : "");
                mObject.addProperty("selectedLevelB",
                        munitPacking.getLevelB() != null ? munitPacking.getLevelB().getId().toString() : "");
                mObject.addProperty("selectedLevelB",
                        munitPacking.getLevelB() != null ? munitPacking.getLevelB().getId().toString() : "");
                mObject.addProperty("selectedLevelC",
                        munitPacking.getLevelC() != null ? munitPacking.getLevelC().getId().toString() : "");
                mObject.addProperty("selectedUnit",
                        munitPacking.getUnits() != null ? munitPacking.getUnits().getId().toString() : "");
                mObject.addProperty("conv",
                        munitPacking.getUnitConversion() != null ? munitPacking.getUnitConversion() : 0);
                mObject.addProperty("unit_marg",
                        munitPacking.getUnitConvMargn() != null ? munitPacking.getUnitConvMargn() : 0);
                mObject.addProperty("mrp", munitPacking.getMrp() != null ? munitPacking.getMrp() : 0);
                mObject.addProperty("pur_rate",
                        munitPacking.getPurchaseRate() != null ? munitPacking.getPurchaseRate() : 0);
                // mObject.addProperty("min_margin", munitPacking.getMinMargin());
                /*
                 * mObject.addProperty("rate_1", munitPacking.getMinRateA() != null ?
                 * munitPacking.getMinRateA() : 0);
                 * mObject.addProperty("rate_2", munitPacking.getMinRateB() != null ?
                 * munitPacking.getMinRateB() : 0);
                 * mObject.addProperty("rate_3", munitPacking.getMinRateC() != null ?
                 * munitPacking.getMinRateC() : 0);
                 * mObject.addProperty("rate_3", munitPacking.getMinRateC() != null ?
                 * munitPacking.getMinRateC() : 0);
                 */
                ...
                mObject.addProperty("is_negetive",
                        munitPacking.getIsNegativeStocks() != null ? munitPacking.getIsNegativeStocks() : false);
                ...
                List<ProductOpeningStocks> openingStocks = openingStocksRepository.findByProductOpening(id,
                        munitPacking.getUnits().getId(), levelaUnit, levelbUnit, levelcUnit);
                for (ProductOpeningStocks mOpeningStocks : openingStocks) {
                    JsonObject mBatchObject = new JsonObject();
                    mBatchObject.addProperty("id", mOpeningStocks.getId());
                    mBatchObject.addProperty("b_no",
                            mOpeningStocks.getProductBatchNo() != null ? mOpeningStocks.getProductBatchNo().getBatchNo()
                                    : "");
                    mBatchObject.addProperty("batch_id",
                            mOpeningStocks.getProductBatchNo() != null
                                    ? mOpeningStocks.getProductBatchNo().getId().toString()
                                    : "");
                    mBatchObject.addProperty("opening_qty", mOpeningStocks.getOpeningStocks());
                    mBatchObject.addProperty("b_free_qty",
                            mOpeningStocks.getFreeOpeningQty() != null ? mOpeningStocks.getFreeOpeningQty().toString()
                                    : "");
                    mBatchObject.addProperty("b_mrp",
                            mOpeningStocks.getMrp() != null ? mOpeningStocks.getMrp().toString() : "");
                    mBatchObject.addProperty("b_sale_rate",
                            mOpeningStocks.getSalesRate() != null ? mOpeningStocks.getSalesRate().toString() : "");
                    mBatchObject.addProperty("b_purchase_rate",
                            mOpeningStocks.getPurchaseRate() != null ? mOpeningStocks.getPurchaseRate().toString()
                                    : "");
                    mBatchObject.addProperty("b_costing", mOpeningStocks.getCosting());
                    mBatchObject.addProperty("b_expiry",
                            mOpeningStocks.getExpiryDate() != null ? mOpeningStocks.getExpiryDate().toString() : "");
                    mBatchObject.addProperty("b_manufacturing_date",
                            mOpeningStocks.getManufacturingDate() != null
                                    ? mOpeningStocks.getManufacturingDate().toString()
                                    : "");
    ...
    public Object updateProduct_new(MultipartHttpServletRequest request) {
        ...
        /*
         * if (paramMap.containsKey("fsrmh"))
         * product.setFsrmh(Double.parseDouble(request.getParameter("fsrmh")));
         * if (paramMap.containsKey("fsrai"))
         * product.setFsrai(Double.parseDouble(request.getParameter("fsrai")));
         * if (paramMap.containsKey("csrmh"))
         * product.setCsrmh(Double.parseDouble(request.getParameter("csrmh")));
         * if (paramMap.containsKey("csrai"))
         * product.setCsrai(Double.parseDouble(request.getParameter("csrai")));
         */
        /**** END ****/
        ...
        try {
            ...
            for (JsonElement mList : array) {
                ...
                double minQty = minQtyElement != null && !minQtyElement.isJsonNull() ? minQtyElement.getAsDouble()
                        : 0.0;
                productUnitPacking.setMinQty(minQty);

                JsonElement maxQtyElement = object.get("max_qty");
                double maxQty = maxQtyElement != null && !maxQtyElement.isJsonNull() ? maxQtyElement.getAsDouble()
                        : 0.0;
                    ...
                    for (JsonElement mBatchElement : mBatchJsonArray) {
                        ...
                        if (mBatchJsonObject.get("isOpeningbatch").getAsBoolean()) {
                            ...
                            if (batch_id.equalsIgnoreCase("") && id == 0) {
                                ...
                                mproductBatchNo = productBatchNoRepository.findByIdAndStatus(Long.parseLong(batch_id),
                                        true);
                            if (mBatchJsonObject.has("b_no"))
                                mproductBatchNo.setBatchNo(mBatchJsonObject.get("b_no").getAsString());
    ...
    public Object productStockList() {
        ...
        String searchKey = "";
        String barcodeKey = "";
        String query = "SELECT * FROM `product_tbl` LEFT JOIN packing_master_tbl ON product_tbl.packing_master_id=packing_master_tbl.id"
                + " WHERE product_tbl.status=1";

        if (!searchKey.equalsIgnoreCase("")) {
            query = query + " AND (product_name LIKE '%" + searchKey + "%' OR product_code LIKE '%" + searchKey
                    + "%' OR barcode_no LIKE '%" + searchKey + "%' OR  packing_master_tbl.pack_name LIKE '%" + searchKey
                    + "%')";
        }
        ...
        if (productDetails != null && productDetails.size() > 0) {
            for (Product mDetails : productDetails) {
                ...
                mObject.addProperty("product_category",
                        mDetails.getCategory() != null ? mDetails.getCategory().getCategoryName() : "");
                if (mDetails.getIsBatchNumber()) {
                    ...
                    if (tranxPurInvoiceDetailsUnits != null) {
                        ...
                        mObject.addProperty("purchaserate",
                                tranxPurInvoiceDetailsUnits.getProductBatchNo() != null
                                        ...
                                        : 0.0);
                        Double closingStocks = inventoryCommonPostings.getmobileClosingStockProduct(mDetails.getId(),
                                tranxPurInvoiceDetailsUnits.getPurchaseTransaction().getFiscalYear());
                        mObject.addProperty("current_stock", closingStocks > 0 ? closingStocks.toString() : "0.0");
                        if (productBatchNo != null) {
                            mObject.addProperty("costing",
                                    productBatchNo.getCosting() != null ? productBatchNo.getCosting() : 0.0);
                        } else {
                    ...
                    if (productUnitPacking != null && productUnitPacking.size() > 0) {
                        ...
                        mObject.addProperty("costing",
                                productUnitPacking.get(0).getCosting() != null ? productUnitPacking.get(0).getCosting()
                                        : 0.0);
                    } else {
                        mObject.addProperty("mrp", 0.0);
                        mObject.addProperty("purchaserate", 0.0);
    ...
    public JsonObject validateProductCode(HttpServletRequest request) {
        ...
        try {
            ...
            if (users.getBranch() != null)
                product = productRepository.findByduplicateProductWithBranch(users.getOutlet().getId(),
                        users.getBranch().getId(), productCode, true);
            else {
    ...
    public JsonObject validateProductCodeUpdate(HttpServletRequest request) {
        ...
        try {
            ...
            if (users.getBranch() != null) {
                product = productRepository.findByProductCodeAndStatusAndOutletIdAndBranchId(productCode, true,
                        users.getOutlet().getId(), users.getBranch().getId());
            } else {
                product = productRepository.findByProductCodeAndStatusAndOutletIdAndBranchIsNull(productCode, true,
                        users.getOutlet().getId());
            }
    ...
    public JsonObject validateProductUpdate(HttpServletRequest request) {
        ...
        try {
            ...
            if (users.getBranch() != null) {
                if (packageId != null) {
                    product = productRepository.findByProductNameAndPackingMasterIdAndOutletIdAndBranchIdAndStatus(
                            productName, packageId, users.getOutlet().getId(), users.getBranch().getId(), true);
                } else {
                    product = productRepository.findByProductNameAndOutletIdAndBranchIdAndStatus(productName,
                            users.getOutlet().getId(), users.getBranch().getId(), true);
                }
            } else {
                if (packageId != null) {
                    product = productRepository.findByProductNameAndPackingMasterIdAndOutletIdAndStatusAndBranchIsNull(
                            productName, packageId, users.getOutlet().getId(), true);
                } else {
                    product = productRepository.findByProductNameAndOutletIdAndStatusAndBranchIsNull(productName,
                            users.getOutlet().getId(), true);
                }
            }
    ...
    public JsonObject productTransactionList(HttpServletRequest request) {
        ...
        if (paramMap.containsKey("barcode"))
            barcodeKey = request.getParameter("barcode");

        String query = "SELECT * FROM `product_tbl` LEFT JOIN packing_master_tbl ON "
                + "product_tbl.packing_master_id=packing_master_tbl.id" + " " + "WHERE product_tbl.outlet_id="
            ...
            for (Product mDetails : productDetails) {
                ...
                if (productUnitPacking != null && productUnitPacking.size() > 0) {
                    ...
                    mObject.addProperty("is_negative",
                            productUnitPacking.get(0).getIsNegativeStocks() != null
                                    ? productUnitPacking.get(0).getIsNegativeStocks()
                                    : false);
                ...
                Double closingStocks = inventoryCommonPostings.getClosingStockProduct(mDetails.getId(),
                        users.getOutlet().getId(), mDetails.getBranch() != null ? mDetails.getBranch().getId() : null,
                        fiscalYear);
    ...
    /***
     * Product , Barcode and Company Barcode Search functionality of Product
     * selection in Tranx Perticular
     *****/
    ...
    public JsonObject deleteProduct(HttpServletRequest request) {
        ...
        if (mProduct != null) {
            jsonObject.addProperty("message", "Product deleted successfully");
            jsonObject.addProperty("responseStatus", HttpStatus.OK.value());
            if (!source.equalsIgnoreCase("product_edit"))
                mProduct.setStatus(false);
    ...
    public Object GVProductList(Map<String, String> req, HttpServletRequest request) {
        ...
        gvHdr.setContentType(MediaType.APPLICATION_JSON);
...
//        gvBody.add("pageNo", req.get("pageNo"));
//        gvBody.add("pageSize", req.get("pageSize"));
//        gvBody.add("searchText", req.get("searchText"));
//        gvBody.add("barcode", req.get("barcode"));
        JsonObject gvBody = new JsonObject();
        gvBody.addProperty("pageNo", req.get("pageNo"));
        gvBody.addProperty("pageSize", req.get("pageSize"));
        gvBody.addProperty("searchText", req.get("searchText"));
        gvBody.addProperty("barcode", req.get("barcode"));

        String gvData = "";
        HttpEntity gvEntity = new HttpEntity<>(gvBody.toString(), gvHdr);
        System.out.println("req" + req.toString());
        try {
            System.out.println("Req url => " + frUrl + "/mobile/v1/gv-live-prd-list");
            ResponseEntity<String> responseEntity = restTemplate.exchange(
                    frUrl + "/mobile/v1/gv-live-prd-list", HttpMethod.POST, gvEntity, String.class);
            gvData = responseEntity.getBody();
    ...
    private FRProductDTO convertProductToFRDTD(Product mDetails) {
        ...
        ProductBatchNo productBatchNo = productBatchNoRepository
                .findTop1ByProductIdAndStatusOrderByIdDesc(mDetails.getId(), true);
        List<ProductUnitPacking> productUnitPacking = productUnitRepository.findByProductIdAndStatus(mDetails.getId(),
                true);
        if (productBatchNo != null) {
            if (productBatchNo.getExpiryDate() != null) {
        ...
        if (productUnitPacking != null && productUnitPacking.size() > 0) {
            frProductDTO.setUnit(
                    productUnitPacking.get(0).getUnits() != null ? productUnitPacking.get(0).getUnits().getUnitName()
                            : "PCS");
            frProductDTO.setUnit_id(
                    productUnitPacking.get(0).getUnits() != null ? productUnitPacking.get(0).getUnits().getId() : 1L);
            frProductDTO.setIs_negative(productUnitPacking.get(0).getIsNegativeStocks() != null
                    ? productUnitPacking.get(0).getIsNegativeStocks()
                    : false);
        } else {
        ...
        if (mDetails.getIsBatchNumber()) {
            ...
            TranxPurInvoiceDetailsUnits tranxPurInvoiceDetailsUnits = tranxPurInvoiceDetailsUnitsRepository
                    .findTop1ByProductIdOrderByIdDesc(mDetails.getId());
            if (tranxPurInvoiceDetailsUnits != null) {
                frProductDTO.setMrp(tranxPurInvoiceDetailsUnits.getProductBatchNo() != null
                        ? tranxPurInvoiceDetailsUnits.getProductBatchNo().getMrp()
                        : 0.00);
                frProductDTO.setSales_rate(tranxPurInvoiceDetailsUnits.getProductBatchNo() != null
                        && tranxPurInvoiceDetailsUnits.getProductBatchNo().getSalesRate() != null
                        ? tranxPurInvoiceDetailsUnits.getProductBatchNo().getSalesRate()
                        : 0.00);
                frProductDTO.setPurchaserate(tranxPurInvoiceDetailsUnits.getProductBatchNo() != null
                        && tranxPurInvoiceDetailsUnits.getProductBatchNo().getPurchaseRate() != null
                        ? tranxPurInvoiceDetailsUnits.getProductBatchNo().getPurchaseRate()
                        : 0.00);
            }
        ...
        Double productOpeningStocks = openingStocksRepository.findSumProductOpeningStocks(mDetails.getId(),
                mDetails.getOutlet().getId(), mDetails.getBranch() != null ? mDetails.getBranch().getId() : null,
                fiscalYear.getId());
        Double closingStocks = inventoryCommonPostings.getClosingStockProduct(mDetails.getId(),
                mDetails.getOutlet().getId(), mDetails.getBranch() != null ? mDetails.getBranch().getId() : null,
                fiscalYear);
    ...
    public Object getProductListByBarcode(HttpServletRequest request) {
        Map<String, String[]> paramMap = request.getParameterMap();
        JsonObject response = new JsonObject();
        JsonArray result = new JsonArray();
        List productDetails = new ArrayList<>();
        ...
        try {
            if (paramMap.containsKey("search"))
                searchKey = request.getParameter("search");
            String query = "SELECT product_tbl.id FROM `product_tbl` LEFT JOIN product_barcode_tbl ON " +
                    "product_tbl.id=product_barcode_tbl.product_id WHERE product_tbl.outlet_id=" + users.getOutlet().getId() +
                    " AND product_tbl.status=1";
            ...
            if (!searchKey.equalsIgnoreCase("")) {
                query = query + " AND (product_barcode_tbl.barcode_unique_code = '" + searchKey + "' OR " +
                        "product_barcode_tbl.company_barcode = '" + searchKey + "')";
            }
            ...
            if (productDetails != null && productDetails.size() > 0) {
                for (Object mList : productDetails) {
                    Product mDetails = productRepository.findByIdAndStatus(Long.parseLong(mList.toString()), true);
                    List<ProductBatchNo> productBatchNoList = productBatchNoRepository
                            .findBatchDataByBarcode(mDetails.getId(),searchKey, true);
                    List<ProductUnitPacking> productUnitPacking = productUnitRepository
                            .findByProductIdAndStatus(mDetails.getId(), true);
                    JsonObject mObject = new JsonObject();

                    JsonArray batchArray=new JsonArray();
                    if (productBatchNoList != null) {
                        for (ProductBatchNo productBatchNo :
                                productBatchNoList) {
                            JsonObject batchObject = new JsonObject();
                            if (productBatchNo.getExpiryDate() != null) {
                                batchObject.addProperty("batch_expiry", productBatchNo.getExpiryDate().toString());
                            } else {
                                batchObject.addProperty("batch_expiry", "");
                            }
                            batchObject.addProperty("mrp", productBatchNo.getMrp());
                            batchObject.addProperty("sales_rate", productBatchNo.getSalesRate());
                            batchObject.addProperty("batch_no",productBatchNo.getBatchNo());
                            batchObject.addProperty("batch_id",productBatchNo.getId());
                    ...
                    if (productUnitPacking != null && productUnitPacking.size() > 0) {
                        ...
                        for (ProductUnitPacking unitPacking :
                            ...
                            if(unitPacking.getIsNegativeStocks()){
                                isNegetive=true;
                            }
                            unitArray.add(jsonObject);
                        }
                        mObject.add("unit_data",unitArray);
                ...
                for (Object mList : productDetails) {
                    ...
                    if (mDetails.getIsBatchNumber()) {
                        ...
                        if (tranxPurInvoiceDetailsUnits != null) {
                            mObject.addProperty("mrp",
                                    tranxPurInvoiceDetailsUnits.getProductBatchNo() != null
                                            ? tranxPurInvoiceDetailsUnits.getProductBatchNo().getMrp()!=null ?tranxPurInvoiceDetailsUnits.getProductBatchNo().getMrp():0.0
                                            : 0.00);
                            mObject.addProperty("sales_rate",
                                    tranxPurInvoiceDetailsUnits.getProductBatchNo() != null
                                            && tranxPurInvoiceDetailsUnits.getProductBatchNo().getSalesRate() != null
    ...
    public Object createProductFromGv(MultipartHttpServletRequest request) {
        Product product = new Product();
        Product newProduct = new Product();
        Map<String, String[]> paramMap = request.getParameterMap();
        ResponseMessage responseObject = new ResponseMessage();
        FileStorageProperties fileStorageProperties = new FileStorageProperties();
//        Users users = usersRepository.findTop1ByUserRoleIgnoreCaseAndCompanyCode("cadmin",request.getHeader("branch"));
        Branch branch = null;
        ...
        try {
           ...
           branch = users.getBranch();
            product.setBranch(branch);*/
            String outletCode = request.getHeader("branch");
            Outlet outlet = outletRepository.findByCompanyCodeIgnoreCase(outletCode);
            product.setOutlet(outlet);
            product.setProductName(request.getParameter("productName").trim());
...
//            product.setCreatedBy(users.getId());
            ...
            if (paramMap.containsKey("brandName")) {
                mBrandMaster = brandRepository.findFirstByBrandName(request.getParameter("brandName"));
                if (mBrandMaster == null) {

                    mBrandMaster = new Brand();
                    mBrandMaster.setBrandName(request.getParameter("brandName"));
//                    mBrandMaster.setBranch(branch);
                    mBrandMaster.setOutlet(outlet);
                    mBrandMaster.setStatus(true);
//                    mBrandMaster.setCreatedBy(users.getId());
                    mBrandMaster = brandRepository.save(mBrandMaster);
                }
                product.setBrand(mBrandMaster);
            }
            if (paramMap.containsKey("packingMaster")) {
                mPackingMaster = packingMasterRepository.findFirstByPackNameIgnoreCase(request.getParameter("packingMaster"));
                if (mPackingMaster == null) {
                    mPackingMaster = new PackingMaster();
                    mPackingMaster.setPackName(request.getParameter("packingMaster"));
//                    mPackingMaster.setBranch(branch);
                    mPackingMaster.setOutlet(outlet);
                    mPackingMaster.setStatus(true);
//                    mPackingMaster.setCreatedBy(users.getId());
            ...
            if (paramMap.containsKey("groupName")) {
                mGroupMaster = groupRepository.findFirstByGroupNameIgnoreCase(request.getParameter("groupName"));
                if (mGroupMaster == null) {
                    mGroupMaster = new Group();
                    mGroupMaster.setGroupName(request.getParameter("groupName"));
//                    mGroupMaster.setBranch(branch);
                    mGroupMaster.setOutlet(outlet);
                    mGroupMaster.setStatus(true);
//                    mGroupMaster.setCreatedBy(users.getId());
            ...
            if (paramMap.containsKey("subGroupName")) {
                mSubgroup = subgroupRepository.findFirstBySubgroupNameIgnoreCase(request.getParameter("subGroupName"));
                if (mSubgroup == null) {
                    mSubgroup = new Subgroup();
                    mSubgroup.setSubgroupName(request.getParameter("subGroupName"));
//                    mSubgroup.setBranch(branch);
                    mSubgroup.setOutlet(outlet);
                    mSubgroup.setStatus(true);
//                    mSubgroup.setCreatedBy(users.getId());
            ...
            if (paramMap.containsKey("categoryName")) {
                mCategoryMaster = categoryRepository.findFirstByCategoryNameIgnoreCase(request.getParameter("categoryName"));
                if (mCategoryMaster == null) {
                    mCategoryMaster = new Category();
                    mCategoryMaster.setCategoryName(request.getParameter("categoryName"));
//                    mCategoryMaster.setBranch(branch);
                    mCategoryMaster.setOutlet(outlet);
                    mCategoryMaster.setStatus(true);
//                    mCategoryMaster.setCreatedBy(users.getId());
                    categoryRepository.save(mCategoryMaster);
            ...
            if (paramMap.containsKey("subcategoryName")) {
                msubCategory = subcategoryRepository.findFirstBySubcategoryNameIgnoreCase(request.getParameter("subcategoryName"));
                if (msubCategory == null) {
                    msubCategory = new Subcategory();
                    msubCategory.setSubcategoryName(request.getParameter("subcategoryName"));
//                    msubCategory.setBranch(branch);
                    msubCategory.setOutlet(outlet);
                    msubCategory.setStatus(true);
//                    msubCategory.setCreatedBy(users.getId());
            ...
            if (paramMap.containsKey("taxPercentage")) {
                LocalDate applicableDate = null;
                TaxMaster taxMaster = taxMasterRepository.findDuplicateGSTWithOutlet(outlet.getId(), request.getParameter("taxPercentage"), true);
                if (taxMaster == null) {
                    Double tx = Double.parseDouble(request.getParameter("taxPercentage"));
                    taxMaster = new TaxMaster();
                    taxMaster.setGst_per(tx + "");
                    taxMaster.setIgst(tx);
                    taxMaster.setCgst(tx / 2);
                    taxMaster.setSgst(tx / 2);
                    taxMaster.setSratio(Double.parseDouble("50"));
                    taxMaster.setCreatedBy(outlet.getId());
                    taxMaster.setCreatedAt(LocalDateTime.now());
                    taxMaster.setStatus(true);
...
*/
                    product.setIgst(tx);
                    product.setSgst(tx / 2);
                    product.setCgst(tx / 2);
            ...
            if (paramMap.containsKey("hsnNo")) {
                ProductHsn productHsn = productHsnRepository.findByHsnNumber(request.getParameter("hsnNo"));
                if (productHsn == null) {
                    productHsn = new ProductHsn();
                    productHsn.setHsnNumber(request.getParameter("hsnNo"));
                    productHsn.setIgst(Double.parseDouble(request.getParameter("igst")));
                    productHsn.setSgst(Double.parseDouble(request.getParameter("sgst")));
                    productHsn.setCgst(Double.parseDouble(request.getParameter("cgst")));
//                    productHsn.setBranch(branch);
                    productHsn.setOutlet(outlet);
                    productHsn.setStatus(true);
//                    productHsn.setCreatedBy(users.getId());
            ...
            if (paramMap.containsKey("fsr")) product.setFsr(Double.parseDouble(request.getParameter("fsr")));
            if (paramMap.containsKey("csr")) product.setCsr(Double.parseDouble(request.getParameter("csr")));
            ...
            if (paramMap.containsKey("cantentMapData")) {
                insertIntoProductContentMaster(newProduct, request);
//            else updateShippingDetails(mLedger, request);
            }
            ...
            for (int i = 0; i < array.size(); i++) {
                ...
                if (!object.get("selectedLevelA").getAsString().equalsIgnoreCase("")) {
                    if (branch == null) {
                        levelA = levelARepository.findByOutletIdAndStatusAndLevelNameIgnoreCaseAndBranchIsNull(outlet.getId(), true, object.get("selectedLevelA").getAsString());
                    } else {
                        levelA = levelARepository.findByOutletIdAndBranchIdAndStatusAndLevelNameIgnoreCase(outlet.getId(), branch.getId(), true, object.get("selectedLevelA").getAsString());
                    }
                }
                if (!object.get("selectedLevelB").getAsString().equalsIgnoreCase("")) {
                    if (branch == null) {
                        levelB = levelBRepository.findByOutletIdAndStatusAndLevelNameIgnoreCaseAndBranchIsNull(outlet.getId(), true, object.get("selectedLevelB").getAsString());
                    } else {
                        levelB = levelBRepository.findByOutletIdAndBranchIdAndStatusAndLevelNameIgnoreCase(outlet.getId(), branch.getId(), true, object.get("selectedLevelB").getAsString());
                    }
                }
                if (!object.get("selectedLevelC").getAsString().equalsIgnoreCase("")) {
                    if (branch == null) {
                        levelC = levelCRepository.findByOutletIdAndStatusAndLevelNameIgnoreCaseAndBranchIsNull(outlet.getId(), true, object.get("selectedLevelC").getAsString());
                    } else {
                        levelC = levelCRepository.findByOutletIdAndBranchIdAndStatusAndLevelNameIgnoreCase(outlet.getId(), branch.getId(), true, object.get("selectedLevelC").getAsString());
                    }
                }
                Units unit = unitsRepository.findFirstByUnitNameIgnoreCase(object.get("selectedUnit").getAsString());
                if (unit == null) {
                    unit = new Units();
                    unit.setUnitName(object.get("selectedUnit").getAsString());
                    unit.setOutlet(outlet);
                    if (branch != null)
                        unit.setBranch(branch);
                    unit.setStatus(true);
                    unit.setUnitCode(object.get("selectedUnit").getAsString());
                    unitsRepository.save(unit);
                }
                ProductUnitPacking productUnitPacking = new ProductUnitPacking();
                productUnitPacking.setUnits(unit);
                ...
                if (object.has("is_negetive"))
                    productUnitPacking.setIsNegativeStocks(object.get("is_negetive").getAsBoolean());
                if (object.has("mrp")) productUnitPacking.setMrp(object.get("mrp").getAsDouble());
//                if (object.has("pur_rate")) productUnitPacking.setPurchaseRate(object.get("pur_rate").getAsDouble());
                if (object.has("min_margin")) productUnitPacking.setMinMargin(object.get("min_margin").getAsDouble());
                    /*if (object.has("rate_1")) productUnitPacking.setMinRateA(object.get("rate_1").getAsDouble());//sales Rate
                    if (object.has("rate_2")) productUnitPacking.setMinRateB(object.get("rate_2").getAsDouble());
                    if (object.has("rate_3")) productUnitPacking.setMinRateC(object.get("rate_3").getAsDouble());*/
/*                if (object.has("fsrmh")) productUnitPacking.setMinRateA(object.get("fsrmh").getAsDouble());//sales Rate
                if (object.has("fsrai")) productUnitPacking.setMinRateB(object.get("fsrai").getAsDouble());
                if (object.has("csrmh")) productUnitPacking.setMinRateC(object.get("csrmh").getAsDouble());
                if (object.has("csrai")) productUnitPacking.setMinRateC(object.get("csrai").getAsDouble());*/
                productUnitPacking.setIsRate(object.get("is_rate").getAsBoolean());
                if (request.getHeader("stateCode").equalsIgnoreCase("27")) {
                    productUnitPacking.setPurchaseRate(object.get("fsrmh").getAsDouble());
                    productUnitPacking.setFsrmh(object.get("csrmh").getAsDouble());
                    productUnitPacking.setFsrai(0.0);
                    productUnitPacking.setCsrmh(0.0);
                    productUnitPacking.setCsrai(0.0);
...
//                            productUnitPacking.setCsrai(object.get("rate_4").getAsDouble());
                    ...
                    productUnitPacking.setPurchaseRate(object.get("fsrai").getAsDouble());
                    productUnitPacking.setFsrmh(object.get("csrai").getAsDouble());
                    productUnitPacking.setFsrai(0.0);
                    productUnitPacking.setCsrmh(0.0);
                    productUnitPacking.setCsrai(0.0);
//                            productUnitPacking.setFsrmh(object.get("fsrai").getAsDouble());
//                            productUnitPacking.setFsrai(object.get("rate_2").getAsDouble());
//                            productUnitPacking.setCsrmh(object.get("rate_3").getAsDouble());
//                            productUnitPacking.setCsrai(object.get("rate_4").getAsDouble());
                }
...
//                productUnitPacking.setCreatedBy(users.getId());
                /**** Modification after PK visits at Solapur 25th to 30th January 2023 ******/
                if (object.has("min_qty")) productUnitPacking.setMinQty(object.get("min_qty").getAsDouble());
                if (object.has("max_qty")) productUnitPacking.setMaxQty(object.get("max_qty").getAsDouble());

                if (object.has("selectedLevelA")) productUnitPacking.setLevelA(levelA);
                if (object.has("selectedLevelB")) productUnitPacking.setLevelB(levelB);
                if (object.has("selectedLevelC")) productUnitPacking.setLevelC(levelC);
                productUnitRepository.save(productUnitPacking);
            ...
            responseObject.setResponseStatus(HttpStatus.OK.value());
            responseObject.setMessage("Product Created Successfully at Franchise");

            return responseObject;
        } catch (Exception e) {

            responseObject.setResponseStatus(HttpStatus.INTERNAL_SERVER_ERROR.value());
            responseObject.setMessage("Failed to create proudct at Franchise!" + e.toString());
            ...
            // return "Product :"+newProduct.getProductName()+" Failed to Create";
        }
    ...
    public Object updateProductFromGv(MultipartHttpServletRequest request) {
        Product product = productRepository.findByProductCode(request.getParameter("productCode"));

        Product newProduct = new Product();
        Map<String, String[]> paramMap = request.getParameterMap();
        ResponseMessage responseObject = new ResponseMessage();
        FileStorageProperties fileStorageProperties = new FileStorageProperties();
        System.out.println("branchHeader=>" + request.getHeader("branch"));
        Users users = usersRepository.findTop1ByUserRoleIgnoreCaseAndCompanyCode("cadmin", request.getHeader("branch"));
        Branch branch = null;
        Outlet outlet = users.getOutlet();
        try {
...
//            if (paramMap.containsKey("uploadImage")) product.setUploadImage(request.getParameter("uploadImage"));
            ...
            if (paramMap.containsKey("taxPercentage")) {
                LocalDate applicableDate = null;
                String tax = Constants.decimalFormat.format(Double.parseDouble(request.getParameter("taxPercentage")));
                TaxMaster taxMaster = taxMasterRepository.findDuplicateGSTWithOutlet(outlet.getId(), tax, true);
                if (taxMaster == null) {
                    Double tx = Double.parseDouble(request.getParameter("taxPercentage"));
                    taxMaster = new TaxMaster();
                    taxMaster.setGst_per(tx + "");
            ...
            if (paramMap.containsKey("fsr")) product.setFsr(Double.parseDouble(request.getParameter("fsr")));
            if (paramMap.containsKey("csr")) product.setFsr(Double.parseDouble(request.getParameter("csr")));
...
//                ProductUnitPacking productUnitPacking = productUnitRepository.findTop1ByProductIdAndStatus(newProduct.getId(), true);
                ProductUnitPacking productUnitPacking = productUnitRepository.findByProductIdAndStatusAndUnitsId(newProduct.getId(), true, unit.getId());
                System.out.println("productUnit " + productUnitPacking.getId());
                System.out.println("ProductUnitName" + productUnitPacking.getUnits().getUnitName());
...
//                            productUnitPacking.setCsrai(object.get("rate_4").getAsDouble());
               ...
               /* System.out.println("Rate:"+object.get("csrmh").getAsDouble());
                System.out.println("Pur Rate:"+object.get("fsrmh").getAsDouble());
*/
                productUnitRepository.save(productUnitPacking);
            }

            /***** set Rate conversion for MultiUnit *****/
            List<ProductUnitPacking> unitPackingList = productUnitRepository.findByProductIdAndStatus(newProduct.getId(), true);
    ...
    public Object FRProductList(Map<String, String> request, HttpServletRequest req) {
        Users users = jwtRequestFilter.getUserDataFromToken(req.getHeader("Authorization").substring(7));
        ResponseMessage responseMessage = new ResponseMessage();
...
//            String query = "SELECT * FROM `product_tbl` LEFT JOIN packing_master_tbl ON product_tbl.packing_master_id=packing_master_tbl.id WHERE product_tbl.outlet_id=" + users.getOutlet().getId() + " AND product_tbl.status=1";
            String query = "SELECT * FROM `product_tbl` WHERE product_tbl.outlet_id=" + users.getOutlet().getId() + " AND product_tbl.status=1";
            if (users.getBranch() != null) {
                query = query + " AND product_tbl.branch_id=" + users.getBranch().getId();
            } else {
                query = query + " AND product_tbl.branch_id IS NULL";
...
//            }
            ...
            for (Product mDetails : productDetails) {
                productDTOList.add(convertProductToFRDTD(mDetails, users));
            }
    ...
    private FRProductDTO convertProductToFRDTD(Product mDetails, Users users) {
        FiscalYear fiscalYear = generateFiscalYear.getFiscalYear(LocalDate.now());
        FRProductDTO frProductDTO = new FRProductDTO();
