...
import in.truethics.ethics.ethicsapiv10.repository.ledgerdetails_repository.LedgerLicenseDetailsRepository;
...
//                outlet.setDistrictId(Long.valueOf(request.getParameter("districtId")));
//                outlet.setFranStateId(Long.valueOf(request.getParameter("stateId")));
//                outlet.setZoneId(Long.valueOf(request.getParameter("zoneId")));
//                outlet.setRegionalId(Long.valueOf(request.getParameter("regionalId")));
            ...
            if (outlet != null && !request.getHeader("branch").equalsIgnoreCase("gvmh001")) {
                ...
                ledgerMaster.setOutlet(mOutlet);
                ledgerMaster.setIsCredit(false);
                ledgerMaster.setIsLicense(true);
                ledgerMaster.setIsShippingDetails(false);
                ledgerMaster.setIsDepartment(false);
                ledgerMaster.setIsBankDetails(true);
                ledgerMaster.setGstin("27AAKCG7827N1ZU");

                LedgerMaster mLedger = ledgerMasterRepository.save(ledgerMaster);

                //Save License details
                LedgerLicenseDetails ledgerLicenseDetails=new LedgerLicenseDetails();
                ledgerLicenseDetails.setLedgerMaster(mLedger);
                ledgerLicenseDetails.setLicenseNum("20B-MH-TZ3-547683/21B-MH-TZ3-547684");
                ledgerLicenseDetails.setLicenseExp(LocalDate.of(2029,1,30));
                ledgerLicenseDetails.setSlugName("drug_number");
                ledgerLicenseDetails.setStatus(true);

                ledgerLicenseDetailsRepository.save(ledgerLicenseDetails);


                //Save GST details
                LedgerGstDetails ledgerGstDetails=new LedgerGstDetails();
                ledgerGstDetails.setGstin("27AAKCG7827N1ZU");
                ledgerGstDetails.setPanCard("AAKCG7827N");
                ledgerGstDetails.setDateOfRegistration(LocalDate.of(2024,1,3));
                ledgerGstDetails.setLedgerMaster(mLedger);
                ledgerGstDetails.setRegistrationType(1L);
                ledgerGstDetails.setStateCode(state.getStateCode());
                ledgerGstDetails.setStatus(true);

                ledgerGstDetailsRepository.save(ledgerGstDetails);

                /****** Bank Details of GV ****/

                System.out.println(serverGVUrl + "/getGvBankLedgers");
                ...
                String bankData = restTemplate.exchange(
                        serverGVUrl + "/getGvBankLedgers", HttpMethod.POST, gvEntity, String.class).getBody();
                System.out.println("bankData Response => "+bankData);
                if(!bankData.isEmpty() && bankData != null){
//                        String apiResponse=request.getParameter("data");
//                        System.out.println("apiResponse=>"+apiResponse);
                    JsonObject response = new JsonParser().parse(bankData).getAsJsonObject();

                    if(response.get("responseStatus").getAsInt() == 200) {
                        JsonArray bankList = response.get("data").getAsJsonArray();
                        for (int i = 0; i < bankList.size(); i++) {
                            JsonObject obj = bankList.get(i).getAsJsonObject();

                            LedgerBankDetails bankDetails = new LedgerBankDetails();
                            bankDetails.setStatus(true);
                            bankDetails.setBankName(obj.get("bankName").getAsString());
                            bankDetails.setAccountNo(obj.get("accountNo").getAsString());
                            bankDetails.setIfsc(obj.get("ifsc").getAsString());
                            bankDetails.setBankBranch(obj.get("branchName").getAsString());
                            bankDetails.setCreatedBy(mLedger.getCreatedBy());
                            bankDetails.setLedgerMaster(mLedger);
                            ledgerBankDetailsRepository.save(bankDetails);
                        }
...
//                    responseObject.setResponseObject(mAppConfig.getId().toString());
    ...
    public JsonObject getOutletById(HttpServletRequest request) {
        ...
        if (mOutlet != null) {
            ...
            response.addProperty("uploadImage", mOutlet.getCompanyLogo() != null ? serverGVUrl +
