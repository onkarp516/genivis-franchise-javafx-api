...
import in.truethics.ethics.ethicsapiv10.dto.purchasedto.FRPurOrderDTO;
import in.truethics.ethics.ethicsapiv10.dto.purchasedto.PurOrderDTO;
import in.truethics.ethics.ethicsapiv10.dto.purchasedto.PurOrderGVRowResDTO;
...
import in.truethics.ethics.ethicsapiv10.util.ExceptionUtil;
...
public class TranxPurOrderService {
    ...
    public Object insertPOInvoice(HttpServletRequest request) {
        ...
        if (mPurchaseTranx != null) {
            ...
            /*** CROSS API to INSERT SO INTO GV DB *****/
            if (mPurchaseTranx.getSundryCreditors().getLedgerCode().equalsIgnoreCase("gvmh001") &&
                ...
                gvBody.add("order_reference_no", request.getParameter("invoice_no"));
                ...
                try {
                    ...
                    purOrderLogger.error("Exception on Calling Url -> " + serverUrl + "/po_to_so_invoices" + "Request Body " + gvBody.toString() + "Exception " + ExceptionUtil.getExceptionString(e));
                }
...
//                        Users users = usersRepository.findByUsername(uname);
                    ...
                    body.add("newReference", false);
                    body.add("bill_no", csCode);
                    body.add("reference_no", request.getParameter("invoice_date"));
                    body.add("sales_acc_id", salesAcc.getId());
                    body.add("sales_sr_no", count + 1);
                    body.add("transaction_date", request.getParameter("transaction_date"));
                    body.add("debtors_id", franchiseLedger.getId());
...
//        LocalDate mDate = LocalDate.parse(request.getParameter("invoice_date"));
        ...
        if (sundryCreditors.getLedgerCode().equalsIgnoreCase("gvmh001")) {
            tranxPurOrder.setOrderStatus("pickup");// if ledger is NON GV: NON Gv Purchase
        } else {
            tranxPurOrder.setOrderStatus("pending");// if ledger is NON GV: NON Gv Purchase
        }
...
//        System.out.println("request " + request + "  req=" + req);
    ...
    private PurOrderDTO convertToDTDTO(TranxPurOrder purOrder) {
        ...
        purOrderDTO.setOrderStatus(purOrder.getOrderStatus() != null ? purOrder.getOrderStatus() : "");
        return purOrderDTO;
 ...
 /*       String startYear = generateFiscalYear.getStartYear();
    ...
    public JsonObject getPOInvoiceWithIds(HttpServletRequest request) {
        ...
        for (JsonElement mList : jsonArray) {
            ...
            for (TranxPurOrderDetailsUnits mUnits : unitsArray) {
                if (mUnits.getTranxPurOrder().getSundryCreditors().getLedgerCode().equalsIgnoreCase("gvmh001") &&
                        mUnits.getTranxPurOrder().getTransactionTrackingNo() != null) {
                    //!: get order details from gv server
                    return getPOInvoiceWithIdsFromGV(mUnits.getTranxPurOrder().getTransactionTrackingNo(), unitsArray);
                } else {
                    return getPOInvoiceWithIdsNormal(request);
                }
            }
        }
    ...
    private JsonObject getPOInvoiceWithIdsFromGV(String transactionTrackingNo, List<TranxPurOrderDetailsUnits> unitsArray) {
        /***** Calling Batch Details of GV against Tracking No ****/
        ...
        gvHdr.add("branch", "gvmh001");
        LinkedMultiValueMap gvBody = new LinkedMultiValueMap();
        gvBody.add("trackingNo", transactionTrackingNo);
        HttpEntity gvEntity = new HttpEntity<>(gvBody, gvHdr);
        JsonObject output = new JsonObject();
        try {
            String gvData = restTemplate.exchange(
                    serverUrl + "/get_sales_invoice_by_track_no", HttpMethod.POST, gvEntity, String.class).getBody();
            JsonObject convertedObject = new Gson().fromJson(gvData, JsonObject.class);
            System.out.println("ConvertedObj -> " + convertedObject);
            JsonObject invoiceData = new JsonObject();
            JsonArray row = new JsonArray();
            JSONArray inRow = null;
            if (convertedObject.has("row") && !convertedObject.get("row").isJsonNull()) {
                inRow = new JSONArray(convertedObject.get("row").getAsJsonArray().toString());
            }
            for (TranxPurOrderDetailsUnits mUnits : unitsArray) {
                ...
                System.out.println("in productName" + mUnits.getProduct().getProductName());
                JSONObject findOBj = null;
                if (inRow != null) {
                    findOBj = findJsonArrToObj(inRow, "product_name", mUnits.getProduct().getProductName());
                }

                System.out.println("findOBJ" + findOBj);

                ProductUnitPacking unitPackingById = productUnitRepository.findByProductIdAndStatusAndUnitsId(mUnits.getProduct().getId(), true, mUnits.getUnits().getId());
                if (findOBj == null) {
                    ...
                    unitsJsonObjects.addProperty("rate", mUnits.getRate() != null ? mUnits.getRate() : 0.0);
                    ...
                    unitsJsonObjects.addProperty("mrp", unitPackingById.getMrp() != null ? unitPackingById.getMrp() : 0.0);
                } else {
                    PurOrderGVRowResDTO purOrderGVRowResDTO = new Gson().fromJson(findOBj.toString(), PurOrderGVRowResDTO.class);
                    System.out.println("purOrderGVRowResDTO" + purOrderGVRowResDTO);
                    unitsJsonObjects.addProperty("level_a_id", mUnits.getLevelA() != null ?
                            mUnits.getLevelA().getId().toString() : "");
                    unitsJsonObjects.addProperty("level_b_id", mUnits.getLevelB() != null ?
                            mUnits.getLevelB().getId().toString() : "");
                    ...
                    unitsJsonObjects.addProperty("unitId", mUnits.getUnits().getId());
                    unitsJsonObjects.addProperty("unit_conv", mUnits.getUnitConversions());
                    unitsJsonObjects.addProperty("qty", purOrderGVRowResDTO.getQty());
                    unitsJsonObjects.addProperty("rate", purOrderGVRowResDTO.getRate() != null ? purOrderGVRowResDTO.getRate() : 0.0);
                    unitsJsonObjects.addProperty("pack_name", mUnits.getProduct().getPackingMaster() != null ?
                            mUnits.getProduct().getPackingMaster().getPackName() : "");
                    unitsJsonObjects.addProperty("base_amt", purOrderGVRowResDTO.getBaseAmt());
                    unitsJsonObjects.addProperty("dis_amt", purOrderGVRowResDTO.getDisAmt());
                    unitsJsonObjects.addProperty("dis_per", purOrderGVRowResDTO.getDisPer());
                    unitsJsonObjects.addProperty("dis_per_cal", purOrderGVRowResDTO.getDisPerCal());
                    unitsJsonObjects.addProperty("dis_amt_cal", purOrderGVRowResDTO.getDisAmtCal());
                    unitsJsonObjects.addProperty("total_amt", purOrderGVRowResDTO.getTotalAmt());
                    unitsJsonObjects.addProperty("gst", mUnits.getIgst());
                    unitsJsonObjects.addProperty("igst", mUnits.getIgst());
                    unitsJsonObjects.addProperty("cgst", mUnits.getCgst());
                    unitsJsonObjects.addProperty("sgst", mUnits.getSgst());
                    unitsJsonObjects.addProperty("total_igst", purOrderGVRowResDTO.getTotalIgst());
                    unitsJsonObjects.addProperty("total_cgst", purOrderGVRowResDTO.getTotalCgst());
                    unitsJsonObjects.addProperty("total_sgst", purOrderGVRowResDTO.getTotalSgst());
                    unitsJsonObjects.addProperty("final_amt", mUnits.getFinalAmount());
                    unitsJsonObjects.addProperty("free_qty", purOrderGVRowResDTO.getFreeQty());
                    unitsJsonObjects.addProperty("dis_per2", mUnits.getDiscountBInPer());
                    unitsJsonObjects.addProperty("row_dis_amt", mUnits.getTotalDiscountInAmt());
                    unitsJsonObjects.addProperty("gross_amt", purOrderGVRowResDTO.getGrossAmt());
                    unitsJsonObjects.addProperty("grossAmt1", purOrderGVRowResDTO.getGrossAmt1());
                    unitsJsonObjects.addProperty("invoice_dis_amt", purOrderGVRowResDTO.getInvoiceDisAmt());
                    unitsJsonObjects.addProperty("reference_id", mUnits.getTranxPurOrder().getId());
                    unitsJsonObjects.addProperty("reference_type", tranxType.getTransactionCode());
                    unitsJsonObjects.addProperty("mrp", unitPackingById.getMrp() != null ? unitPackingById.getMrp() : 0.0);

                    if (purOrderGVRowResDTO.getBatch() == true) {
                        ProductBatchNo mproductBatchNo = new ProductBatchNo();
                        mproductBatchNo.setBatchNo(purOrderGVRowResDTO.getBatchNo());
//                        mproductBatchNo.setMrp(0.0);
//                        if (findOBj.has("b_rate"))
                        mproductBatchNo.setMrp(mUnits.getProduct().getMrp());
                        mproductBatchNo.setPurchaseRate(0.0);
                        if (findOBj.has("purchase_rate")
                                && purOrderGVRowResDTO.getPurchaseRate() > 0)
                            mproductBatchNo.setPurchaseRate(purOrderGVRowResDTO.getRate());
                        if (findOBj.has("expiry_date") &&
                                !findOBj.getString("expiry_date").equalsIgnoreCase("") &&
                                !findOBj.getString("expiry_date").toLowerCase().contains("invalid"))
                            mproductBatchNo.setExpiryDate(LocalDate.parse(purOrderGVRowResDTO.getbExpiry()));
                        mproductBatchNo.setQnty(purOrderGVRowResDTO.getQty());
                        mproductBatchNo.setFreeQty(purOrderGVRowResDTO.getFreeQty());
                        mproductBatchNo.setSalesRate(0.0);
                        if (findOBj.has("sales_rate") && purOrderGVRowResDTO.getSalesRate() > 0)
                            mproductBatchNo.setSalesRate(purOrderGVRowResDTO.getSalesRate());
                        if (findOBj.has("costing") && purOrderGVRowResDTO.getCosting() > 0)
                            mproductBatchNo.setCosting(purOrderGVRowResDTO.getCosting());
                        else
                            mproductBatchNo.setCosting(0.0);
                        if (findOBj.has("costingWithTax") && purOrderGVRowResDTO.getCostingWithTax() > 0)
                            mproductBatchNo.setCostingWithTax(purOrderGVRowResDTO.getCostingWithTax());
                        else
                        ...
                        if (findOBj.has("min_rate_a") && purOrderGVRowResDTO.getMinRateA() > 0)
                            mproductBatchNo.setMinRateA(purOrderGVRowResDTO.getMinRateA());
                        if (findOBj.has("min_rate_b") && purOrderGVRowResDTO.getMinRateB() > 0)
                            mproductBatchNo.setMinRateB(purOrderGVRowResDTO.getMinRateB());
                        if (findOBj.has("min_rate_c") && purOrderGVRowResDTO.getMinRateC() > 0)
                            mproductBatchNo.setMinRateC(purOrderGVRowResDTO.getMinRateC());
                        if (findOBj.has("min_margin") &&
                                purOrderGVRowResDTO.getMinMargin() > 0)
                            mproductBatchNo.setMinMargin(purOrderGVRowResDTO.getMinMargin());
                        if (findOBj.has("manufacturing_date") &&
                                !findOBj.getString("manufacturing_date").equalsIgnoreCase("") &&
                                !findOBj.getString("manufacturing_date").toLowerCase().contains("invalid"))
                            mproductBatchNo.setManufacturingDate(LocalDate.parse(purOrderGVRowResDTO.getManufacturingDate()));
                        mproductBatchNo.setStatus(true);

                        Product mProduct = productRepository.findByProductCode(purOrderGVRowResDTO.getProductCode());
                        mproductBatchNo.setProduct(mProduct);
/*                        mproductBatchNo.setOutlet(users.getOutlet());
                        mproductBatchNo.setBranch(users.getBranch());*/

                        mproductBatchNo.setUnits(mUnits.getUnits());

                        LedgerMaster supplier = ledgerMasterRepository.findByLedgerCodeAndStatusAndOutletId("gvmh001",
                                true, mUnits.getTranxPurOrder().getOutlet().getId());
                        mproductBatchNo.setSupplierId(supplier.getId());

                        ProductBatchNo productBatchNo = productBatchNoRepository.save(mproductBatchNo);
                        unitsJsonObjects.addProperty("b_detailsId", productBatchNo.getId());
                        unitsJsonObjects.addProperty("is_batch", purOrderGVRowResDTO.getBatch());
                        unitsJsonObjects.addProperty("product_name", productBatchNo.getProduct().getProductName());
                        unitsJsonObjects.addProperty("batch_no", purOrderGVRowResDTO.getBatchNo());
                        unitsJsonObjects.addProperty("qty", purOrderGVRowResDTO.getQty());
                        unitsJsonObjects.addProperty("free_qty", purOrderGVRowResDTO.getFreeQty());
                        unitsJsonObjects.addProperty("expiry_date", purOrderGVRowResDTO.getbExpiry());
                        unitsJsonObjects.addProperty("purchase_rate", purOrderGVRowResDTO.getRate());
                        unitsJsonObjects.addProperty("sales_rate", purOrderGVRowResDTO.getSalesRate());
                        unitsJsonObjects.addProperty("mrp", unitPackingById.getMrp() != null ? unitPackingById.getMrp() : 0.0);
                        unitsJsonObjects.addProperty("min_rate_a", unitPackingById.getFsrmh() != null ? unitPackingById.getFsrmh() : 0.0);
                        unitsJsonObjects.addProperty("min_rate_b", unitPackingById.getFsrai() != null ? unitPackingById.getFsrai() : 0.0);
                        unitsJsonObjects.addProperty("min_rate_c", unitPackingById.getCsrmh() != null ? unitPackingById.getCsrmh() : 0.0);
                        unitsJsonObjects.addProperty("manufacturing_date", purOrderGVRowResDTO.getManufacturingDate());
                        unitsJsonObjects.addProperty("min_margin", purOrderGVRowResDTO.getMinMargin());
                        unitsJsonObjects.addProperty("b_rate", purOrderGVRowResDTO.getbRate());
                        double costing = purOrderGVRowResDTO.getQty() * purOrderGVRowResDTO.getRate() / (purOrderGVRowResDTO.getQty() + purOrderGVRowResDTO.getFreeQty());
                        double costingWithTax = costing + (purOrderGVRowResDTO.getTotalIgst() / (purOrderGVRowResDTO.getQty() + purOrderGVRowResDTO.getFreeQty()));
                        unitsJsonObjects.addProperty("costing", costing);
                        unitsJsonObjects.addProperty("costingWithTax", costingWithTax);
                        unitsJsonObjects.addProperty("max_discount", purOrderGVRowResDTO.getMinDiscount());
                        unitsJsonObjects.addProperty("min_discount", purOrderGVRowResDTO.getMinDiscount());
                        unitsJsonObjects.addProperty("dis_per", purOrderGVRowResDTO.getDisPer());
                        unitsJsonObjects.addProperty("dis_amt", purOrderGVRowResDTO.getDisAmt());
                        unitsJsonObjects.addProperty("barcode", productBatchNo.getBarcode());
                        unitsJsonObjects.addProperty("pur_date", productBatchNo.getPurchaseDate() != null ? productBatchNo.getPurchaseDate().toString() : "");
                        unitsJsonObjects.addProperty("tax_per", productBatchNo.getProduct().getIgst());
                        unitsJsonObjects.addProperty("cess_amt", "");
                        unitsJsonObjects.addProperty("cess_per", "");
                        unitsJsonObjects.addProperty("fsrmh", purOrderGVRowResDTO.getMinRateB());
                ...
                System.out.println("Date:" + DateConvertUtil.convertDateToLocalDate(
                ...
                invoiceData.addProperty("transactionTrackingNo", mUnits.getTranxPurOrder().
                        getTransactionTrackingNo() != null ? mUnits.getTranxPurOrder().
                        getTransactionTrackingNo() : "");
            ...
            e.printStackTrace();
            purOrderLogger.error("Exception in getPOInvoiceWithIdsFromGV() " + ExceptionUtil.getExceptionString(e));
        }
    ...
    private JsonObject getPOInvoiceWithIdsNormal(HttpServletRequest request) {
        JsonObject output = new JsonObject();
        String str = request.getParameter("purchase_order_id");
        JsonParser parser = new JsonParser();
        JsonElement purDetailsJson = parser.parse(str);
        ...
        for (JsonElement mList : jsonArray) {
            ...
            for (TranxPurOrderDetailsUnits mUnits : unitsArray) {
                ...
                if (mUnits.getTranxPurOrder().getSundryCreditors().getLedgerCode().equalsIgnoreCase("gvmh001") &&
                    ...
                    LinkedMultiValueMap gvBody = new LinkedMultiValueMap();
                    gvBody.add("usercode", "gvmh001");
                    gvBody.add("companyCode", users.getCompanyCode());
                    gvBody.add("trackingNo", mUnits.getTranxPurOrder().getTransactionTrackingNo());
                    gvBody.add("productId", mUnits.getProduct().getProductCode());
                    HttpEntity gvEntity = new HttpEntity<>(gvBody, gvHdr);
                    String gvData = restTemplate.exchange(
                            serverUrl + "/get_gv_batch_data", HttpMethod.POST, gvEntity, String.class).getBody();
                    JsonObject convertedObject = new Gson().fromJson(gvData, JsonObject.class);
                    //! Create Batch of FR

                    if (convertedObject.get("is_batch").getAsBoolean() == true) {
                        ProductBatchNo mproductBatchNo = new ProductBatchNo();
                        if (convertedObject.has("batch_no"))
                            mproductBatchNo.setBatchNo(convertedObject.get("batch_no").getAsString());
                        mproductBatchNo.setMrp(0.0);
                        if (convertedObject.has("b_rate") && !convertedObject.get("b_rate").getAsString().equalsIgnoreCase(""))
                            mproductBatchNo.setMrp(convertedObject.get("b_rate").getAsDouble());
                        mproductBatchNo.setPurchaseRate(0.0);
                        if (convertedObject.has("purchase_rate")
                                && !convertedObject.get("purchase_rate").getAsString().equals(""))
                            mproductBatchNo.setPurchaseRate(convertedObject.get("purchase_rate").getAsDouble());
                        if (convertedObject.has("expiry_date") &&
                                !convertedObject.get("expiry_date").getAsString().equalsIgnoreCase("") &&
                                !convertedObject.get("expiry_date").getAsString().toLowerCase().contains("invalid"))
                            mproductBatchNo.setExpiryDate(LocalDate.parse(convertedObject.get("expiry_date").getAsString()));
                        mproductBatchNo.setQnty(convertedObject.get("qty").getAsInt());
                        mproductBatchNo.setFreeQty(convertedObject.get("free_qty").getAsDouble());
                        mproductBatchNo.setSalesRate(0.0);
                        if (convertedObject.has("sales_rate") && !convertedObject.get("sales_rate").getAsString().equals(""))
                            mproductBatchNo.setSalesRate(convertedObject.get("sales_rate").getAsDouble());
                        if (convertedObject.has("costing") && !convertedObject.get("costing").isJsonNull())
                            mproductBatchNo.setCosting(convertedObject.get("costing").getAsDouble());
                        else
                            mproductBatchNo.setCosting(0.0);
                        if (convertedObject.has("costingWithTax") && !convertedObject.get("costingWithTax").isJsonNull())
                            mproductBatchNo.setCostingWithTax(convertedObject.get("costingWithTax").getAsDouble());
                        else
                            mproductBatchNo.setCostingWithTax(0.0);
                        mproductBatchNo.setMinRateA(0.0);
                        mproductBatchNo.setMinRateB(0.0);
                        mproductBatchNo.setMinRateC(0.0);
                        if (convertedObject.has("min_rate_a") && !convertedObject.get("min_rate_a").getAsString().equalsIgnoreCase(""))
                            mproductBatchNo.setMinRateA(convertedObject.get("min_rate_a").getAsDouble());
                        if (convertedObject.has("min_rate_b") && !convertedObject.get("min_rate_b").getAsString().equalsIgnoreCase(""))
                            mproductBatchNo.setMinRateB(convertedObject.get("min_rate_b").getAsDouble());
                        if (convertedObject.has("min_rate_c") && !convertedObject.get("min_rate_c").getAsString().equalsIgnoreCase(""))
                            mproductBatchNo.setMinRateC(convertedObject.get("min_rate_c").getAsDouble());
                        if (convertedObject.has("min_margin") &&
                                !convertedObject.get("min_margin").getAsString().equals(""))
                            mproductBatchNo.setMinMargin(convertedObject.get("min_margin").getAsDouble());
                        if (convertedObject.has("manufacturing_date") &&
                                !convertedObject.get("manufacturing_date").getAsString().equalsIgnoreCase("") &&
                                !convertedObject.get("manufacturing_date").getAsString().toLowerCase().contains("invalid"))
                            mproductBatchNo.setManufacturingDate(LocalDate.parse(
                                    convertedObject.get("manufacturing_date").getAsString()));
                        mproductBatchNo.setStatus(true);

                        Product mProduct = productRepository.findByProductCode(convertedObject.get("product_code").getAsString());
                        mproductBatchNo.setProduct(mProduct);
                        mproductBatchNo.setOutlet(users.getOutlet());
                        mproductBatchNo.setBranch(users.getBranch());
                        ...
                        ProductBatchNo productBatchNo = productBatchNoRepository.save(mproductBatchNo);
                        unitsJsonObjects.addProperty("b_detailsId", productBatchNo.getId());
                        unitsJsonObjects.addProperty("is_batch", convertedObject.get("is_batch").getAsBoolean());
                        unitsJsonObjects.addProperty("product_name", convertedObject.get("product_name").getAsString());
                        unitsJsonObjects.addProperty("batch_no", convertedObject.get("batch_no").getAsString());
                        unitsJsonObjects.addProperty("qty", convertedObject.get("qty").getAsDouble());
                        unitsJsonObjects.addProperty("free_qty", convertedObject.get("free_qty").getAsDouble());
                        unitsJsonObjects.addProperty("expiry_date", convertedObject.get("expiry_date").getAsString());
                        unitsJsonObjects.addProperty("purchase_rate", convertedObject.get("purchase_rate").getAsDouble());
                        unitsJsonObjects.addProperty("sales_rate", convertedObject.get("sales_rate").getAsDouble());
                        unitsJsonObjects.addProperty("mrp", convertedObject.get("mrp").getAsDouble());
                        unitsJsonObjects.addProperty("min_rate_a", convertedObject.get("min_rate_a").getAsDouble());
                        unitsJsonObjects.addProperty("min_rate_b", convertedObject.get("min_rate_b").getAsDouble());
                        unitsJsonObjects.addProperty("min_rate_c", convertedObject.get("min_rate_c").getAsDouble());
                        unitsJsonObjects.addProperty("manufacturing_date", convertedObject.get("manufacturing_date").getAsString());
                        unitsJsonObjects.addProperty("min_margin", convertedObject.get("min_margin").getAsDouble());
                        unitsJsonObjects.addProperty("b_rate", convertedObject.get("b_rate").getAsDouble());
                        unitsJsonObjects.addProperty("costing", convertedObject.get("costing").getAsDouble());
                        unitsJsonObjects.addProperty("costingWithTax", convertedObject.get("costingWithTax").getAsDouble());
                        unitsJsonObjects.addProperty("max_discount", convertedObject.get("max_discount").getAsDouble());
                        unitsJsonObjects.addProperty("min_discount", convertedObject.get("min_discount").getAsDouble());
                        unitsJsonObjects.addProperty("dis_per", convertedObject.get("dis_per").getAsDouble());
                        unitsJsonObjects.addProperty("dis_amt", convertedObject.get("dis_amt").getAsDouble());
                        unitsJsonObjects.addProperty("cess_amt", convertedObject.get("cess_amt").getAsDouble());
                        unitsJsonObjects.addProperty("barcode", convertedObject.get("barcode").getAsString());
                        unitsJsonObjects.addProperty("pur_date", convertedObject.get("pur_date").getAsString());
                        unitsJsonObjects.addProperty("tax_per", convertedObject.get("tax_per").getAsDouble());
                        unitsJsonObjects.addProperty("cess_per", convertedObject.get("cess_per").getAsDouble());
                    }
                }
                row.add(unitsJsonObjects);
            }
...
//                    }
    ...
    public Object FrPoInvoiceList(String req, HttpServletRequest request) {
//        System.out.println("req => " + req);
        Users users = jwtRequestFilter.getUserDataFromToken(request.getHeader("Authorization").substring(7));
        ResponseMessage responseMessage = new ResponseMessage();
//        System.out.println("request => " + request);
//        String stDay = requestuest.getParameter("start_date");
        JSONObject inReqJson = new JSONObject(req);
        Integer pageNo = inReqJson.getInt("pageNo");
        Integer pageSize = inReqJson.getInt("pageSize");
        String searchText = inReqJson.getString("searchText");
        String startDate = inReqJson.getString("startDate");
        String endDate = inReqJson.getString("endDate");
        String frOrderStatus = inReqJson.getString("frOrderStatus");
        LocalDate endDatep = null;
        LocalDate startDatep = null;
        Boolean flag = false;
//        System.out.println("startdate " + startDatep + "  endDate " + endDatep);
        List<TranxPurOrder> purchaseOrder = new ArrayList<>();
        List<TranxPurOrder> purchaseArrayList = new ArrayList<>();
        List<FRPurOrderDTO> purOrderDTOList = new ArrayList<>();
        GenericDTData genericDTData = new GenericDTData();
        try {
            String query = "SELECT * FROM `tranx_purchase_order_tbl` WHERE outlet_id=" + users.getOutlet().getId() + " AND status=1";
            if (users.getBranch() != null) {
            ...
            if (!frOrderStatus.equalsIgnoreCase("")) {
                switch (frOrderStatus.toLowerCase()) {
                    case "pending":
                        query = query + " AND (order_status LIKE '%Placed%' OR order_status LIKE '%Ready For Delivery%' ) ";
                        break;
                    case "accepted":
                        query = query + " AND order_status LIKE '%Accepted%' ";
                        break;
                    case "dispatched":
                        query = query + " AND order_status LIKE '%Dispatched%' ";
                        break;

                }
            }
            JSONObject sortObj = inReqJson.getJSONObject("sort");
//            JsonObject jsonObject = new Gson().fromJson(jsonToStr, JsonObject.class);
            if (sortObj.getString("colId") != null && !sortObj.getString("colId").isEmpty()) {
                String sortBy = sortObj.getString("colId");
                query = query + " ORDER BY " + sortBy;
                if (sortObj.getBoolean("isAsc") == true) {
                    query = query + " ASC";
                } else {
                    query = query + " DESC";
                }
...
//            System.out.println("q ==" + q + "  purchaseOrder " + purchaseOrder);
            ...
            for (TranxPurOrder invoiceListView : purchaseOrder) {
                purOrderDTOList.add(convertToFRDTDTO(invoiceListView));
            }

            GenericDatatable<FRPurOrderDTO> data = new GenericDatatable<>(purOrderDTOList, purchaseArrayList.size(),
                    pageNo, pageSize, total_pages);
    ...
    public JSONObject FrPoLedgerDetails(HttpServletRequest request) {
        Users users = jwtRequestFilter.getUserDataFromToken(
                request.getHeader("Authorization").substring(7));
        Long count = 0L;
        if (users.getBranch() != null) {
 ...
 /*       String startYear = generateFiscalYear.getStartYear();
        ...
        /*String poCode = companyName.toUpperCase() + "-" + startYear + endYear
                + "-" + "PO" + currentMonth + "-" + serailNo;*/
        String poCode = "PO" + currentMonth + serailNo;
        //Ledger details of GV
        LedgerMaster mDetails = ledgerRepository.findByLedgerCodeAndStatus("gvmh001", true);
        JSONObject ledgerObj = new JSONObject();
        ledgerObj.put("id", mDetails.getId());
        ledgerObj.put("code", mDetails.getLedgerCode() != null ? mDetails.getLedgerCode() : "");
        ledgerObj.put("ledger_name", mDetails.getLedgerName());
        ledgerObj.put("ledger_code", mDetails.getLedgerCode() != null ? mDetails.getLedgerCode() : "");
        ledgerObj.put("unique_code", mDetails.getUniqueCode() != null ? mDetails.getUniqueCode() : "");
        List<LedgerGstDetails> gstDetails = new ArrayList<>();
        ...
        if (gstDetails != null && gstDetails.size() > 0) {
            for (LedgerGstDetails mGstDetails : gstDetails) {
                JSONObject mGstObject = new JSONObject();
                mGstObject.put("id", mGstDetails.getId());
                mGstObject.put("gstNo", mGstDetails.getGstin());
                mGstObject.put("state", mGstDetails.getStateCode() != null ? mGstDetails.getStateCode() : "");
                gstArray.put(mGstObject);
            }
        }
        ledgerObj.put("gstDetails", gstArray);
        JSONArray purResult = new JSONArray();
        purResult = ledgerMasterService.getLedgersJSONArray("Purchase Accounts", users.getOutlet().getId(), users.getBranch());
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
        String dttoday = df.format(new Date());
        JSONObject result = new JSONObject();
        result.put("message", "success");
        result.put("responseStatus", HttpStatus.OK.value());
        result.put("purchaseSerial", count + 1);
        result.put("orderNo", poCode);
        result.put("ledger", ledgerObj);
        result.put("purchase", purResult);
        result.put("tranxDate", dttoday);
        result.put("orderDate", dttoday);

        return result;
    }


    private FRPurOrderDTO convertToFRDTDTO(TranxPurOrder purOrder) {
        FRPurOrderDTO purOrderDTO = new FRPurOrderDTO();
        long total_cnt = tranxPurOrderDetailsRepository.countByIdAndStatus(purOrder.getId(), true);
        purOrderDTO.setId(purOrder.getId());
        purOrderDTO.setInvoice_no(purOrder.getVendorInvoiceNo());
        purOrderDTO.setInvoice_date(DateConvertUtil.convertDateToLocalDate(purOrder.getInvoiceDate()).toString());
        purOrderDTO.setTransaction_date(purOrder.getTransactionDate().toString());
        ...
        purOrderDTO.setTotalProducts(total_cnt);
//        purOrderDTO.setFrOrderStatus(purOrder.getOrderStatus()==null);
        purOrderDTO.setFrOrderStatus(purOrder.getOrderStatus() == null ? "Pending" : purOrder.getOrderStatus().equalsIgnoreCase("Placed") ? "Pending" : purOrder.getOrderStatus().equalsIgnoreCase("Order Accepted") ? "Accepted" : purOrder.getOrderStatus().equalsIgnoreCase("Dispatched") ? "Dispatched" : purOrder.getOrderStatus().equalsIgnoreCase("Ready For Delivery") ? "Pending" : "");
        return purOrderDTO;

    }

    public JsonArray getAllPurchaseOrders(HttpServletRequest request) {
        List<TranxPurOrder> orderList = tranxPurOrderRepository.findAll();
        ...
        if (!orderList.isEmpty()) {
            ...
            for (int i = 0; i < orderList.size(); i++) {
                JsonObject jsonObject = new JsonObject();
                TranxPurOrder salesOrderTransaction = orderList.get(i);

                if (salesOrderTransaction.getBranch() != null) branch = salesOrderTransaction.getBranch();
                outlet = salesOrderTransaction.getOutlet();
                sundryDebtors = salesOrderTransaction.getSundryCreditors();
                salesAccountLedger = salesOrderTransaction.getPurchaseAccountLedger();
                salesRoundOff = salesOrderTransaction.getPurchaseRoundOff();
...
//                salesQuotationDutiesTaxes = salesOrderTransaction.getSalesQuotationDutiesTaxes();
                ...
                jsonObject.addProperty("bill_date", DateConvertUtil.convertDateToLocalDate(salesOrderTransaction.getInvoiceDate()).toString());
                jsonObject.addProperty("bill_no", salesOrderTransaction.getPurOrdSrno());
                jsonObject.addProperty("sales_acc_id", salesAccountLedger.getId());
                jsonObject.addProperty("transaction_dt", salesOrderTransaction.getTransactionDate().toString());
...
//                jsonObject.addProperty("additionalChargesTotal", salesOrderTransaction.geta());
                jsonObject.addProperty("totalqty", salesOrderTransaction.getTotalqty());
                jsonObject.addProperty("sale_type", "sales order");
                ...
                Double orderDetailsUnits = tranxPurOrderDetailsUnitRepository.totalNumberOfProduct(salesOrderTransaction.getId(), 1, true);
    ...
    public void updatePurOrder(HttpServletRequest request) {
        Map<String, String[]> paramMap = request.getParameterMap();


    }

    private JSONObject findJsonArrToObj(JSONArray jsonArray, String findKey, String findValue) {
        JSONObject outObj = null;
        for (Object find : jsonArray) {
            if (((JSONObject) find).get(findKey).equals(findValue)) {
                outObj = (JSONObject) find;
            }
        }
        return outObj;
    }
